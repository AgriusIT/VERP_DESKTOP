...
Public Class frmProSales

    Implements IGeneral
    Dim Id As Integer = 0I
    Dim DetailAcccountId As Integer = 0
    Public Shared PSID As Integer
    Dim Sales As PropertySalesBE
    Public SalesDAL As PropertySalesDAL = New PropertySalesDAL()
    Dim DetailAcccount As COADeail
    Public DetailAcccountDAL As COADetailDAL = New COADetailDAL()
    Dim PropertyTypeId As Integer
    Dim list As List(Of PurchaseTypeSalesBE)
    Dim Tasklist As List(Of TaskSalesBE)
    Dim VoucherNo As String = ""
    Dim AdjustedVoucherNo As String = ""
    Dim date1 As String = ""
    ...
    Dim Account As Integer = 0 'Val(getConfigValueByType("BuyerSubSub").ToString)
    Public SellerPrice As Double = 0
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            Dim AccountSubSubId As Integer = Val(getConfigValueByType("BuyerSubSub").ToString)
                    ...
                    If COADetailDAL.GetAccountName(DetailAcccount.DetailTitle, AccountSubSubId) = True Then
                        ...
                        If msg_Confirm("Account name already exist do you want to Continue with same name") = True Then
                            ...
                            If DetailAcccountId > 0 Then
                                Sales.BuyerAccountId = DetailAcccountId
                            Else
                                DetailAcccountId = New COADetailDAL().Add(DetailAcccount, AccountSubSubId)
                                Sales.BuyerAccountId = DetailAcccountId
                            End If
                            If SalesDAL.Add(Sales) Then
                                If SalesDAL.AddPurchaseType(list) = True Then
                                    If SalesDAL.AddTasks(Tasklist) = True Then
                ...
                Else
                    ...
                    If COADetailDAL.GetAccountName(DetailAcccount.DetailTitle, AccountSubSubId) = True Then
                        ...
                        If msg_Confirm("Account name already exist do you want to Continue with same name") = True Then
                            ...
                            'Sales.BuyerAccountId = DetailAcccountId
                            If SalesDAL.Update(Sales) Then
                                If SalesDAL.UpdatePurchaseType(list) = True Then
                                    If SalesDAL.UpdateTasks(Tasklist) = True Then
    ...
    Private Sub frmProPurchase_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            Account = Val(getConfigValueByType("BuyerSubSub").ToString)

            If Account <= 0 Then

                ShowErrorMessage("Please Select a sub sub Account to map Against the Buyer")
            ...
            Dim dt As DataTable
            'Saba Shabbir TFS2558 added a new function in propertySalesDAL 
            If (EditAble = -1) Then
                dt = New PropertySalesDAL().UpdateRecord(DocNo)
                If dt.Rows.Count > 0 Then
                    PSID = dt.Rows(0).Item("PropertySalesId")
                Else
                    PSID = -1
                End If
            End If
            'ToolTip
            dt = New PropertySalesDAL().GetById(PSID)
            Dim i As Integer
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    ...
                    'Sales.SellerAccountId = txtCNIC.Text

                    txtPropertyType.Text = dt.Rows(0).Item("PropertyType")
                    txtCellNo.Text = dt.Rows(0).Item("CellNo")
                    cmbName.Text = GetAccountById(Val(dt.Rows(0).Item("BuyerAccountId").ToString))
                    txtPrice.Text = dt.Rows(0).Item("Price")
                    VoucherNo = dt.Rows(0).Item("voucher_no").ToString
                    AdjustedVoucherNo = dt.Rows(0).Item("AdjustedVoucherNo").ToString
                    ...
                    If IsDBNull(dt.Rows(0).Item("SalesDate")) Then
                        Me.dtpSalesDate.Value = Now
                    Else
                        Me.dtpSalesDate.Value = dt.Rows(0).Item("SalesDate")
                    End If
                Next
            Else
                ReSetControls()
            ...
            Ttip.SetToolTip(Me.cmbName, "Select Buyer account")
            Ttip.SetToolTip(Me.cmbName, "Enter property price")
    ...
    Private Function GetPaymentType(ByVal PSID As Integer) As Boolean
        Try
            Dim str As String
            Dim dt As DataTable
            str = "Select PaymentTypeId, PropertyPurchaseId, PropertySalesId, PaymentTypeDate, PaymentTypeName, Isnull(Amount, 0) as Amount, Isnull(AmountPaid, 0) as AmountPaid from PaymentType where PropertysalesId = " & PSID & ""
            dt = GetDataTable(str)
            If dt.Rows.Count > 0 Then
                ...
                Me.grdPayment.RootTable.Columns("PropertySalesId").Visible = False
    ...
    Private Function GetTaskTemplate() As Boolean
        Try
            ...
            Dim dt As DataTable
            If PSID > 0 Then
                str = "select TaskId, Name, DueDate, Completed as Status  from TblDefTasks where FormName = 'frmProSales' AND Ref_No = N'" & txtSerialNo.Text & "'"
                dt = GetDataTable(str)
                If dt.Rows.Count > 0 Then
                ...
                End If
            Else
                str = "select TaskTemplateDetailId as TaskId, TaskTemplateId, Title as Name, DueDate, isNull(Status,0) as Status from TaskTemplateDetail where TaskTemplateId = 2"
                dt = GetDataTable(str)
                If dt.Rows.Count > 0 Then
                    grdTask.DataSource = dt
                    grdTask.RetrieveStructure()
                    ...
                    'status = grdTask.RootTable.Columns("Status").ToString
                    'If grdTask.RootTable.Columns("Status").ToString = 0 Then
                    '    'grdTask.RootTable.Columns("Status").edi()
                    'End If
                    'If (status = "") Then
                    '    Val(Me.grdTask.RootTable.Columns("Status").ToString) = 0
                    'End If
                End If
            End If
        Catch ex As Exception
    ...
    '        Dim dt As DataTable
    '        If e.KeyCode = Keys.Enter Then
    '            str = "SELECT        PropertyItem.PropertyItemId, PropertyItem.Title, PropertyItem.EntryDate, PropertyItem.PlotNo, PropertyItem.Sector, PropertyItem.Block, PropertyItem.PlotSize,tblListTerritory.TerritoryName, PropertyItem.Ownership, PropertyType.PropertyType, " & _
    ...
    '            dt = GetDataTable(str)
    '            If dt.Rows.Count > 0 Then
    '                ItemId = dt.Rows(0).Item("PropertyItemId").ToString
    ...
    Function GetAccountById(ByVal ID As Integer) As String
        ...
        Try
            ...
            strSQL = " SELECT        tblCOAMainSubSubDetail.coa_detail_id, tblCOAMainSubSubDetail.detail_title, ISNULL(SUM(tblVoucherDetail.Credit_Amount),0) as Amount ,IsNull(SUM(tblVoucherDetail.debit_amount) - SUM(tblVoucherDetail.credit_amount), 0) as NetAmount " & _
    ...
    Public Sub FillModel(Optional Condition As String = "") Implements IGeneral.FillModel
        Try
            ...
            Sales.PropertySalesId = PSID
            Sales.PropertyDate = Me.dtpSalesDate.Value
            Sales.Title = Me.txtTitle.Text
            Sales.PlotNo = Me.txtPlotNo.Text
            Sales.Block = Me.txtBlockNo.Text
            Sales.Sector = txtSector.Text
            Sales.Remarks = txtRemarks.Text
            Sales.SerialNo = txtSerialNo.Text
            Sales.Location = txtLocation.Text
            Sales.City = txtCity.Text
            Sales.CellNo = txtCellNo.Text
            Sales.ItemId = ItemId
            Sales.SalesId = getConfigValueByType("PropertySalesAccount").ToString
            '' TASK TFS4412
            If Not getConfigValueByType("PropertyPurchaseAccount") = "Error" Then
                Sales.PurchaseAccountId = Val(getConfigValueByType("PropertyPurchaseAccount"))
            End If
            If Not getConfigValueByType("CostOfSalesAccountProperty") = "Error" Then
                Sales.CostOfSalesAccountId = Val(getConfigValueByType("CostOfSalesAccountProperty"))
            End If
            ''END TASK TFS4412
            'Sales.SellerAccountId = txtCNIC.Text
            Sales.Price = txtPrice.Text
            ''TASK TFS4412
            Sales.SellerPrice = SellerPrice
            ''
            Sales.PropertyType = txtPropertyType.Text
            Sales.Cost_CenterId = Cost_CenterId
            Sales.PlotSize = txtLandArea.Text
            If PSID = -1 Then
                Sales.VoucherNo = GetDocumentNo()
                Sales.AdjustedVoucherNo = GetAdjustedVoucherNo()
            Else
                If VoucherNo.Length > 0 Then
                    Sales.VoucherNo = VoucherNo
                Else
                    Sales.VoucherNo = GetDocumentNo()
                End If
                If AdjustedVoucherNo.Length > 0 Then
                    Sales.AdjustedVoucherNo = AdjustedVoucherNo
                Else
                    Sales.VoucherNo = GetDocumentNo()
                    Sales.AdjustedVoucherNo = GetAdjustedVoucherNo()
                ...
                End If
            End If
            Sales.ActivityLog = New ActivityLog
            list = New List(Of PurchaseTypeSalesBE)
            For i As Integer = 0 To grdPayment.RowCount - 1
                Dim PDetail As New PurchaseTypeSalesBE
                ...
                list.Add(PDetail)
            Next
            Tasklist = New List(Of TaskSalesBE)
            For i As Integer = 0 To grdTask.RowCount - 1
                Dim TDetail As New TaskSalesBE
    ...
    Public Function IsValidate(Optional Mode As SBUtility.Utility.EnumDataMode = SBUtility.Utility.EnumDataMode.Disabled, Optional Condition As String = "") As Boolean Implements IGeneral.IsValidate
        Try
            ...
            '    Return False
            'End If
            If Me.cmbName.Text = ".... Select Any Value ...." Then
    ...
    Function GetDocumentNo() As String
        Try
            If getConfigValueByType("VoucherNo").ToString = "Yearly" Then
                Return GetSerialNo("PS-" + Microsoft.VisualBasic.Right(dtpSalesDate.Value.Year, 2) + "-", "tblVoucher", "voucher_no")
            ElseIf getConfigValueByType("VoucherNo").ToString = "Monthly" Then
                Return GetNextDocNo("PS-" & Format(dtpSalesDate.Value, "yy") & dtpSalesDate.Value.Month.ToString("00"), 4, "tblVoucher", "voucher_no")
            Else
                Return GetNextDocNo("PS-", 6, "tblVoucher", "Voucher_No")
            End If
        Catch ex As Exception
            Throw ex
        End Try
    ...
    'When text will be chnaged then the lable value will be changed. Added by Abubakar
    ...
    '        Dim str As String
    '        Dim dt As DataTable
    '        str = "SELECT PropertySales.CellNo FROM PropertySales INNER JOIN vwCOADetail ON PropertySales.BuyerAccountId = vwCOADetail.coa_detail_id where vwCOADetail.main_sub_sub_id = " & Account & " and detail_title = '" & cmbName.Text & "'"
    '        dt = GetDataTable(str)
    '        If dt.Rows.Count > 0 Then
    '            txtCellNo.Text = dt.Rows(0).Item("CellNo")
    '        End If
    ...
    End Sub
    Function GetAdjustedVoucherNo() As String
        Try
            If getConfigValueByType("VoucherNo").ToString = "Yearly" Then
                Return GetSerialNo("JV-" + Microsoft.VisualBasic.Right(dtpSalesDate.Value.Year, 2) + "-", "tblVoucher", "voucher_no")
            ElseIf getConfigValueByType("VoucherNo").ToString = "Monthly" Then
                Return GetNextDocNo("JV-" & Format(dtpSalesDate.Value, "yy") & dtpSalesDate.Value.Month.ToString("00"), 4, "tblVoucher", "voucher_no")
            Else
