...
Public Class frmReceivingNote
    ...
    Private Sub FillCombo(ByVal strCondition As String)
        ...
        ElseIf strCondition = "TransportationVendor" Then
            ...
            str = "Select ISNULL([AccountId],0) as AccountId, [TransporterName],[TransporterId] From tblDefTransporter"
            FillUltraDropDown(Me.cmbTransportationVendor, str)
            If Me.cmbTransportationVendor.DisplayLayout.Bands.Count > 0 Then
        ...
        ElseIf strCondition = "SO" Then
            ...
            str = "Select PurchaseOrderID, PurchaseOrderNo + ' ~ ' + Convert(Varchar(12), PurchaseOrderDate, 113) as PurchaseOrderNo, DcNo,Remarks, IsNull(CostCenterId, 0) As CostCenterId, IsNull(LCId, 0) As LCId from PurchaseOrderMasterTable where VendorID=" & Me.cmbVendor.Value & " AND  IsNull(Post,0)=1 AND Status='Open' Order By PurchaseOrderID DESC "
        ...
        ElseIf strCondition = "SOSpecific" Then
            ...
            'TASK16 Filter Posted Purchase Order
            str = "Select PurchaseOrderID, PurchaseOrderNo, DcNo,Remarks, IsNull(CostCenterId, 0) As CostCenterId, IsNull(LCId, 0) As LCId from PurchaseOrderMasterTable where IsNull(Post,0)=1 AND PurchaseorderId not in(select PurchaseOrderId from ReceivingNoteMasterTable) and vendorid=" & Me.cmbVendor.Value & ""
            FillDropDown(cmbPo, str)

        ElseIf strCondition = "SOComplete" Then
            'str = "Select PurchaseOrderID, PurchaseOrderNo, DcNo,Remarks  from PurchaseOrderMasterTable "
            'TASK16 Filter Posted Purchase Order
            str = "Select PurchaseOrderID, PurchaseOrderNo, DcNo,Remarks, IsNull(CostCenterId, 0) As CostCenterId, IsNull(LCId, 0) As LCId from PurchaseOrderMasterTable WHERE IsNull(Post,0)=1 "
            FillDropDown(cmbPo, str)
        ...
        ElseIf strCondition = "LCPO" Then
            str = "Select PurchaseOrderID, PurchaseOrderNo + ' ~ ' + Convert(Varchar(12), PurchaseOrderDate, 113) as PurchaseOrderNo, DcNo,Remarks, IsNull(CostCenterId, 0) As CostCenterId from PurchaseOrderMasterTable where LCId=" & Me.cmbLC.Value & " AND  IsNull(Post,0)=1 AND Status='Open' Order By PurchaseOrderID DESC "
            Me.cmbPo.DataSource = Nothing
            FillDropDown(cmbPo, str)
    ...
    Private Function Save() As Boolean
        ...
        Try
            ...
            objCommand.CommandText = "Insert into ReceivingNoteMasterTable (locationId,ReceivingNo,ReceivingDate,VendorId,PurchaseOrderId, ReceivingQty,ReceivingAmount, CashPaid, Remarks,UserName, DcNo, Post, Vehicle_No, Driver_Name, dcDate, Vendor_Invoice_No, CostCenterId, IGPNo, CurrencyType, CurrencyRate, Transportation_Vendor, Total_Discount_Amount,Arrival_Time,Departure_Time, LCId , Status) values( " _
                                & gobjLocationId & ", N'" & txtPONo.Text & "',N'" & dtpPODate.Value.ToString("yyyy-M-d h:mm:ss tt") & "'," & cmbVendor.ActiveRow.Cells(0).Value & "," & Me.cmbPo.SelectedValue & ", " & Val(Me.grd.GetTotal(Me.grd.RootTable.Columns("TotalQty"), Janus.Windows.GridEX.AggregateFunction.Sum)) & "," & Val(Me.grd.GetTotal(Me.grd.RootTable.Columns("Total"), Janus.Windows.GridEX.AggregateFunction.Sum)) & ", " & Val(txtPaid.Text) & ",N'" & txtRemarks.Text.Replace("'", "''") & "',N'" & LoginUserName & "', N'" & Me.txtDcNo.Text.Replace("'", "''") & "', " & IIf(Me.chkPost.Checked = True, 1, 0) & ", N'" & Me.txtVhNo.Text.Replace("'", "''") & "', N'" & Me.txtDriverName.Text.Replace("'", "''") & "', " & IIf(Me.dtpDcDate.Checked = True, "N'" & Me.dtpDcDate.Value.ToString("yyyy-M-d h:mm:ss tt") & "'", "NULL") & ", N'" & Me.txtInvoiceNo.Text.Replace("'", "''") & "', " & IIf(Me.cmbProject.SelectedIndex = -1, 0, Me.cmbProject.SelectedValue) & ", N'" & Me.txtIGPNo.Text.Replace("'", "''") & "', 0,0, " & IIf(Me.cmbTransportationVendor.ActiveRow Is Nothing, 0, Me.cmbTransportationVendor.Value) & ", 0,Convert(DateTime,'" & Me.dtpArrivalTime.Value.ToString("yyyy-M-d hh:mm:ss tt") & "',102)," & IIf(Me.dtpDepartureTime.Checked = True, "Convert(Datetime,'" & Me.dtpDepartureTime.Value.ToString("yyyy-M-d hh:mm:ss tt") & "',102)", "NULL") & ", " & IIf(Me.cmbLC.ActiveRow Is Nothing, 0, Me.cmbLC.Value) & ", N'" & EnumStatus.Open.ToString & "') Select @@Identity"
            ...
            For i = 0 To grd.RowCount - 1
                ...
                '                    PurchasePriceNew = Val(grd.GetRows(i).Cells("rate").Value.ToString) + Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString) - Val(Me.grd.GetRows(i).Cells("Discount_Price").Value.ToString)
                '                Else
                '                    PurchasePriceNew = Val(grd.GetRows(i).Cells("rate").Value.ToString) + Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString) - Val(Me.grd.GetRows(i).Cells("Discount_Price").Value.ToString)
                '                End If
                '            Else
                ...
                StockDetail.CostPrice = dblCostPrice
                ...
                '    objCommand.CommandText = ""
                '    objCommand.CommandText = "UPDATE ArticleDefTableMaster Set PurchasePrice=" & (PurchasePriceNew - Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString) + Val(Me.grd.GetRows(i).Cells("Discount_Price").Value.ToString)) & " WHERE ArticleId in (Select MasterId From ArticleDefTable WHERE ArticleId=" & grd.GetRows(i).Cells("ItemId").Value & ")"
                '    objCommand.ExecuteNonQuery()


                '    objCommand.CommandText = ""
                '    objCommand.CommandText = "UPDATE ArticleDefTable Set PurchasePrice=" & (PurchasePriceNew - Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString) + Val(Me.grd.GetRows(i).Cells("Discount_Price").Value.ToString)) & " WHERE ArticleId=" & grd.GetRows(i).Cells("ItemId").Value & ""
                '    objCommand.ExecuteNonQuery()

                '    objCommand.CommandText = ""
                '    objCommand.CommandText = " Select a.ArticleDefId, b.SalesItem From IncrementReductionTable a INNER JOIN ArticleDefView b ON b.ArticleId = a.ArticleDefId WHERE (Convert(varchar, a.IncrementReductionDate, 102) = Convert(Datetime, N'" & Me.dtpPODate.Value.ToString("yyyy-M-d 00:00:00") & "', 102))  AND a.ArticleDefId=" & grd.GetRows(i).Cells("ItemId").Value & ""
                '    Dim dtRate As New DataTable
                '    Dim daRate As New OleDbDataAdapter(objCommand)
                '    daRate.Fill(dtRate)

                '    objCommand.CommandText = "Select ISNULL(SalesItem,0) as SalesItem From ArticleDefView WHERE Active=1 AND ArticleId=" & Val(grd.GetRows(i).Cells("ItemId").Value) & " "
                ...
                '                objCommand.CommandText = "Update IncrementReductionTable Set PurchaseNewPrice=" & (PurchasePriceNew - Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString) + Val(Me.grd.GetRows(i).Cells("Discount_Price").Value.ToString)) & ", SaleNewPrice=" & IIf(dtSalesItem.Rows(0).Item(0) = True, 0, (PurchasePriceNew - Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString))) & "  WHERE ArticleDefId=" & Val(grd.GetRows(i).Cells("ItemId").Value) & " AND (Convert(varchar, IncrementReductionDate, 102) = Convert(Datetime, N'" & Me.dtpPODate.Value.ToString("yyyy-M-d 00:00:00") & "', 102)) "
                '                objCommand.ExecuteNonQuery()
                '            Else
                ...
                '                & " Values(N'" & Me.dtpPODate.Value.ToString("yyyy-M-d h:mm:ss tt") & "', " & grd.GetRows(i).Cells("ItemId").Value & ",  " & Val(0) & ", " & Val(0) & ", " & (PurchasePriceNew - Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString) + Val(Me.grd.GetRows(i).Cells("Discount_Price").Value.ToString)) & ", " & Val(0) & ", " & IIf(dtSalesItem.Rows(0).Item(0) = True, 0, (PurchasePriceNew - Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString) + Val(Me.grd.GetRows(i).Cells("Discount_Price").Value.ToString))) & ")"
                '                objCommand.ExecuteNonQuery()
                '            End If
                '        End If
                '    End If
                'Else
                '    'Apply Current Rate 
                '    objCommand.CommandText = "UPDATE ArticleDefTable Set PurchasePrice=" & Val(grd.GetRows(i).Cells("Rate").Value.ToString) & " WHERE ArticleId=" & grd.GetRows(i).Cells("ItemId").Value & ""
                '    objCommand.ExecuteNonQuery()
                ...
                '        If Not dtRate Is Nothing Then
                '            If dtRate.Rows.Count > 0 Then
                '                objCommand.CommandText = "Update IncrementReductionTable Set PurchaseNewPrice=" & Val(grd.GetRows(i).Cells("Rate").Value.ToString) & ", SaleNewPrice=" & IIf(dtSalesItem.Rows(0).Item(0) = True, 0, Val(grd.GetRows(i).Cells("Rate").Value.ToString)) & "  WHERE ArticleDefId=" & Val(grd.GetRows(i).Cells("ItemId").Value) & " AND (Convert(varchar, IncrementReductionDate, 102) = Convert(Datetime, N'" & Me.dtpPODate.Value.ToString("yyyy-M-d 00:00:00") & "', 102)) "
                '                objCommand.ExecuteNonQuery()
                '            Else
                '                objCommand.CommandText = "INSERT INTO IncrementReductionTable(IncrementReductionDate, ArticleDefId, StockQty, PurchaseOldPrice, PurchaseNewPrice, SaleOldPrice,SaleNewPrice) " _
                '                & " Values(N'" & Me.dtpPODate.Value.ToString("yyyy-M-d h:mm:ss tt") & "', " & grd.GetRows(i).Cells("ItemId").Value & ",  " & Val(0) & ", " & Val(0) & ", " & Val(grd.GetRows(i).Cells("Rate").Value.ToString) & ", " & Val(0) & ", " & IIf(dtSalesItem.Rows(0).Item(0) = True, 0, Val(grd.GetRows(i).Cells("Rate").Value.ToString)) & ")"
                '                objCommand.ExecuteNonQuery()
                '            End If
                '        End If
                '    End If
    ...
    Private Function Update_Record() As Boolean
        ...
        Try
            ...
            objCommand.CommandText = ""
            objCommand.CommandText = "Update ReceivingNoteMasterTable set LocationId=" & gobjLocationId & ", ReceivingNo =N'" & txtPONo.Text & "',ReceivingDate=N'" & dtpPODate.Value.ToString("yyyy-M-d h:mm:ss tt") & "',VendorId=" & cmbVendor.ActiveRow.Cells(0).Value & ", PurchaseOrderId=" & IIf(Me.cmbPo.SelectedIndex > 0, Me.cmbPo.SelectedValue, 0) & ", " _
         & " ReceivingQty=" & Val(Me.grd.GetTotal(Me.grd.RootTable.Columns("Qty"), Janus.Windows.GridEX.AggregateFunction.Sum)) & ",ReceivingAmount=" & Val(Me.grd.GetTotal(Me.grd.RootTable.Columns("Total"), Janus.Windows.GridEX.AggregateFunction.Sum)) & ", CashPaid=" & Val(txtPaid.Text) & ", Remarks=N'" & txtRemarks.Text.Replace("'", "''") & "',UpdateUserName=N'" & LoginUserName & "', DcNo=N'" & Me.txtDcNo.Text.Replace("'", "''") & "', Post=" & IIf(Me.chkPost.Checked = True, 1, 0) & ", Vehicle_No=N'" & Me.txtVhNo.Text.Replace("'", "''") & "', Driver_Name=N'" & Me.txtDriverName.Text.Replace("'", "''") & "', DcDate=" & IIf(Me.dtpDcDate.Checked = True, "N'" & dtpDcDate.Value.ToString("yyyy-M-d h:mm:ss tt") & "'", "NULL") & ", Vendor_Invoice_No=N'" & Me.txtInvoiceNo.Text.Replace("'", "''") & "', CostCenterId=" & Me.cmbProject.SelectedValue & ", IGPNo=N'" & Me.txtIGPNo.Text.Replace("'", "''") & "', Transportation_Vendor=" & IIf(Me.cmbTransportationVendor.ActiveRow Is Nothing, 0, Me.cmbTransportationVendor.Value) & ",Arrival_Time=Convert(DateTime,'" & Me.dtpArrivalTime.Value.ToString("yyyy-M-d hh:mm:ss tt") & "',102), Departure_Time=" & IIf(Me.dtpDepartureTime.Checked = True, "Convert(Datetime,'" & Me.dtpDepartureTime.Value.ToString("yyyy-M-d hh:mm:ss tt") & "',102)", "NULL") & ", LCId = " & IIf(Me.cmbLC.ActiveRow Is Nothing, 0, Me.cmbLC.Value) & "   Where ReceivingNoteID= " & txtReceivingID.Text & " "
            objCommand.ExecuteNonQuery()
            'Marked Against Task#2015060001 Ali Ansari
            'Altered Against Task#2015060001 Ali Ansari
            ...
            For i = 0 To grd.RowCount - 1
                ...
                '    objCommand.CommandText = "Select ISNULL(SalesItem,0) as SalesItem From ArticleDefView WHERE Active=1 AND ArticleId=" & Val(grd.GetRows(i).Cells("ItemId").Value.ToString) & " "
                '    Dim dtSalesItem As New DataTable
                '    Dim daSalesItem As New OleDbDataAdapter(objCommand)
                '    daSalesItem.Fill(dtSalesItem)
                ...
                '        If Not dtRate Is Nothing Then
                '            If dtRate.Rows.Count > 0 Then
                '                objCommand.CommandText = "Update IncrementReductionTable Set PurchaseNewPrice=" & (PurchasePriceNew - Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString) + Val(Me.grd.GetRows(i).Cells("Discount_Price").Value.ToString)) & ", SaleNewPrice=" & IIf(dtSalesItem.Rows(0).Item(0) = True, 0, (PurchasePriceNew - Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString) + Val(Me.grd.GetRows(i).Cells("Discount_Price").Value.ToString))) & "  WHERE ArticleDefId=" & Val(grd.GetRows(i).Cells("ItemId").Value.ToString) & " AND (Convert(varchar, IncrementReductionDate, 102) = Convert(Datetime, N'" & Me.dtpPODate.Value.ToString("yyyy-M-d 00:00:00") & "', 102)) "
                '                objCommand.ExecuteNonQuery()
                '            Else
                '                objCommand.CommandText = "INSERT INTO IncrementReductionTable(IncrementReductionDate, ArticleDefId, StockQty, PurchaseOldPrice, PurchaseNewPrice, SaleOldPrice,SaleNewPrice) " _
                '                & " Values(N'" & Me.dtpPODate.Value.ToString("yyyy-M-d h:mm:ss tt") & "', " & grd.GetRows(i).Cells("ItemId").Value & ",  " & Val(0) & ", " & Val(0) & ", " & (PurchasePriceNew - Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString) + Val(Me.grd.GetRows(i).Cells("Discount_Price").Value.ToString)) & ", " & Val(0) & ", " & IIf(dtSalesItem.Rows(0).Item(0) = True, 0, (PurchasePriceNew - Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString) + Val(Me.grd.GetRows(i).Cells("Discount_Price").Value.ToString))) & ")"
                ...
                '        If Not dtRate Is Nothing Then
                '            If dtRate.Rows.Count > 0 Then
                '                objCommand.CommandText = "Update IncrementReductionTable Set PurchaseNewPrice=" & Val(grd.GetRows(i).Cells("Rate").Value.ToString) & ", SaleNewPrice=" & IIf(dtSalesItem.Rows(0).Item(0) = True, 0, Val(grd.GetRows(i).Cells("Rate").Value.ToString)) & "  WHERE ArticleDefId=" & Val(grd.GetRows(i).Cells("ItemId").Value.ToString) & " AND (Convert(varchar, IncrementReductionDate, 102) = Convert(Datetime, N'" & Me.dtpPODate.Value.ToString("yyyy-M-d 00:00:00") & "', 102)) "
                '                objCommand.ExecuteNonQuery()
                '            Else
                '                objCommand.CommandText = "INSERT INTO IncrementReductionTable(IncrementReductionDate, ArticleDefId, StockQty, PurchaseOldPrice, PurchaseNewPrice, SaleOldPrice,SaleNewPrice) " _
                '                & " Values(N'" & Me.dtpPODate.Value.ToString("yyyy-M-d h:mm:ss tt") & "', " & Val(grd.GetRows(i).Cells("ItemId").Value.ToString) & ",  " & Val(0) & ", " & Val(0) & ", " & Val(grd.GetRows(i).Cells("Rate").Value.ToString) & ", " & Val(0) & ", " & IIf(dtSalesItem.Rows(0).Item(0) = True, 0, Val(grd.GetRows(i).Cells("Rate").Value.ToString)) & ")"
                '                objCommand.ExecuteNonQuery()
                '            End If
                '        End If
                '    End If
    ...
    Private Sub DeleteToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDelete.Click
        ...
        Try
            ...
            '                    & " Values(N'" & Me.dtpPODate.Value.ToString("yyyy-M-d h:mm:ss tt") & "', " & Val(dtSavedItems.Rows(i).Item("ArticleDefId").ToString) & ",  " & Val(0) & ", " & Val(0) & ", " & PurchasePriceNew & ", " & Val(0) & ", " & IIf(dtSalesItem.Rows(0).Item(0) = True, 0, PurchasePriceNew) & ")"
            '                    cm.ExecuteNonQuery()
            '                End If
            '            End If
            '        End If
    ...
    Public Sub ApplyGridSettings(Optional ByVal Condition As String = "") Implements IGeneral.ApplyGridSettings
        Try
            ...
            For Each col As Janus.Windows.GridEX.GridEXColumn In Me.grd.RootTable.Columns
                If col.Index <> grdEnm.LocationId AndAlso col.Index <> grdEnm.ReceivedQty AndAlso col.Index <> grdEnm.Qty AndAlso col.Index <> grdEnm.VendorQty AndAlso col.Index <> grdEnm.Deduction AndAlso col.Index <> grdEnm.GrossQty AndAlso col.Index <> grdEnm.RejectedQty AndAlso col.Index <> grdEnm.Price AndAlso col.Index <> grdEnm.TaxPercent AndAlso col.Index <> grdEnm.ExpiryDate AndAlso col.Index <> grdEnm.BatchNo AndAlso col.Index <> grdEnm.Comments AndAlso col.Index <> grdEnm.Transportation_Charges AndAlso col.Index <> grdEnm.PackPrice AndAlso col.Index <> grdEnm.X_Tray_Weights AndAlso col.Index <> grdEnm.X_Gross_Weights AndAlso col.Index <> grdEnm.Y_Gross_Weights AndAlso col.Index <> grdEnm.Y_Tray_Weights AndAlso col.Index <> grdEnm.AdTax_Percent AndAlso col.Index <> grdEnm.CurrentPrice AndAlso col.Index <> grdEnm.RateDiscPercent AndAlso col.Index <> grdEnm.Column1 AndAlso col.Index <> grdEnm.Column2 AndAlso col.Index <> grdEnm.Column3 Then
        ...
        Try
            ...
            If GetSMSConfig("Receiving Note").Enable = True Then
                ...
                If obj IsNot Nothing Then
                    ...
                    strMessage = strMessage.Replace("@AccountTitle", Me.cmbVendor.ActiveRow.Cells("Name").Value.ToString).Replace("@AccountCode", Me.cmbVendor.ActiveRow.Cells("Code").Value.ToString).Replace("@DocumentNo", Me.txtPONo.Text).Replace("@DocumentDate", Me.dtpPODate.Value.ToShortDateString).Replace("@OtherDoc", Me.txtInvoiceNo.Text).Replace("@Remarks", Me.txtRemarks.Text).Replace("@Amount", Me.grd.GetTotal(grd.RootTable.Columns("Total"), Janus.Windows.GridEX.AggregateFunction.Sum)).Replace("@Quantity", Me.grd.GetTotal(grd.RootTable.Columns("Qty"), Janus.Windows.GridEX.AggregateFunction.Sum)).Replace("@CompanyName", CompanyTitle).Replace("@SIRIUS", "Automated by www.SIRIUS.net").Replace("@RejectedQuantity", Me.grd.GetTotal(Me.grd.RootTable.Columns("RejectedQty"), Janus.Windows.GridEX.AggregateFunction.Sum)).Replace("@DetailInformation", strDetailMessage)
