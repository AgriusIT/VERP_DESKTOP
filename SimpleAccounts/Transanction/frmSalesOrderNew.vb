...
Public Class frmSalesOrderNew
    ...
    Private Sub DisplayRecord(Optional ByVal strCondition As String = "")
        ...
        str = "SELECT " & IIf(strCondition.ToString = "All", "", "Top 50") & " Recv.SalesOrderNo, Recv.SalesOrderDate AS Date, dbo.vwCOADetail.detail_title AS CustomerName, Pack.Packs, Recv.SalesOrderQty, " & _
                            "Recv.SalesOrderAmount, Recv.SalesOrderId, Recv.VendorId, Recv.Remarks, CONVERT(varchar, Recv.CashPaid) AS CashPaid, ISNULL(Recv.LocationId,0) as LocationId, ISNULL(Recv.SpecialAdjustment,0) as SpecialAdjustment, Recv.PoNo as [Po No], isnull(Recv.SOP_ID,0) as SOP_ID, tblDefEmployee.Employee_Name, ISNULL(Recv.Posted,0) as Posted, Recv.Delivery_Date, Isnull(Recv.Adj_Flag,0) as Adj_Flag, Isnull(Recv.Adjustment,0) as Adjustment," & IIf(flgExcludeTaxPrice = True, "(Recv.SalesOrderAmount + NetInfo.SedTaxAmount  ) As NetAmount ", "(Recv.SalesOrderAmount + NetInfo.SedTaxAmount + NetInfo.TaxAmount ) As NetAmount ") & ",Isnull(Recv.CostCenterID,0) as CostCenterId, Recv.PO_Date,  ISNULL(Recv.EditionalTax_Percentage,0) as EditionalTax_Percentage, ISNULL(Recv.SED_Percentage,0) as SED_Percentage, CASE WHEN ISNULL(PrintLog.Cont,0)=0 THEN 'Print Pending' ELSE 'Printed' end as [Print Status], CstCntr.Name, dbo.vwCOADetail.Contact_Email as Email,IsNull(Recv.SaleOrderStatusId,0) as SaleOrderStatusId, SalesOrderStatusTable.OrderStatus as [Order Status], IsNull(Recv.QuotationId,0) as QuotationId, Quot.QuotationNo, IsNull(Doc_Att.[No Of Attachment],0) as  [No Of Attachment],  Recv.Terms_And_Condition,recv.UserName,Recv.UpdateUserName as [Modified By], IsNull(Recv.SaleOrderTypeId,0) as SaleOrderTypeId, SalesOrderTypeTable.SalesOrderTypeName as [Order Type], Recv.SpecialInstructions, IsNull(Recv.DispatchToLocation, 0) As DispatchToLocation, IsNull(Recv.InvoiceToLocation, 0) As InvoiceToLocation, " & _
                            ...
                            " FROM tblDefCostCenter CstCntr Right Outer Join dbo.SalesOrderMasterTable Recv ON CstCntr.CostCenterID=Recv.CostCenterId LEFT OUTER JOIN (Select SalesOrderId, Sum(IsNull(Sz1, 0)) As Packs From SalesOrderDetailTable Group By SalesOrderId) As Pack ON Recv.SalesOrderId = Pack.SalesOrderId  " & _
        ...
        End If
        str += " ORDER BY Recv.SalesOrderNo DESC"
        ' End If
        ...
        Try
            ...
            For i = 0 To grd.RowCount - 1
                ...
                objCommand.CommandText = "Insert into SalesOrderDetailTable (SalesOrderId, LocationId, ArticleDefId,ArticleSize, Sz1,Qty,Price,Sz7,CurrentPrice, TradePrice, SalesTax_Percentage, SchemeQty, Discount_Percentage, Freight, MarketReturns, PurchasePrice,PackPrice, Pack_Desc, Comments, Engine_No, Chassis_No,DeliveredQty,PlanedQty,Other_Comments,CostPrice, QuotationDetailId,SED_Tax_Percent,SED_Tax_Amount,ArticleAliasName,SaleOrderType, BaseCurrencyId, BaseCurrencyRate, CurrencyId, CurrencyRate, CurrencyAmount, SerialNo , DiscountId, DiscountValue, DiscountFactor , PostDiscountPrice , BatchNo , ExpiryDate) values( " _
                                    ...
                                    & " " & Val(grd.GetRows(i).Cells(GrdEnum.BaseCurrencyId).Value.ToString) & ", " & Val(grd.GetRows(i).Cells(GrdEnum.BaseCurrencyRate).Value.ToString) & ", " & Val(grd.GetRows(i).Cells(GrdEnum.CurrencyId).Value.ToString) & ", " & Val(grd.GetRows(i).Cells(GrdEnum.CurrencyRate).Value.ToString) & ", " & Val(grd.GetRows(i).Cells(GrdEnum.CurrencyAmount).Value.ToString) & ", N'" & grd.GetRows(i).Cells("SerialNo").Value.ToString.Replace("'", "''") & "'," & Val(grd.GetRows(i).Cells("DiscountId").Value.ToString) & "," & Val(Me.grd.GetRows(i).Cells("DiscountValue").Value.ToString) & "," & Val(Me.grd.GetRows(i).Cells("DiscountFactor").Value.ToString) & ", " & Val(Me.grd.GetRows(i).Cells("PostDiscountPrice").Value) & " , '" & Val(Me.grd.GetRows(i).Cells(GrdEnum.BatchNo).Value.ToString) & "' , '" & CType(grd.GetRows(i).Cells(GrdEnum.ExpiryDate).Value, Date).ToString("yyyy-M-d h:mm:ss tt") & "' )"
    ...
    Private Sub ApplyGridSetting(Optional ByVal Condition As String = "")
        Try
            ...
            Me.grd.RootTable.Columns(GrdEnum.PostDiscountPrice).FormatString = "N" & DecimalPointInValue
            Me.grd.RootTable.Columns(GrdEnum.PostDiscountPrice).FormatString = "N" & TotalAmountRounding
            Me.grd.RootTable.Columns(GrdEnum.PostDiscountPrice).TotalFormatString = "N" & TotalAmountRounding

            Me.grd.RootTable.Columns(GrdEnum.DiscountValue).FormatString = "N" & DecimalPointInValue
            Me.grd.RootTable.Columns(GrdEnum.DiscountValue).FormatString = "N" & TotalAmountRounding
            Me.grd.RootTable.Columns(GrdEnum.DiscountValue).TotalFormatString = "N" & TotalAmountRounding
            ...
            Me.grd.RootTable.Columns(GrdEnum.CurrencyAmount).FormatString = "N" & DecimalPointInValue
            Me.grd.RootTable.Columns(GrdEnum.CurrencyAmount).FormatString = "N" & TotalAmountRounding
            Me.grd.RootTable.Columns(GrdEnum.CurrencyAmount).TotalFormatString = "N" & TotalAmountRounding
    ...
    Function GetDocumentNo() As String
        Try
            ...
            ElseIf getConfigValueByType("VoucherNo").ToString = "Monthly" Then
                Return GetNextDocNo("SO" & Me.cmbCompany.SelectedValue & "-" & Format(Me.dtpPODate.Value, "yy") & Me.dtpPODate.Value.Month.ToString("00"), 4, "SalesOrderMasterTable", "SalesOrderNo")
            Else
