...
Public Class frmPaymentNew
    ...
    Function GetVoucherNo() As String
        'rafay declare the variable date1 for PAK and date2 for UAE
    ...
    Public Sub btnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAdd.Click
        Try
            If Me.ValidateInput Then
                ...
                '' ''Rafay Task Start
                ' ''drGrd.Item(grdEnm.TaxAmount) = Format(Math.Round(Val(Me.txtTaxAmount.Text), TotalAmountRounding), "0.00")                'added by zainab task1608
                ' ''drGrd.Item(grdEnm.SalesTaxPer) = Format(Val(Me.txtSalesTax.Text), "0.00")
                ' ''drGrd.Item(grdEnm.SalesTaxAmount) = Format(Math.Round(Val(Me.txtSalesTaxAmount.Text), TotalAmountRounding), "0.00")
                ' ''drGrd.Item(grdEnm.WHTaxPer) = Format(Val(Me.txtWithHolding.Text), "0.00")
                ' ''drGrd.Item(grdEnm.WHTaxAmount) = Format(Val(Me.txtWithHoldingTaxAmount.Text), "0.00")
    ...
    Private Sub DisplayDetail(ByVal VoucherID As Integer)
        ...
        Try
            ...
            ' ''dtDisplayDetail.Columns("Sales_Tax").Expression = "((Amount-Discount)/(SalesTaxPer))"
...
#End Region
    ...
    Private Sub txtSalesTaxAmount_MouseHover(sender As Object, e As EventArgs) Handles txtSalesTaxAmount.MouseHover
    ...
    Private Sub grd_DoubleClick(sender As Object, e As EventArgs) Handles grd.DoubleClick
        Try
            ...
            'Rafay:Task Start: change format t0 2 decimal places
            Me.txtTax.Text = Format(Me.grd.GetRow.Cells(grdEnm.Tax).Value, "0.00")
            Me.txtTaxAmount.Text = Format(Me.grd.GetRow.Cells(grdEnm.TaxAmount).Value, "0.00")
            Me.cmbSalesTax.Text = Format(Me.grd.GetRow.Cells(grdEnm.AccountId).Text.ToString, "0.00")
            'Me.txtDebit.Text = Val(Me.grd.GetRow.Cells("Debit").Value)
            'Me.txtCredit.Text = Val(Me.grd.GetRow.Cells("Credit").Value)
            Me.txtSalesTax.Text = Format(Val(Me.grd.GetRow.Cells(grdEnm.SalesTaxPer).Value), "0.00")
            Me.txtSalesTaxAmount.Text = Format(Val(Me.grd.GetRow.Cells(grdEnm.SalesTaxAmount).Value), "0.00")
            Me.txtWithHolding.Text = Format(Val(Me.grd.GetRow.Cells(grdEnm.WHTaxPer).Value), "0.00")
