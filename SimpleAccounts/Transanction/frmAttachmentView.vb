'Task# 2015060005 Ali Ansari To Open File On Click Ali Ansari
'Altered Against Task#2015060005 to designer in grid to convert coulumn for Click
...
Public Class frmAttachmentView
    ...
    Public RemoveAttachmentForSalesOrder As Boolean = True
    ...
    Private Sub grdSaved_ColumnButtonClick(ByVal sender As System.Object, ByVal e As Janus.Windows.GridEX.ColumnActionEventArgs) Handles grdSaved.ColumnButtonClick
        ...
        Try
            ...
            If e.Column.Key = "Column1" Then
                If IO.File.Exists(Me.grdSaved.GetRow.Cells("Path").Value.ToString & "\" & Me.grdSaved.GetRow.Cells("FileName").Value.ToString) Then
                    IO.File.Delete(Me.grdSaved.GetRow.Cells("Path").Value.ToString & "\" & Me.grdSaved.GetRow.Cells("FileName").Value.ToString)
                End If
    ...
    Private Sub frmAttachmentView_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        Try
            ...
            Me.grdSaved.DataSource = dt
            Me.grdSaved.RootTable.Columns("Column1").Visible = True
            Me.grdSaved.RootTable.Columns("Column1").EditType = Janus.Windows.GridEX.EditType.TextBox
            If Me._Source = "frmSalesOrderNew" Then
                If Me.RemoveAttachmentForSalesOrder = False Then
                    Me.grdSaved.RootTable.Columns("Column1").Visible = False
                    Me.grdSaved.RootTable.Columns("Column1").EditType = Janus.Windows.GridEX.EditType.NoEdit
                End If
            End If
        Catch ex As Exception
            ShowErrorMessage(ex.Message)
