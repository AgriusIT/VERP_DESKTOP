...
Public Class frmReturnToFactory
    Implements IGeneral

    Public ID As Integer = 0
    Dim ObjDAL As ReturnToFactoryMasterDAL
    Dim ObjDetailDAL As ReturnToFactoryDetailDAL
    Dim ObjModel As ReturnToFactoryMasterBE
    ...
    Public Sub New(ByVal DoHaveSaveRights As Boolean, ByVal DoHavePrintRight As Boolean, ByVal DoHaveExportRights As Boolean, ByVal DoHaveFieldChooserRights As Boolean)
        Try
            ...
            InitializeComponent()
            IsFormOpened = True
            ObjDetailDAL = New ReturnToFactoryDetailDAL()
        ...
        End Try
    End Sub
    Public Sub New(ByVal Obj As ReturnToFactoryMasterBE, ByVal UpdateRights As Boolean, ByVal PostRights As Boolean, ByVal PrintRights As Boolean, ByVal DoHaveExportRights As Boolean, ByVal DoHaveFieldChooserRights As Boolean)
        Try
            ' This call is required by the designer.
            InitializeComponent()
            IsFormOpened = True
    ...
    Public Sub FillCombos(Optional Condition As String = "") Implements IGeneral.FillCombos
        ...
        If Condition = "Article" Then
            ...
            Str = "SELECT ArticleDefView.ArticleId as Id, ArticleCode as Code, ArticleDescription as Item, ArticleSizeName as Size, ArticleColorName as Combination, ArticleDefView.ArticleBrandName As Grade, PurchasePrice AS Price, Isnull(SalePrice,0) as SalePrice, ArticleDefView.SizeRangeID as [Size ID], Isnull(ArticleDefView.ArticleTaxId,0) as [Tax ID], Isnull(PurchasePrice,0) as PurchasePrice, Isnull(SubSubId,0) as AccountId, Isnull(ServiceItem,0) as ServiceItem, ArticleDefView.SortOrder, ArticleGroupName as [Dept], ArticleTypeName as [Type], ArticleGenderName as [Origin],ArticleLPOName as [Brand],Isnull(LogicalItem,0) as LogicalItem, SalesAccountId, CGSAccountId, MasterID , IsNull(Cost_Price,0) as Cost_Price, IsNull(TradePrice,0) as [Trade Price], IsNull(PrintedRetailPrice,0) as [Retail Price], Isnull(StockDetail.Stock,0) as Stock, IsNull(dbo.ArticleDefView.ApplyAdjustmentFuelExp,1) as  ApplyAdjustmentFuelExp FROM ArticleDefView LEFT OUTER JOIN  (Select ArticleDefId, Sum(IsNull(InQty, 0)-IsNull(OutQty, 0)) As Stock From StockDetailTable " & IIf(Me.cmbLocation.ActiveRow IsNot Nothing, " WHERE LocationId=" & Me.cmbLocation.Value & "", "") & "  Group By ArticleDefId) As StockDetail ON ArticleDefView.ArticleId = StockDetail.ArticleDefId Where ArticleDefView.Active=1  "
    ...
    Public Sub FillModel(Optional Condition As String = "") Implements IGeneral.FillModel
        Try
            ObjModel = New ReturnToFactoryMasterBE()
            ObjModel.ID = ID
            ...
            For Each _Row As Janus.Windows.GridEX.GridEXRow In Me.grdReturnToFactory.GetRows
                Dim _Detail As New ReturnToFactoryDetailBE
                _Detail.ID = Val(_Row.Cells(Detail.ID).Value.ToString)
                _Detail.ReturnToFactoryId = Val(_Row.Cells(Detail.ReturnToFactoryId).Value.ToString)
                ...
                StockDetail.ArticleDefId = Val(_Row.Cells(Detail.ArticleId).Value.ToString)
                StockDetail.InQty = 0
                StockDetail.OutQty = Val(_Row.Cells(Detail.Qty).Value.ToString)
                StockDetail.Rate = Val(_Row.Cells(Detail.Rate).Value.ToString)
                StockDetail.InAmount = 0
                StockDetail.OutAmount = Val(_Row.Cells(Detail.Qty).Value.ToString) * Val(_Row.Cells(Detail.Rate).Value.ToString)
    ...
    Public Function IsValidate(Optional Mode As SBUtility.Utility.EnumDataMode = SBUtility.Utility.EnumDataMode.Disabled, Optional Condition As String = "") As Boolean Implements IGeneral.IsValidate
        Try
            ...
            If Me.cmbParty.Value < 1 Then
                ...
                Return False
            End If
            If Me.grdReturnToFactory.RowCount = 0 Then
                ShowErrorMessage("Grid is empty.")
                Me.grdReturnToFactory.Focus()
                Return False
            End If
            FillModel()
            Return True
    ...
    Private Sub AddToGrid()
        ...
        'Public Shared Comments As String = "Comments"
        Try
            Dim dtGrid As DataTable = CType(Me.grdReturnToFactory.DataSource, DataTable)
            Dim NewRow As DataRow
            NewRow = dtGrid.NewRow
            NewRow(Detail.ID) = 0
            NewRow(Detail.ReturnToFactoryId) = ID
    ...
    Private Sub DisplayDetail(ByVal ID As Integer)
        Try
            Me.grdReturnToFactory.DataSource = ObjDetailDAL.DisplayDetail(ID)
            Me.grdReturnToFactory.RootTable.Columns(Detail.Sz1).FormatString = "N" & DecimalPointInQty
            Me.grdReturnToFactory.RootTable.Columns(Detail.Sz7).FormatString = "N" & DecimalPointInQty
            Me.grdReturnToFactory.RootTable.Columns(Detail.Qty).FormatString = "N" & DecimalPointInQty
            Me.grdReturnToFactory.RootTable.Columns(Detail.Rate).FormatString = "N" & DecimalPointInValue
            Me.grdReturnToFactory.RootTable.Columns(Detail.Sz1).TotalFormatString = "N" & DecimalPointInQty
            Me.grdReturnToFactory.RootTable.Columns(Detail.Sz7).TotalFormatString = "N" & DecimalPointInQty
            Me.grdReturnToFactory.RootTable.Columns(Detail.Qty).TotalFormatString = "N" & DecimalPointInQty
            'Me.grdReturnToFactory.RootTable.Columns(Detail.Rate).TotalFormatString = "N" & DecimalPointInValue
            Me.grdReturnToFactory.RootTable.Columns(Detail.Sz1).HeaderAlignment = Janus.Windows.GridEX.TextAlignment.Far
            Me.grdReturnToFactory.RootTable.Columns(Detail.Sz7).HeaderAlignment = Janus.Windows.GridEX.TextAlignment.Far
            Me.grdReturnToFactory.RootTable.Columns(Detail.Qty).HeaderAlignment = Janus.Windows.GridEX.TextAlignment.Far
            Me.grdReturnToFactory.RootTable.Columns(Detail.Rate).HeaderAlignment = Janus.Windows.GridEX.TextAlignment.Far
            Me.grdReturnToFactory.RootTable.Columns(Detail.Sz1).TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
            Me.grdReturnToFactory.RootTable.Columns(Detail.Sz7).TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
            Me.grdReturnToFactory.RootTable.Columns(Detail.Qty).TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
            Me.grdReturnToFactory.RootTable.Columns(Detail.Rate).TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
            Me.grdReturnToFactory.RootTable.Columns(Detail.Qty).EditType = Janus.Windows.GridEX.EditType.NoEdit
            '' TASK TFS
            ...
            Me.grdReturnToFactory.RootTable.Columns("LocationId").ValueList.PopulateValueList(dtLocation.DefaultView, "LocationId", "LocationName")
            '' 
        Catch ex As Exception
            Throw ex
        End Try
    ...
    Function GetDocumentNo() As String
        Try
            ...
            If getConfigValueByType("VoucherNo").ToString = "Yearly" Then
                Return GetSerialNo("RTF" + "-" + Microsoft.VisualBasic.Right(Me.dtpReturnDate.Value.Year, 2) + "-", "ReturnToFactoryMaster", "ReturnNo")
            ElseIf getConfigValueByType("VoucherNo").ToString = "Monthly" Then
                Return GetNextDocNo("RTF" & "-" & Format(Me.dtpReturnDate.Value, "yy") & Me.dtpReturnDate.Value.Month.ToString("00"), 4, "ReturnToFactoryMaster", "ReturnNo")
            Else
                Return GetNextDocNo("RTF", 6, "ReturnToFactoryMaster", "ReturnNo")
            End If
        Catch ex As Exception
            Throw ex
        End Try
    End Function
    Private Sub EditRecord(ByVal Obj As ReturnToFactoryMasterBE)
        Try
            ID = Obj.ID
    ...
    Private Sub grdReturnToFactory_CellUpdated(sender As Object, e As Janus.Windows.GridEX.ColumnActionEventArgs) Handles grdReturnToFactory.CellUpdated
        Try
            If grdReturnToFactory.GetRow.RowType = Janus.Windows.GridEX.RowType.Record Then
                If e.Column.Key = Detail.Sz1 Then
                    If Val(Me.grdReturnToFactory.GetRow.Cells(Detail.Sz1).Value.ToString) > 0 Then
                        Me.grdReturnToFactory.GetRow.Cells(Detail.Qty).Value = Me.grdReturnToFactory.GetRow.Cells(Detail.Sz1).Value * Me.grdReturnToFactory.GetRow.Cells(Detail.Sz7).Value
                    End If
                ...
                ElseIf e.Column.Key = Detail.Qty Then
                    If Val(Me.grdReturnToFactory.GetRow.Cells(Detail.Sz7).Value.ToString) = 1 Then
                        Me.grdReturnToFactory.GetRow.Cells(Detail.Sz1).Value = Me.grdReturnToFactory.GetRow.Cells(Detail.Qty).Value
                    End If
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub grdReturnToFactory_ColumnButtonClick(sender As Object, e As Janus.Windows.GridEX.ColumnActionEventArgs) Handles grdReturnToFactory.ColumnButtonClick
        Try
            If Me.grdReturnToFactory.GetRow.RowType = Janus.Windows.GridEX.RowType.Record Then
                If e.Column.Key = "Delete" Then
                    If IsPosted = False Then
                        If msg_Confirm(str_ConfirmDelete) = False Then Exit Sub
                        ObjDetailDAL.Delete(Val(Me.grdReturnToFactory.GetRow.Cells("ID").Value.ToString))
                        Me.grdReturnToFactory.GetRow.Delete()
                    Else
    ...
    Private Sub grdReturnToFactory_CellValueChanged(sender As Object, e As Janus.Windows.GridEX.ColumnActionEventArgs) Handles grdReturnToFactory.CellValueChanged
        ''TASK TFS4852 
        If grdReturnToFactory.GetRow.RowType = Janus.Windows.GridEX.RowType.Record Then
            If e.Column.Key = "Sz7" Then
                If grdReturnToFactory.GetRow.Cells("Unit").Value.ToString = "Loose" Then
                    grdReturnToFactory.CancelCurrentEdit()
                End If
            End If
        End If
        ''END TASK TFS4852
    ...
    Private Sub CtrlGrdBar1_Load(sender As Object, e As EventArgs) Handles CtrlGrdBar1.Load
        Try
            If IO.File.Exists(Application.ExecutablePath & "\..\Layouts\" & Me.Name & "_" & Me.grdReturnToFactory.Name) Then
                Dim fs As New IO.FileStream(Application.ExecutablePath & "\..\Layouts\" & Me.Name & "_" & Me.grdReturnToFactory.Name, IO.FileMode.Open, IO.FileAccess.ReadWrite, IO.FileShare.ReadWrite)
                'Me.grd.SaveLayoutFile(fs)
                Me.grdReturnToFactory.LoadLayoutFile(fs)
                fs.Close()
                fs.Dispose()
            End If
            'CtrlGrdBar.strForm_Name = CtrlGrdBar.EnmAccountType.Customers
            Me.CtrlGrdBar1.txtGridTitle.Text = CompanyTitle & Chr(10) & "Return To Factory"
        Catch ex As Exception
            ShowErrorMessage(ex.Message)
        End Try
    End Sub
