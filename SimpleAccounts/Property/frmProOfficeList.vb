...
Public Class frmProOfficeList
    Implements IGeneral
    Dim Office As OfficeBE
    Dim objDAL As OfficeDAL
    ...
    Public Sub ApplyGridSettings(Optional Condition As String = "") Implements IGeneral.ApplyGridSettings
        Try
            'OfficeId, Name, LandlinePhone, CellPhone, Email, FaxNumber, AddressLine1, AddressLine2, AreaId, Remarks, EstateId, CityId from Office Order by 1 Desc
            Me.grd.RootTable.Columns("OfficeId").Visible = False
            Me.grd.RootTable.Columns("AreaId").Visible = False
    ...
    Private Sub btnAddDock_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAddDock.Click
        Try
            frmProOffice.OfficeId = 0
            frmProOffice.DoHaveSaveRights = DoHaveSaveRights
            frmProOffice.ShowDialog()
            GetAllRecords()
        Catch ex As Exception
    ...
    Private Sub grd_DoubleClick(sender As Object, e As EventArgs) Handles grd.DoubleClick
        Try
            If Me.grd.RowCount > 0 Then
                frmProOffice.OfficeId = Val(Me.grd.CurrentRow.Cells("OfficeId").Value.ToString)
                frmProOffice.DoHaveUpdateRights = DoHaveUpdateRights
            Else
                frmProOffice.OfficeId = 0
            End If
            frmProOffice.ShowDialog()
    ...
    Private Sub grd_ColumnButtonClick(sender As Object, e As Janus.Windows.GridEX.ColumnActionEventArgs) Handles grd.ColumnButtonClick
        Try
            Office = New OfficeBE
            Office.OfficeId = Val(Me.grd.CurrentRow.Cells("OfficeId").Value.ToString)
            If e.Column.Key = "Delete" Then
                If Not msg_Confirm(str_ConfirmDelete) = True Then Exit Sub
                If DoHaveDeleteRights = True Then
                    objDAL.Delete(Office)
