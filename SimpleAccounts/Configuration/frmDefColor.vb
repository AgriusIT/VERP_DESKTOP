...
Public Class frmDefColor
    ...
    Sub BindGrid()
        Try
            ...
            adp = New OleDbDataAdapter("SELECT ArticleColorId, ArticleColorName, ColorCode, Comments, Active, SortOrder FROM         ArticleColorDefTable order by sortorder", Con)
            adp.Fill(dt)
    ...
    Sub EditRecord()
        Try
            Me.uitxtName.Text = DataGridView1.CurrentRow.Cells("ArticleColorName").Value.ToString
            Me.txtColorCode.Text = DataGridView1.CurrentRow.Cells("ColorCode").Value.ToString
    ...
    Private Sub frmDefColor_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub NewToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnNew.Click
        ...
        End Try
    End Sub
    '' ReqId-925 Set Color Code Max Length 5
    Private Sub SaveToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        If Me.BtnSave.Text = "&Save" Then
            ...
            str = "Select ColorCode From ArticleColorDefTable WHERE ColorCode=N'" & Me.txtColorCode.Text.ToString.Replace("'", "''") & "'"
        ...
        Try
            ...
            If Me.BtnSave.Text = "&Save" Or Me.BtnSave.Text = "&Save" Then
                ...
                cm.CommandText = ""
                cm.CommandText = "insert into ArticleColorDefTable(ArticleColorName, ColorCode,  Comments,sortorder, Active ) values(N'" & Me.uitxtName.Text.ToString.Replace("'", "''") & "', N'" & Me.txtColorCode.Text.ToString.Replace("'", "''") & "', N'" & Me.uitxtComments.Text.ToString.Replace("'", "''") & "',N'" & Me.uitxtSortOrder.Text.ToString.Replace("'", "''") & "'," & IIf(Me.uichkActive.Checked = False, 0, 1) & ") Select @@Identity"
                identity = Convert.ToInt32(cm.ExecuteScalar())
            Else
                cm.CommandText = ""
                cm.CommandText = "update ArticleColorDefTable set ArticleColorName=N'" & Me.uitxtName.Text.ToString.Replace("'", "''") & "', ColorCode=N'" & Me.txtColorCode.Text.ToString.Replace("'", "''") & "', Comments=N'" & Me.uitxtComments.Text.ToString.Replace("'", "''") & "', sortorder=N'" & Me.uitxtSortOrder.Text.ToString.Replace("'", "''") & "',Active=" & IIf(Me.uichkActive.Checked = False, 0, 1) & " where ArticleColorId=" & Me.CurrentId
                cm.ExecuteNonQuery()
                ''925 Color Code Update In Article Detail Table 
                cm.CommandText = ""
                cm.CommandText = "update     ArticleDefTable set  ArticleDefTable.ArticleCode =     ArticleDefTableMaster.ArticleCode +'-'+ ArticleSizeDefTable.SizeCode +'-'+ ArticleColorDefTable.ColorCode " _
                    ...
                    & "  where(ArticleColorDefTable.ArticleColorId = " & Me.CurrentId & ") "
                cm.ExecuteNonQuery()
    ...
    Private Sub DeleteToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDelete.Click
        If Not DataGridView1.RowCount > 0 Then
            ...
            Exit Sub
        End If
        If IsValidToDelete("ArticleDefTable", "ArticleColorId", Me.DataGridView1.CurrentRow.Cells("ArticleColorId").Value.ToString) = True Then
            If msg_Confirm(str_ConfirmDelete) = True Then
                Try
                    ...
                    If Con.State = ConnectionState.Closed Then Con.Open()
                    cm.Connection = Con
                    cm.CommandText = "delete from ArticleColorDefTable where ArticleColorId=" & Me.DataGridView1.CurrentRow.Cells("ArticleColorId").Value.ToString
                    cm.ExecuteNonQuery()
                    ...
                    Try
                        ''insert Activity Log
                        SaveActivityLog("Config", Me.Text, EnumActions.Delete, LoginUserId, EnumRecordType.Configuration, Me.DataGridView1.CurrentRow.Cells("ArticleColorId").Value.ToString, True)
                    Catch ex As Exception
                    Finally
                        Me.lblProgress.Visible = False
                    End Try
    ...
    Public Function Get_All(ByVal Id As String)
        Try
            ...
            If IsLoadedForm = True Then
                Dim dt As DataTable = GetDataTable("Select * From ArticleColorDefTable WHERE ArticleColorId=N'" & Id & "'")
                If dt IsNot Nothing Then
                    If dt.Rows.Count > 0 Then
                        Me.uitxtName.Text = dt.Rows(0).Item("ArticleColorName").ToString 'DataGridView1.CurrentRow.Cells("CityName").Value
                        ...
                        Me.CurrentId = dt.Rows(0).Item("ArticleColorId") 'Me.DataGridView1.CurrentRow.Cells(0).Value
                        Me.txtColorCode.Text = dt.Rows(0).Item("ColorCode").ToString
                        Me.BtnSave.Text = "&Update"
                        Me.GetSecurityRights()
                        IsDrillDown = True
                    End If
