...
Public Class FrmZone
    ...
    Sub BindGrid()
        ...
        adp = New OleDbDataAdapter("SELECT zoneId, ZoneName, Comments,Active, SortOrder FROM tblListzone where regionid=" & Me.cmbMain.SelectedValue & " order by sortorder", Con)
        adp.Fill(dt)
    ...
    End Sub
    Sub EditRecord()
        Me.uitxtName.Text = DataGridView1.CurrentRow.Cells("ZoneName").Value
    ...
    Private Sub FrmZone_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub FrmZone_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        FillDropDown(Me.cmbMain, "select * from tblListregion where active=1 order by sortorder")
        IsLoadedForm = True
        Me.RefreshForm()
    ...
    Private Sub SaveToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        Try
            If Me.BtnSave.Text = "&Save" Or Me.BtnSave.Text = "&Save" Then
                cm.CommandText = "insert into tbllistzone(ZoneName,regionid, Comments,sortorder, Active ) values(N'" & Me.uitxtName.Text.ToString.Replace("'", "''") & "'," & Me.cmbMain.SelectedValue & ",N'" & Me.uitxtComments.Text.ToString.Replace("'", "''") & "',N'" & Me.uitxtSortOrder.Text.ToString.Replace("'", "''") & "'," & IIf(Me.uichkActive.Checked = False, 0, 1) & ") Select @@Identity"
            Else
                cm.CommandText = "update tbllistzone set ZoneName=N'" & Me.uitxtName.Text.ToString.Replace("'", "''") & "',Comments=N'" & Me.uitxtComments.Text.ToString.Replace("'", "''") & "', sortorder=N'" & Me.uitxtSortOrder.Text.ToString.Replace("'", "''") & "',Active=" & IIf(Me.uichkActive.Checked = False, 0, 1) & " where ZoneId=" & Me.CurrentId
            End If
    ...
    Private Sub DeleteToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDelete.Click
        If Not DataGridView1.RowCount > 0 Then
            ...
            Exit Sub
        End If
        If IsValidToDelete("tblListBelt", "ZoneId", Me.DataGridView1.CurrentRow.Cells("ZoneId").Value.ToString) = True Then
            If msg_Confirm(str_ConfirmDelete) = True Then
                Try
                    ...
                    If Con.State = ConnectionState.Closed Then Con.Open()
                    cm.Connection = Con
                    cm.CommandText = "delete from tbllistzone where zoneid=" & Me.DataGridView1.CurrentRow.Cells("ZoneId").Value.ToString
                    cm.ExecuteNonQuery()
                    Me.CurrentId = 0

                    Try
                        SaveActivityLog("Config", Me.Text, EnumActions.Delete, LoginUserId, EnumRecordType.Configuration, Me.DataGridView1.CurrentRow.Cells("ZoneId").Value.ToString, True)
                    Catch ex As Exception
            ...
            If IsLoadedForm = True Then
                ...
                If dt IsNot Nothing Then
                    If dt.Rows.Count > 0 Then
                        Me.uitxtName.Text = dt.Rows(0).Item("ZoneName").ToString 'DataGridView1.CurrentRow.Cells("CityName").Value
                        ...
                        Me.CurrentId = dt.Rows(0).Item("ZoneId") 'Me.DataGridView1.CurrentRow.Cells(0).Value
                        Me.BtnSave.Text = "&Update"
                        Me.GetSecurityRights()
                        IsDrillDown = True
                    End If
