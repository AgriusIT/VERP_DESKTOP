...
Public Class urSalesType
    ...
    Public Sub BackgroundWorker1_DoWork(ByVal sender As System.Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorker1.DoWork
        ...
        Try
            ...
            Dim strSQL As String = "SELECT 'Sales' AS Type, Round(ISNULL(SUM(ISNULL(dbo.SalesDetailTable.Qty, 0) * ISNULL(dbo.SalesDetailTable.Price, 0))+SUM(((isnull(Qty,0)*ISNULL(Price,0))*isnull(TaxPercent,0))/100),0),0) AS Amount " _
                                              & "  FROM dbo.SalesMasterTable INNER JOIN " _
                                              & "  dbo.SalesDetailTable ON dbo.SalesMasterTable.SalesId = dbo.SalesDetailTable.SalesId WHERE (dbo.SalesMasterTable.InvoiceType='Cash') AND (Convert(varchar, SalesDate, 102) BETWEEN Convert(Datetime, '" & DateFrom.ToString("yyyy-M-d 00:00:00") & "',102) AND Convert(Datetime, '" & DateTo.ToString("yyyy-M-d 23:59:59") & "', 102))  " & IIf(flgCompanyRights = True, " AND SalesMasterTable.LocationId=" & MyCompanyId & " ", "") & ""
    ...
    Public Sub BackgroundWorker2_DoWork(ByVal sender As System.Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorker2.DoWork
        ...
        Try
            ...
            Dim strSQL As String = "SELECT 'Sales' AS Type, Round(ISNULL(SUM(ISNULL(dbo.SalesDetailTable.Qty, 0) * ISNULL(dbo.SalesDetailTable.Price, 0))+SUM(((isnull(Qty,0)*ISNULL(Price,0))*isnull(TaxPercent,0))/100),0),0) AS Amount " _
                                              & "  FROM dbo.SalesMasterTable INNER JOIN " _
                                              & "  dbo.SalesDetailTable ON dbo.SalesMasterTable.SalesId = dbo.SalesDetailTable.SalesId WHERE (IsNull(dbo.SalesMasterTable.InvoiceType,'Credit')='Credit') AND (Convert(varchar, SalesDate, 102) BETWEEN Convert(Datetime, '" & DateFrom.ToString("yyyy-M-d 00:00:00") & "',102) AND Convert(Datetime, '" & DateTo.ToString("yyyy-M-d 23:59:59") & "', 102))  " & IIf(flgCompanyRights = True, " AND SalesMasterTable.LocationId=" & MyCompanyId & " ", "") & ""
            Dim dt As New DataTable 'Create object of datatable
            dt = GetDataTable(strSQL) 'Using datatable function (GetDataTable()) to assign dt object.
            dt.AcceptChanges() 'Update Record In DataTable
