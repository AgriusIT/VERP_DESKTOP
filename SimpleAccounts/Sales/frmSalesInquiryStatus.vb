...
Public Class frmSalesInquiryStatus
    Private _Dt As DataTable
    Dim SalesInquiryDAL As New SalesInquiryDAL()
    Enum SIStatus
        SalesInquiryId
        SalesInquiryNo
        SalesInquiryDate
    ...
    Private Sub DisplayDetail()
        Try
            ...
            End If
            Dim dt As New DataTable
            dt = SalesInquiryDAL.GetSalesInquiryStatus(CustomerId, FromDate, ToDate, Me.cmbStatus.Text)
            'dt = SalesInquiryDetailDAL.GetAgainstSearch(CustomerId, SalesInquiryId, FromDate, ToDate)
            'dt.AcceptChanges()
    ...
    Private Sub grdSaved_SelectionChanged(sender As Object, e As EventArgs) Handles grdSaved.SelectionChanged
        Try
            ...
            If Me.grdSaved.RowCount = 0 Then Exit Sub
            If Me.SplitContainer1.Panel2Collapsed = True Then Exit Sub
            Me.grdDetail.DataSource = SalesInquiryDAL.GetStatusDetail(Val(Me.grdSaved.GetRow.Cells("SalesInquiryId").Value.ToString))
            Me.grdDetail.RetrieveStructure()
            Me.grdDetail.RootTable.Columns("SalesInquiryId").Visible = False
            Me.grdDetail.RootTable.Columns("SalesInquiryDetailId").Visible = False
            Me.grdDetail.RootTable.Columns("ArticleId").Visible = False
            Me.grdDetail.RootTable.Columns("UnitId").Visible = False
            Me.grdDetail.RootTable.Columns("ItemTypeId").Visible = False
            Me.grdDetail.RootTable.Columns("CategoryId").Visible = False
            Me.grdDetail.RootTable.Columns("SubCategory").Visible = False
            Me.grdDetail.RootTable.Columns("OriginId").Visible = False
            Me.grdDetail.RootTable.Columns("PurchasedQty").Caption = "Purchased Qty"
            Me.grdDetail.RootTable.Columns("VendorQuotationQty").Caption = " Vendor Quot Qty"
        Catch ex As Exception
            ShowErrorMessage(ex.Message)
        End Try
    End Sub
