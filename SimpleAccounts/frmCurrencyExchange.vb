...
Public Class frmCurrencyExchange
    Implements IGeneral
    ...
    Private Sub Button1_Click_1(sender As Object, e As EventArgs) Handles Button1.Click
        Try
            ...
            If Double.TryParse(txtAmount.Text.Trim(), amount) Then
                Dim url As String = String.Format("http://rate-exchange.appspot.com/currency?from={0}&to={1}", cmbfrom.SelectedText, cmbto.SelectedText)
                Dim client As New WebClient()
                Dim rates As String = client.DownloadString(url)
                Dim rate As RateBE = New JavaScriptSerializer().Deserialize(Of RateBE)(rates)
                Dim converted_amount As Double = amount * rate.rate
                Dim message As String = cmbfrom.SelectedItem.Value & ": " & amount & "\n"
                message &= cmbto.SelectedItem.Value & ": " & converted_amount & "\n"
                message &= "Rate: 1 " & cmbfrom.SelectedItem.Value & " = " & rate.rate & " " & cmbto.SelectedItem.Value
