...
Public Class frmReceivingNote
    ...
    Private Function Save() As Boolean
        ...
        Try
            ...
            For i = 0 To grd.RowCount - 1
                ...
                StockDetail.OutQty = 0
                StockDetail.Rate = (Val(grd.GetRows(i).Cells("rate").Value.ToString) + Val(grd.GetRows(i).Cells("Transportation_Charges").Value.ToString) - Val(Me.grd.GetRows(i).Cells("Discount_Price").Value.ToString)) * Val(grd.GetRows(i).Cells("CurrencyRate").Value.ToString)
    ...
    Private Function Update_Record() As Boolean
        ...
        Try
            ...
            objCommand.CommandText = ""
            objCommand.CommandText = "Update ReceivingNoteMasterTable set LocationId=" & gobjLocationId & ", ReceivingNo =N'" & txtPONo.Text & "',ReceivingDate=N'" & dtpPODate.Value.ToString("yyyy-M-d h:mm:ss tt") & "',VendorId=" & cmbVendor.ActiveRow.Cells(0).Value & ", PurchaseOrderId=" & IIf(Me.cmbPo.SelectedIndex > 0, Me.cmbPo.SelectedValue, 0) & ", " _
         & " ReceivingQty=" & Val(Me.grd.GetTotal(Me.grd.RootTable.Columns("Qty"), Janus.Windows.GridEX.AggregateFunction.Sum)) & ",ReceivingAmount=" & Val(Me.grd.GetTotal(Me.grd.RootTable.Columns("Total"), Janus.Windows.GridEX.AggregateFunction.Sum)) & ", CashPaid=" & Val(txtPaid.Text) & ", Remarks=N'" & txtRemarks.Text.Replace("'", "''") & "',UpdateUserName=N'" & LoginUserName & "', DcNo=N'" & Me.txtDcNo.Text.Replace("'", "''") & "', Post=" & IIf(Me.chkPost.Checked = True, 1, 0) & ", Vehicle_No=N'" & Me.txtVhNo.Text.Replace("'", "''") & "', Driver_Name=N'" & Me.txtDriverName.Text.Replace("'", "''") & "', DcDate=" & IIf(Me.dtpDcDate.Checked = True, "N'" & dtpDcDate.Value.ToString("yyyy-M-d h:mm:ss tt") & "'", "NULL") & ", Vendor_Invoice_No=N'" & Me.txtInvoiceNo.Text.Replace("'", "''") & "', CostCenterId=" & Me.cmbProject.SelectedValue & ", IGPNo=N'" & Me.txtIGPNo.Text.Replace("'", "''") & "', CurrencyType=" & IIf(Me.grpCurrency.Visible = True, "" & Me.cmbCurrency.SelectedValue & "", "NULL") & ", CurrencyRate=" & IIf(Me.grpCurrency.Visible = True, "" & Val(Me.txtCurrencyRate.Text) & "", "NULL") & ", Transportation_Vendor=" & IIf(Me.cmbTransportationVendor.ActiveRow Is Nothing, 0, Me.cmbTransportationVendor.Value) & ",Arrival_Time=Convert(DateTime,'" & Me.dtpArrivalTime.Value.ToString("yyyy-M-d hh:mm:ss tt") & "',102), Departure_Time=" & IIf(Me.dtpDepartureTime.Checked = True, "Convert(Datetime,'" & Me.dtpDepartureTime.Value.ToString("yyyy-M-d hh:mm:ss tt") & "',102)", "NULL") & ", LCId = " & IIf(Me.cmbLC.ActiveRow Is Nothing, 0, Me.cmbLC.Value) & "   Where ReceivingNoteID= " & txtReceivingID.Text & " "
            objCommand.ExecuteNonQuery()
            'Marked Against Task#2015060001 Ali Ansari
            'Altered Against Task#2015060001 Ali Ansari
    ...
    Private Sub cmbCurrency_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbCurrency.SelectedIndexChanged
        Try
            If Me.cmbCurrency.SelectedIndex > 0 Then
                ...
                Me.txtCurrencyRate.Text = Math.Round(Convert.ToInt32(dr.Row.Item("CurrencyRate").ToString), 4)
                ''TASK TFS1474
