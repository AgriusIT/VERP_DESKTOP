...
''TASK TFS2179 done by Muhammad Ameen. Creation of new form 'Production Planning Standard'
Imports System.Data.OleDb
Public Class frmProductionPlanningStandard
    ...
    Private Function Validate_AddToGrid() As Boolean
        ...
        End If
        ''Commented Against TFS2976 : Ayesha Rehman : 06-04-2018
    ...
    Private Sub AddItemToGrid()
        ...
        dr(enmGridDetail.PackQty) = Val(Me.txtPackQty.Text)
        'dr(enmGridDetail.CurrentPrice) = Val(Me.cmbItem.SelectedRow.Cells("Price").Text)
        dr(enmGridDetail.CurrentPrice) = Val(Me.cmbItem.SelectedRow.Cells("Sale Price").Value.ToString)
        dr(enmGridDetail.Pack_Desc) = Me.cmbUnit.Text.ToString
        dr(enmGridDetail.PlanDetailId) = 0
        dt.Rows.InsertAt(dr, 0)
        'End If
    ...
    Private Sub FillCombo(ByVal strCondition As String)
        ...
        If strCondition = "Item" Then
            ...
            If Me.cmbItem.DisplayLayout.Bands.Count > 0 Then
                ...
                Me.cmbItem.DisplayLayout.Bands(0).Columns("ProductionProcessId").Hidden = True
            End If


        ElseIf strCondition = "Unit" Then
            'FillDropDown(Me.cmbUnit, "Select PackName, PackName, PackQty From ArticleDefPackTable WHERE ArticleMasterId=" & IIf(Me.cmbItem.IsItemInList = False, 0, Val(Me.cmbItem.SelectedRow.Cells(0).Value.ToString)) & " ORDER BY 1 ASC", False)
    ...
    Private Function Save() As Boolean
        ...
        Try
            ...
            'End Task:2832
            trans.Commit()
            SaveActivityLog("Production", Me.Text, EnumActions.Save, LoginUserId, EnumRecordType.Configuration, Me.txtPONo.Text, True)
            Save = True
            'InsertVoucher()
        Catch ex As Exception
            trans.Rollback()
    ...
    Private Sub DisplayDetail(ByVal ReceivingID As Integer, Optional ByVal Condition As String = "")
        ...
        If Condition = String.Empty Then
          ...
          & " Recv_D.ArticleDefId as ItemId, Recv_D.Sz7 as PackQty, Recv_D.CurrentPrice, Isnull(Recv_D.Pack_Desc,Recv_D.ArticleSize) as Pack_Desc,IsNull(Recv_D.ProducedQty,0) as ProducedQty,IsNull(Recv_D.PlanDetailId,0) as PlanDetailId, IsNull(Recv_D.SODetailId,0) as SODetailId, IsNull(Recv_D.SOId,0) as SOId, IsNull(Recv_D.PLevelId,0) as PLevelId,Recv_D.Comments FROM dbo.PlanDetailTable Recv_D INNER JOIN  " _
          & " dbo.ArticleDefTableMaster Article ON Recv_D.ArticleDefId = Article.ArticleId LEFT JOIN ArticleUnitDefTable ON Article.ArticleUnitId = ArticleUnitDefTable.ArticleUnitId  " _
          & " Where Recv_D.PlanID =" & ReceivingID & ""
        ElseIf Condition = "LoadOrder" Then
            ...
            '            & " dbo.ArticleDefTableMaster Article ON Recv_D.ArticleDefId = Article.ArticleId " _
            ...
            str = "SELECT Recv_D.LocationId, Article.ArticleDescription AS Item, Recv_D.ArticleSize AS Unit, ArticleUnitDefTable.ArticleUnitName As [Unit Name], Recv_D.ArticleAliasName, (Isnull(Recv_D.Qty,0)-IsNull(PlanedQty,0)) AS Qty, Recv_D.Price as Rate, " _
                        & " Recv_D.Qty * Recv_D.Price AS Total, " _
                        & " ArticleDefTable.MasterId as ItemId, Recv_D.Sz7 as PackQty, Recv_D.CurrentPrice, Isnull(Recv_D.Pack_Desc,Recv_D.ArticleSize) as Pack_Desc,0 as ProducedQty, 0 as  PlanDetailId,  IsNull(Recv_D.SalesOrderDetailId,0) as SODetailId, IsNull(Recv_D.SalesOrderId,0) as SOId, 0 as PLevelId, Recv_D.Comments FROM dbo.SalesOrderDetailTable Recv_D INNER JOIN dbo.ArticleDefTable ON Recv_D.ArticleDefId = ArticleDefTable.ArticleId INNER JOIN  " _
                        & " dbo.ArticleDefTableMaster Article ON ArticleDefTable.MasterId = Article.ArticleId " _
                        & " LEFT JOIN ArticleUnitDefTable ON Article.ArticleUnitId = ArticleUnitDefTable.ArticleUnitId " _
                        & " Where Recv_D.SalesOrderId =" & ReceivingID & " AND (Isnull(Recv_D.Sz1,0)-IsNull(PlanedQty,0)) > 0"
        ...
        Me.grd.RootTable.Columns("CurrentPrice").FormatString = "N" & DecimalPointInValue
        Me.grd.RootTable.Columns("PackQty").FormatString = "N" & DecimalPointInQty
        Me.grd.RootTable.Columns("ProducedQty").FormatString = "N" & DecimalPointInQty
    ...
    Private Function Update_Record() As Boolean
        ...
        Try
            ...
            trans.Commit()
            SaveActivityLog("Production", Me.Text, EnumActions.Update, LoginUserId, EnumRecordType.Configuration, Me.txtPONo.Text, True)
            Update_Record = True
            'InsertVoucher()
        Catch ex As Exception
            trans.Rollback()
    ...
    Private Sub cmbItem_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbItem.Leave
        Try
            ...
            Me.txtRate.Text = Me.cmbItem.ActiveRow.Cells("Sale Price").Value.ToString
    ...
    Private Sub DeleteToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDelete.Click
        ...
        If IsValidToDelete("DispatchMasterTable", "PlanId", Me.grdSaved.CurrentRow.Cells("PlanId").Value.ToString) = True Then
            ...
            Try
                ...
                objTrans.Commit()
                SaveActivityLog("Production", Me.Text, EnumActions.Delete, LoginUserId, EnumRecordType.Purchase, grdSaved.CurrentRow.Cells(0).Value.ToString, True)
    ...
    Private Sub CtrlGrdBar1_Load(sender As Object, e As EventArgs) Handles CtrlGrdBar1.Load
        Try
            If IO.File.Exists(Application.ExecutablePath & "\..\Layouts\" & Me.Name & "_" & Me.grdSaved.Name) Then
                ...
                fs.Dispose()
            End If
            Me.CtrlGrdBar1.txtGridTitle.Text = CompanyTitle & Chr(10) & "Production Planning Standard"
    ...
    End Sub
    Public Function GetPackData(ByVal MasterArticleId As Integer) As DataTable
        Try
            ...
            Dim strSQL As String = "Select ArticlePackId, PackName, Isnull(PackQty,1) as PackQty , Isnull(PackRate,1) as PackRate From ArticleDefPackTable WHERE ArticleMasterId = " & MasterArticleId & " ORDER BY PackName ASC "
            'End Task:2618
