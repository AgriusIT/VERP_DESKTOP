...
Public Class frmWIP
    ...
    Public Function Delete(Optional ByVal Condition As String = "") As Boolean Implements IGeneral.Delete
        ...
        Try
            ...
            cmd.CommandText = ""
            cmd.CommandText = "Delete From WIPDetailTable WHERE DocId=" & Val(Me.grdSaved.GetRow.Cells("DocId").Value.ToString) & ""
            cmd.ExecuteNonQuery()

            cmd.CommandText = ""
            cmd.CommandText = "Delete From WIPMasterTable WHERE DocId=" & Val(Me.grdSaved.GetRow.Cells("DocId").Value.ToString) & ""
            cmd.ExecuteNonQuery()
    ...
    Public Sub FillCombos(Optional ByVal Condition As String = "") Implements IGeneral.FillCombos
        Try
            ...
            ElseIf Condition = "Documents" Then
                ...
                'strSQL = "select IGPMasterTable.DocId,DocNo,Job_No from IGPMasterTable left outer join (select DocId,Sum(IsNull(Qty,0)-IsNull(IssuedQty,0)) as Qty " _
                ...
                strSQL = "select IGPMasterTable.DocId,DocNo from IGPMasterTable left outer join (select DocId,Sum(IsNull(Qty,0)-IsNull(IssuedQty,0)) as Qty " _
                            & " from IGPDetailTable group by DocId) a on a.DocId=IGPMasterTable.DocId  left outer join vwCOADetail on IGPMasterTable.CustomerCode = vwCOADetail.coa_detail_id " _
                            & " where DocNo <> '' and IsNull(a.Qty,0) > 0 And vwCOADetail.coa_detail_id = " & Val(Me.cmbVendor.ActiveRow.Cells("coa_detail_id").Value.ToString)
    ...
    Public Sub GetAllRecords(Optional ByVal Condition As String = "") Implements IGeneral.GetAllRecords
        Try
            ...
            If Condition = "Master" Then
                ...
                Me.grdSaved.RootTable.Columns("IGP_NO").Visible = False
                Me.grdSaved.RootTable.Columns("IGPDocNo").Visible = False
            ...
            ElseIf Condition = "Detail" Then
                ...
                strSQL = String.Empty
                strSQL = "SELECT WIP_D.LocationId, WIP_D.ArticleDefId, Art.ArticleDescription, Art.ArticleCode, WIP_D.PackQty, WIP_D.Qty, WIP_D.TotalQty, WIP_D.CurrentPrice,IsNull(WIP_D.PackPrice,0) as PackPrice, " _
                & "  WIP_D.Price, WIP_D.TotalAmount, WIP_D.Comments,IsNull(WIP_D.IssuedQty,0) as IssuedQty,IsNull(WIP_D.IssuedTotalQty,0) as IssuedTotalQty, IsNull(WIP_D.DocDetailId,0) as DocDetailId, IsNull(WIP_D.Ref_DocId,0) as Ref_DocId, IsNull(WIP_D.Ref_DocDetailId,0) as Ref_DocDetailId FROM dbo.WIPDetailTable AS WIP_D INNER JOIN dbo.ArticleDefView AS Art ON WIP_D.ArticleDefId = Art.ArticleId WHERE WIP_D.DocId=" & DocId & " ORDER BY WIP_D.DocDetailId ASC "
                Dim dt As New DataTable
                dt = GetDataTable(strSQL)
                dt.AcceptChanges()
    ...
    Public Sub ReSetControls(Optional ByVal Condition As String = "") Implements IGeneral.ReSetControls
        Try
            ...
            Me.cmbGatepass.SelectedValue = 0
    ...
    Public Function Save(Optional ByVal Condition As String = "") As Boolean Implements IGeneral.Save
        ...
        Try
            ...
            cmd.CommandText = ""
            cmd.CommandText = "INSERT INTO WIPMasterTable(DocNo,DocDate,LocationId,CustomerCode,Job_No,Remarks,TotalQty,TotalAmount,EntryDate,UserName) " _
            & " VALUES('" & Me.txtDocNo.Text & "',Convert(DateTime,'" & Me.dtpDocDate.Value.ToString("yyyy-M-d hh:mm:ss tt") & "',102),1," & Me.cmbVendor.Value & ",N'" & Me.txtJobNo.Text.Replace("'", "''") & "',N'" & Me.txtRemarks.Text.Replace("'", "''") & "'," & Me.grdGatePass.GetTotal(Me.grdGatePass.RootTable.Columns("TotalQty"), Janus.Windows.GridEX.AggregateFunction.Sum) & "," & Me.grdGatePass.GetTotal(Me.grdGatePass.RootTable.Columns("TotalAmount"), Janus.Windows.GridEX.AggregateFunction.Sum) & ",Convert(DateTime,'" & Date.Now.ToString("yyyy-M-d hh:mm:ss tt") & "',102),'" & LoginUserName.Replace("'", "''") & "') Select @@Identity"
    ...
    Public Sub SaveDetail(ByVal DocId As Integer, ByVal trans As OleDbTransaction)
        ...
        Try
            ...
            For Each jsRow As Janus.Windows.GridEX.GridEXRow In Me.grdGatePass.GetRows
                cmd.CommandText = ""
                cmd.CommandText = "INSERT INTO WIPDetailTable(DocId,LocationId,ArticleDefId,PackQty,Qty,TotalQty,CurrentPrice,PackPrice,Price,TotalAmount,Comments,IssuedQty, IssuedTotalQty,Ref_DocId,Ref_DocDetailId) " _
                ...
                & " " & Val(jsRow.Cells("CurrentPrice").Value.ToString) & "," & Val(jsRow.Cells("PackPrice").Value.ToString) & "," & Val(jsRow.Cells("Price").Value.ToString) & "," & Val(jsRow.Cells("TotalAmount").Value.ToString) & ",'" & jsRow.Cells("Comments").Value.ToString & "'," & Val(jsRow.Cells("IssuedQty").Value.ToString) & "," & Val(jsRow.Cells("IssuedTotalQty").Value.ToString) & ", " & Val(jsRow.Cells("Ref_DocId").Value.ToString) & "," & Val(jsRow.Cells("Ref_DocDetailId").Value.ToString) & ")Select @@Identity "
                Dim id As Long = cmd.ExecuteScalar()


                cmd.CommandText = ""
                cmd.CommandText = "Update ServicesInvoiceDetailTable SET Ref_WIP_DocDetailId=" & id & " WHERE Ref_WIP_DocId=" & Val(DocId) & " AND Ref_WIP_DocDetailId=" & Val(jsRow.Cells("DocDetailId").Value.ToString) & ""
                cmd.ExecuteNonQuery()
    ...
    Private Sub SaveDocDetail(ByVal DocId As Integer, ByVal Mode As String, ByVal trans As OleDbTransaction)
        ...
        Try
            ...
            & " (Select Ref_DocId as IGP_No, Ref_DocDetailId, ArticleDefId,PackQty,WIPDetailTable.LocationId, SUM(IsNull(WIPDetailTable.Qty,0))  as Qty, SUM(ISNull(WIPDetailTable.TotalQty,0)) as TotalQty  From WIPDetailTable " _
            & " inner join WIPMasterTable on WIPDetailTable.DocId = WIPMasterTable.DocId WHERE WIPDetailTable.DocId= " & DocId & "  Group By  Ref_DocId, ArticleDefId,PackQty,WIPDetailTable.LocationId,Ref_DocDetailId ) a" _
    ...
    Function GetDocumentNo() As String
        Try
            ...
            If getConfigValueByType("VoucherNo").ToString = "Yearly" Then
                Return GetSerialNo("WIP" + "-" + Microsoft.VisualBasic.Right(Me.dtpDocDate.Value.Year, 2) + "-", "WIPMasterTable", "DocNo")
            ElseIf getConfigValueByType("VoucherNo").ToString = "Monthly" Then
                Return GetNextDocNo("WIP" & "-" & Format(Me.dtpDocDate.Value, "yy") & Me.dtpDocDate.Value.Month.ToString("00"), 4, "WIPMasterTable", "DocNo")
            Else
                Return GetNextDocNo("WIP", 6, "WIPMasterTable", "DocNo")
            End If
        Catch ex As Exception
            Throw ex
        End Try
    ...
    Sub EditRecords()
        Try
            ...
            'Me.cmbGatepass.SelectedValue = Val(Me.grdSaved.GetRow.Cells("IGP_No").Value.ToString)
            'If Me.cmbGatepass.SelectedValue Is Nothing Then
            '    Dim dt As DataTable
            '    dt = CType(Me.cmbGatepass.DataSource, DataTable)
            '    Dim row As DataRow
            '    row = dt.NewRow
            '    row(0) = Val(Me.grdSaved.GetRow.Cells("IGP_NO").Value.ToString)
            '    row(1) = Me.grdSaved.GetRow.Cells("IGPDocNo").Value.ToString
            '    dt.Rows.Add(row)
            '    dt.AcceptChanges()
            'End If
            'Me.cmbGatepass.SelectedValue = Val(Me.grdSaved.GetRow.Cells("IGP_No").Value.ToString)
    ...
    Private Sub btnAdd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAdd.Click
        Try
            ...
            dr(enmDetail.DocDetailId) = 0
            dr(enmDetail.RefDocId) = 0
            dr(enmDetail.RefDocDetailId) = 0
            dt.Rows.Add(dr)
            dt.AcceptChanges()
            DetailClear()
        Catch ex As Exception
    ...
    Private Sub LoadGatePass(ByVal DocId As Integer)
        Try
            ...
            strSQL = "SELECT IGP_D.LocationId, IGP_D.ArticleDefId, Art.ArticleDescription, Art.ArticleCode, IGP_D.PackQty, IsNull(IGP_D.Qty,0)  - IsNull(IGP_D.IssuedQty,0) as Qty , IsNull(IGP_D.TotalQty,0)-IsNull(IGP_D.IssuedTotalQty,0) as TotalQty, IGP_D.CurrentPrice, IsNull(IGP_D.PackPrice,0) as PackPrice, " _
                & "  IGP_D.Price, IGP_D.TotalAmount, IGP_D.Comments,0 as DocDetailId, IGP_D.DocId as Ref_DocId, IGP_D.DocDetailId as Ref_DocDetailId FROM dbo.IGPDetailTable AS IGP_D INNER JOIN dbo.ArticleDefView AS Art ON IGP_D.ArticleDefId = Art.ArticleId WHERE IGP_D.DocId=" & DocId & " And  (IsNull(IGP_D.Qty,0)  - IsNull(IGP_D.IssuedQty,0)) > 0 ORDER BY IGP_D.DocId "
    ...
    'Task#19082015 validate gate pass already enter into grid or not by AHmad SHarif
    Private Function ValidateGatePass(ByVal dt As DataTable) As Boolean
        Try
            For i As Integer = 0 To dt.Rows.Count - 1
                ...
                End If
                'If Val(dt.Rows(i).Item("Ref_DocId").ToString) = Val(Me.cmbGatepass.SelectedValue) Then
                '    For j As Integer = 0 To Me.grdGatePass.RowCount - 1
                '        If Val(dt.Rows(i).Item("ArticleDefId").ToString) = Val(Me.grdGatePass.GetRows(j).Cells("ArticleDefId").Value.ToString) Then
                '            If Val(dt.Rows(i).Item("TotalQty").ToString) = Val(Me.grdGatePass.GetRows(i).Cells("TotalQty").Value.ToString) Then
                '                Return False
                '            End If
                '        End If
                '    Next
    ...
    Private Sub btnRefresh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRefresh.Click
        Try
            ...
            id = Me.cmbGatepass.SelectedValue
            FillCombos("Documents")
            Me.cmbGatepass.SelectedValue = id
    ...
    'Task#19082015 filter Gate passes related to vendor by Ahmad Sharif
    ...
    'Private Sub GetVendorGatePasses()
    '    Try

    '        Dim strSQL As String = String.Empty
    '        strSQL = "select IGPMasterTable.DocId,DocNo from IGPMasterTable left outer join (select DocId,Sum(IsNull(Qty,0)-IsNull(IssuedQty,0)) as Qty " _
    '        & " from IGPDetailTable group by DocId) a on a.DocId=IGPMasterTable.DocId  left outer join vwCOADetail on IGPMasterTable.CustomerCode = vwCOADetail.coa_detail_id " _
    '        & " where DocNo <> '' and a.Qty > 0 And vwCOADetail.coa_detail_id = " & Val(Me.cmbVendor.ActiveRow.Cells("coa_detail_id").Value.ToString)
    '        FillDropDown(Me.cmbGatepass, strSQL)
