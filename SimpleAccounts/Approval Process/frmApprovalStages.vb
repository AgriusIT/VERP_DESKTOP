Imports SBModel
Imports SBDal
Public Class frmApprovalStages
    Implements IGeneral

    Dim Id As Integer = 0I
    Dim ApprovalStages As ApprovalStagesBE
    Public objDAL As ApprovalStagesDAL = New ApprovalStagesDAL()
    Enum enmApprovalStages
        Id
        Title
        Details
        Active
    ...
    Public Sub ApplyGridSettings(Optional Condition As String = "") Implements IGeneral.ApplyGridSettings
        Try
            Me.grd.RootTable.Columns(enmApprovalStages.Id).Visible = False
            Me.grd.RootTable.Columns(enmApprovalStages.Active).TextAlignment = Janus.Windows.GridEX.TextAlignment.Center
            Me.grd.RootTable.Columns(enmApprovalStages.SortOrder).TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
        ...
        Try
            ...
            Else
                If getConfigValueByType("NewSecurityRights").ToString = "False" Or getConfigValueByType("NewSecurityRights").ToString = "Error" Then
                    ...
                    Dim dt As DataTable = GetFormRights(EnumForms.frmApprovalStages)
    ...
    Private Sub CtrlGrdBar1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CtrlGrdBar1.Load
        Try
            If IO.File.Exists(Application.ExecutablePath & "\..\Layouts\" & Me.Name & "_" & Me.grd.Name) Then
                ...
                fs.Dispose()
            End If
            Me.CtrlGrdBar1.txtGridTitle.Text = CompanyTitle & Chr(10) & "Approval Stages"
        Catch ex As Exception
    ...
    Public Function Delete(Optional Condition As String = "") As Boolean Implements IGeneral.Delete
        Try
            ...
            ApprovalStages = New ApprovalStagesBE
            ApprovalStages.ApprovalStagesId = Me.grd.GetRow.Cells(enmApprovalStages.Id).Value
            If New ApprovalStagesDAL().Delete(ApprovalStages) Then
    ...
    Public Sub FillModel(Optional Condition As String = "") Implements IGeneral.FillModel
        Try
            ApprovalStages = New ApprovalStagesBE
            ApprovalStages.ApprovalStagesId = Id
            ApprovalStages.Title = Me.txtTitle.Text
            ApprovalStages.Details = Me.txtDetails.Text
            ApprovalStages.Active = IIf(Me.chkActive.Checked = True, 1, 0)
            ApprovalStages.SortOrder = Me.txtSortOrder.Text
            ApprovalStages.ActivityLog = New ActivityLog
    ...
    Public Sub GetAllRecords(Optional Condition As String = "") Implements IGeneral.GetAllRecords
        Try
            ...
            Me.grd.DataSource = New ApprovalStagesDAL().GetAll
    ...
    Public Function IsValidate(Optional Mode As SBUtility.Utility.EnumDataMode = SBUtility.Utility.EnumDataMode.Disabled, Optional Condition As String = "") As Boolean Implements IGeneral.IsValidate
        Try
            ...
            If Me.txtTitle.Text = String.Empty Then
                ShowErrorMessage("Please enter Stage Title")
                Me.txtTitle.Focus()
                Return False
            End If
    ...
    Public Function Save(Optional Condition As String = "") As Boolean Implements IGeneral.Save
        Try
            If New ApprovalStagesDAL().Add(ApprovalStages) Then
    ...
    Public Function Update1(Optional Condition As String = "") As Boolean Implements IGeneral.Update
        Try
            If New ApprovalStagesDAL().Update(ApprovalStages) Then
    ...
    Public Function ValidateGroupTitle() As Boolean
        Try
            ...
            'Ayesha Rehman: Added to check if type is exists already
            Dim str As String = "SELECT * FROM ApprovalStages WHERE Title = '" & Me.txtTitle.Text & "'"
            Dim dt As DataTable = GetDataTable(str)
            If dt IsNot Nothing Then
                If dt.Rows.Count > 0 Then
    ...
    Public Function ValidateGroupTitleInUpdate() As Boolean
        Try
            ...
            'Ayesha Rehman: Added to check if type is exists already
            Dim str As String = "SELECT * FROM ApprovalStages WHERE Title = '" & Me.txtTitle.Text & "' And ApprovalStagesId != " & Me.grd.CurrentRow.Cells(0).Value & ""
            Dim dt As DataTable = GetDataTable(str)
            If dt IsNot Nothing Then
                If dt.Rows.Count > 0 Then
                    ShowErrorMessage("This Stage Already Exists")
    ...
    Public Sub EditRecords()
        Try
            ...
            Id = Me.grd.GetRow.Cells(enmApprovalStages.Id).Value
            Me.txtTitle.Text = Me.grd.GetRow.Cells(enmApprovalStages.Title).Value.ToString
            Me.txtDetails.Text = Me.grd.GetRow.Cells(enmApprovalStages.Details).Value.ToString
            Me.txtSortOrder.Text = Me.grd.GetRow.Cells(enmApprovalStages.SortOrder).Value.ToString

            If IsDBNull(Me.grd.GetRow().Cells(enmApprovalStages.Active).Value) Then
                Me.chkActive.Checked = False
            Else
                Me.chkActive.Checked = Me.grd.GetRow().Cells(enmApprovalStages.Active).Value
            End If
    ...
    Private Sub grd_ColumnButtonClick(sender As Object, e As Janus.Windows.GridEX.ColumnActionEventArgs) Handles grd.ColumnButtonClick
        Try
            ApprovalStages = New ApprovalStagesBE
            ApprovalStages.ApprovalStagesId = Val(Me.grd.CurrentRow.Cells("ApprovalStagesId").Value.ToString)
            ApprovalStages.ActivityLog = New ActivityLog
            If e.Column.Key = "Delete" Then
                If Not msg_Confirm(str_ConfirmDelete) = True Then Exit Sub
                objDAL.Delete(ApprovalStages)
    ...
    Private Sub frmApprovalStages_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        Try
            CtrlGrdBar1_Load(Nothing, Nothing)
            ReSetControls()
        Catch ex As Exception
