Imports SBModel
Imports SBDal
Public Class frmApprovalProcess
    Implements IGeneral

    Dim Id As Integer = 0I
    Dim ApprovalProcess As ApprovalProcessBE
    Public objDAL As ApprovalProcessDAL = New ApprovalProcessDAL()
    Enum enmApprovalProcess
        Id
        Title
        Details
        Active
    ...
    Public Sub ApplyGridSettings(Optional Condition As String = "") Implements IGeneral.ApplyGridSettings
        Try
            Me.grd.RootTable.Columns(enmApprovalProcess.Id).Visible = False
            Me.grd.RootTable.Columns(enmApprovalProcess.Active).TextAlignment = Janus.Windows.GridEX.TextAlignment.Center
            Me.grd.RootTable.Columns(enmApprovalProcess.SortOrder).TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
        ...
        Try
            ...
            Else
                If getConfigValueByType("NewSecurityRights").ToString = "False" Or getConfigValueByType("NewSecurityRights").ToString = "Error" Then
                    ...
                    Dim dt As DataTable = GetFormRights(EnumForms.frmApprovalProcess)
    ...
    Private Sub CtrlGrdBar1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CtrlGrdBar1.Load
        Try
            If IO.File.Exists(Application.ExecutablePath & "\..\Layouts\" & Me.Name & "_" & Me.grd.Name) Then
                ...
                fs.Dispose()
            End If
            Me.CtrlGrdBar1.txtGridTitle.Text = CompanyTitle & Chr(10) & "Approval Process"
        Catch ex As Exception
    ...
    Public Function Delete(Optional Condition As String = "") As Boolean Implements IGeneral.Delete
        Try
            ...
            ApprovalProcess = New ApprovalProcessBE
            ApprovalProcess.ApprovalProcessId = Me.grd.GetRow.Cells(enmApprovalProcess.Id).Value
            If New ApprovalProcessDAL().Delete(ApprovalProcess) Then
    ...
    Public Sub FillModel(Optional Condition As String = "") Implements IGeneral.FillModel
        Try
            ApprovalProcess = New ApprovalProcessBE
            ApprovalProcess.ApprovalProcessId = Id
            ApprovalProcess.Title = Me.txtTitle.Text
            ApprovalProcess.Details = Me.txtDetails.Text
            ApprovalProcess.Active = IIf(Me.chkActive.Checked = True, 1, 0)
            ApprovalProcess.SortOrder = Me.txtSortOrder.Text
            ApprovalProcess.ActivityLog = New ActivityLog
    ...
    Public Sub GetAllRecords(Optional Condition As String = "") Implements IGeneral.GetAllRecords
        Try
            ...
            Me.grd.DataSource = New ApprovalProcessDAL().GetAll
    ...
    Public Function IsValidate(Optional Mode As SBUtility.Utility.EnumDataMode = SBUtility.Utility.EnumDataMode.Disabled, Optional Condition As String = "") As Boolean Implements IGeneral.IsValidate
        Try
            ...
            If Me.txtTitle.Text = String.Empty Then
                ShowErrorMessage("Please enter Process Title")
                Me.txtTitle.Focus()
                Return False
            End If
    ...
    Public Function Save(Optional Condition As String = "") As Boolean Implements IGeneral.Save
        Try
            If New ApprovalProcessDAL().Add(ApprovalProcess) Then
    ...
    Public Function Update1(Optional Condition As String = "") As Boolean Implements IGeneral.Update
        Try
            If New ApprovalProcessDAL().Update(ApprovalProcess) Then
    ...
    Public Function ValidateGroupTitle() As Boolean
        Try
            ...
            'Ayesha Rehman: Added to check if type is exists already
            Dim str As String = "SELECT * FROM ApprovalProcess WHERE Title = '" & Me.txtTitle.Text & "'"
            Dim dt As DataTable = GetDataTable(str)
            If dt IsNot Nothing Then
                If dt.Rows.Count > 0 Then
                    ShowErrorMessage("This Approval Process Already Exists ")
                    Me.txtTitle.Focus()
                    Return False
                End If
            End If
    ...
    Public Function ValidateGroupTitleInUpdate() As Boolean
        Try
            ...
            'Ayesha Rehman: Added to check if type is exists already
            Dim str As String = "SELECT * FROM ApprovalProcess WHERE Title = '" & Me.txtTitle.Text & "' And ApprovalProcessId != " & Me.grd.CurrentRow.Cells(0).Value & ""
            Dim dt As DataTable = GetDataTable(str)
            If dt IsNot Nothing Then
                If dt.Rows.Count > 0 Then
                    ShowErrorMessage("This ApprovalProcess Already Exists")
                    Me.txtTitle.Focus()
                    Return False
                End If
            End If
    ...
    Public Sub EditRecords()
        Try
            ...
            Id = Me.grd.GetRow.Cells(enmApprovalProcess.Id).Value
            Me.txtTitle.Text = Me.grd.GetRow.Cells(enmApprovalProcess.Title).Value.ToString
            Me.txtDetails.Text = Me.grd.GetRow.Cells(enmApprovalProcess.Details).Value.ToString
            Me.txtSortOrder.Text = Me.grd.GetRow.Cells(enmApprovalProcess.SortOrder).Value.ToString

            If IsDBNull(Me.grd.GetRow().Cells(enmApprovalProcess.Active).Value) Then
                Me.chkActive.Checked = False
            Else
                Me.chkActive.Checked = Me.grd.GetRow().Cells(enmApprovalProcess.Active).Value
            End If
    ...
    Private Sub grd_ColumnButtonClick(sender As Object, e As Janus.Windows.GridEX.ColumnActionEventArgs) Handles grd.ColumnButtonClick
        Try
            ApprovalProcess = New ApprovalProcessBE
            ApprovalProcess.ApprovalProcessId = Val(Me.grd.CurrentRow.Cells("ApprovalProcessId").Value.ToString)
            ApprovalProcess.ActivityLog = New ActivityLog
            If e.Column.Key = "Delete" Then
                If Not msg_Confirm(str_ConfirmDelete) = True Then Exit Sub
                objDAL.Delete(ApprovalProcess)
    ...
    Private Sub frmApprovalProcess_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        Try
            CtrlGrdBar1_Load(Nothing, Nothing)
            ReSetControls()
        Catch ex As Exception
