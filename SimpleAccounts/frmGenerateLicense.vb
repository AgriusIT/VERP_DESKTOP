...
Public Class frmGenerateLicense
    ...
    Private Sub GetSecurityRights()
        Try
            If LoginGroup = "Administrator" Then
                Me.Visible = True
                Me.btnGetFingerPrints.Enabled = True
                Me.btnLoadLicense.Enabled = True
                Exit Sub
            End If
            Dim dt As DataTable = GetFormRights(EnumForms.frmGrdRptClosingStockByGRNnDC)
            Me.Visible = False
            Me.btnGetFingerPrints.Enabled = False
            Me.btnLoadLicense.Enabled = False
    ...
    Private Sub btnSaveKey_Click(sender As Object, e As EventArgs) Handles btnSaveKey.Click
        ...
        Try
            ...
            If msg_Confirm("Do you want to update your key?") = True Then
                ...
                msg_Information("Key saved successfully, thank you for using V ERP.")
    ...
    Private Sub btnGetFingerPrints_Click(sender As Object, e As EventArgs) Handles btnGetFingerPrints.Click
        Try
            If SaveFileDialog1.ShowDialog = Windows.Forms.DialogResult.OK AndAlso SaveFileDialog1.CheckPathExists = True Then
                ...
                Dim strFingerPrintsValue As String = "type=AITL|dbname=" & Me.txtDBName.Text.ToString.ToUpper & "|expiry=" & dtpExpiryDate.Value.ToString.ToUpper & "|users=" & Me.txtUsers.Text.ToString.ToUpper
                IO.File.WriteAllText(SaveFileDialog1.FileName.ToString, Encrypt(strFingerPrintsValue))

                msg_Information("License generted successfully. You can get file from " & SaveFileDialog1.FileName)
    ...
    Private Sub btnLoadLicense_Click(sender As Object, e As EventArgs) Handles btnLoadLicense.Click
        Try
            If Me.OpenFileDialog1.ShowDialog = Windows.Forms.DialogResult.OK AndAlso IO.File.Exists(OpenFileDialog1.FileName) Then
                ...
                If LicenseKey.Contains("|") Then
                    ...
                    For Each key As String In str
                        If key.Contains("=") Then
                            ...
                            Select Case strKey(0).ToString
                                Case "dbname"
                                    Me.txtDBName.Text = strKey(1)
                                Case "expriy"
                                    Me.dtpExpiryDate.Value = CType(strKey(1).ToString, DateTime).ToString("dd-MMM-yyyy")
                                Case "users"
                                    Me.txtUsers.Text = strKey(1)
                            End Select
                        End If
                    Next
                Else
                ...
                'msg_Information("Key saved successfully, thank you for using SIRIUS products.")
                'frmaboutus_Load(Me, Nothing)
                'pnlLicenseUpdate.Visible = False
