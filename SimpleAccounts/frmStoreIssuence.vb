...
''TASK: TFS1083 Load data from Estimation according to New Estimation and old Cost Sheet. Done by Ameen
...
''TFS4858 Ayesha Rehman : 22-10-2018 : "Date wise Avg Rate" configuration should implement on Item drop down while adding item to grid on Store Issuance.
...
Public Class frmStoreIssuence
    ...
    Private Sub frmStoreIssuence_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        ...
        Try
            ...
            UltraDropDownSearching(cmbSecurity, frmModProperty.blnListSeachStartWith, frmModProperty.blnListSeachContains)
            UltraDropDownSearching(cmbCGAccount, frmModProperty.blnListSeachStartWith, frmModProperty.blnListSeachContains)
            UltraDropDownSearching(cmbFixedAssets, frmModProperty.blnListSeachStartWith, frmModProperty.blnListSeachContains)
    ...
    Private Sub DisplayRecord(Optional ByVal strCondition As String = "")
        ...
        Me.grdSaved.RootTable.Columns("DispatchAmount").AggregateFunction = Janus.Windows.GridEX.AggregateFunction.Sum
        'Task:2759 Set rounded foramt
        Me.grdSaved.RootTable.Columns("DispatchAmount").FormatString = "N" & DecimalPointInValue
        Me.grdSaved.RootTable.Columns("DispatchAmount").TotalFormatString = "N" & DecimalPointInValue
        'End Task:2759
    ...
    Private Sub RefreshControls()
        ...
        Try
            ...
            'End If
            If Not Me.cmbBatchNo.SelectedIndex = -1 Then Me.cmbBatchNo.SelectedIndex = 0 ''TFS2739
            'me.cmbVendor
    ...
    Private Sub AddItemToGrid()
        ...
        Try
            ...
            ''TASKT TFS2572 done on 28-02-2018
            If Not Me.cmbBatchNo.SelectedIndex = -1 Then
                drGrd.Item(grdEnm.BatchId) = Val(Me.cmbBatchNo.SelectedValue.ToString)
            Else
                drGrd.Item(grdEnm.BatchId) = 0
            End If
            ''End TFS2572
    ...
    Private Sub FillCombo(ByVal strCondition As String)
        ...
        If strCondition = "Item" Then
            ...
            If Not getConfigValueByType("AvgRate").ToString = "True" Then
                ...
                ''Below line is commented on 25-05-2018 to replace the getting last price formula as it is taking longer time to load. so it needs to be replaced. This issue occured at GreatYuomi
                ...
                str += " where Active=1 "
                'isNull((SELECT isNull(StockDetailTable.Rate,0) FROM StockDetailTable WHERE StockDetailTable.StockTransDetailId = (SELECT  MAX(StockDetailTable.StockTransDetailId) FROM StockDetailTable WHERE StockDetailTable.ArticleDefID = ArticleDefView.ArticleId)),0) as Price
            Else
                'str = "SELECT ArticleDefView.ArticleId as Id, ArticleCode Code, ArticleDescription Item, ArticleSizeName as Size, ArticleColorName as Combination, ArticleUnitName as Unit, Case When IsNull(ArticleDefView.Cost_Price,0) > 0 Then IsNull(ArticleDefView.Cost_Price,0) Else Isnull(PurchasePrice,0) End as Price ,ArticleDefView.ArticleCompanyName as Category,ArticleDefView.ArticleLpoName as Model , ArticleDefView.SizeRangeID as [Size ID], Location.Ranks As Rake, Isnull(SubSubId,0) as AccountId, CGSAccountId, Isnull(ServiceItem,0) as ServiceItem, IsNull(ArticleDefView.SortOrder,0) as SortOrder, IsNull(ArticleDefView.Cost_Price,0) as [Cost Price] , ArticleDefView.MasterId FROM  ArticleDefView LEFT OUTER JOIN (Select ArticalID, Ranks From ArticalDefLocation WHERE Ranks <> '' AND Ranks IS NOT NULL) Location  ON Location.ArticalID = ArticleDefView.MasterId "
                'Ali Faisal : TFS3612 : Avg Rate getting from StockDetailTable existing was not working fine
        ...
        ElseIf strCondition = "CostSheetItem" Then
            ...
            End If
            If Me.cmbPlan.SelectedIndex > 0 Then
                str += " AND ArticleDefTable.MasterId In (Select DISTINCT MasterId From PlanMasterTable INNER JOIN PlanDetailTable On PlanMasterTable.PlanId = PlanDetailTable.PlanId INNER JOIN ArticleDefView on ArticleDefView.ArticleId = PlanDetailTable.ArticleDefId WHERE PlanMasterTable.PlanId=" & Me.cmbPlan.SelectedValue & ")"
            End If
        ...
        ElseIf strCondition = "Department" Then
            'str = "Select Distinct ProdStep_Id, prod_step, prod_Less, sort_Order from tblProSteps INNER JOIN AllocationDetail ON tblProSteps.ProdStep_Id = AllocationDetail.DepartmentID INNER JOIN AllocationMaster ON AllocationDetail.Master_Allocation_ID = AllocationMaster.Master_Allocation_ID Where AllocationMaster.TicketID =" & Me.cmbTicket.SelectedValue & " order by sort_Order "
        ...
        ElseIf strCondition = "ContractTicket" Then
            'Str = "Select Ticket.PlanTicketsId, Ticket.TicketNo + ' ~ ' + Convert(Varchar(12), Ticket.ProductionStartDate, 113) As TicketNo, Ticket.ArticleId, Article.ArticleDescription, Ticket.ProductionStartDate, Ticket.TicketQuantity, Ticket.PlanId, Ticket.PlanDetailId  FROM PlanTickets Ticket Join ArticleDefTable Article On Ticket.ArticleId = Article.ArticleId Where Ticket.PlanId = " & Me.cmbPlan.SelectedValue & " And Ticket.PlanTicketsId Not in (Select PlanTicketId From MaterialEstimation)"
            'str = "Select Distinct Ticket.PlanTicketsId, Ticket.TicketNo + ' ~ ' + Convert(Varchar(12), Ticket.ProductionStartDate, 113) As TicketNo, Ticket.ArticleId, Article.ArticleDescription, Ticket.ProductionStartDate, Ticket.TicketQuantity, Ticket.PlanId, Ticket.PlanDetailId  FROM PlanTickets Ticket Join AllocationMaster ON Ticket.PlanTicketsId = AllocationMaster.TicketID Join ArticleDefTable Article On Ticket.ArticleId = Article.ArticleId Where Ticket.PlanId = " & Me.cmbPlan.SelectedValue & ""
            str = "select TicketId, TicketNo from TicketMasterTable where status = 'Close'"
            FillDropDown(cmbContractTicket, str)
        ...
        ElseIf strCondition = "BatchNo" Then
            ...
            str = "Select  DISTINCT BatchNo, BatchNo From  StockDetailTable  WHERE BatchNo NOT IN ('', '0','xxxx') AND LocationId=" & Me.cmbCategory.SelectedValue & ""
            If getConfigValueByType("ArticleFilterByLocation") = "True" Then
                If flgLocationWiseItem = True Then
            ...
            For i = 0 To dtGrd.Rows.Count - 1
                If dtGrd.Rows(i).Item(grdEnm.TotalQty) > 0 Then
                    ...
                    If flgVehicleIdentificationInfo = True Then
                        ...
                        If Me.grd.GetRows(i).Cells(grdEnm.Engine_No).Value.ToString.Length > 0 Then
                            Engine_No = Me.grd.GetRows(i).Cells(grdEnm.Engine_No).Value.ToString.Substring(4)
                        End If
                        If Me.grd.GetRows(i).Cells(grdEnm.Chassis_No).Value.ToString.Length > 0 Then
                            Chassis_No = Me.grd.GetRows(i).Cells(grdEnm.Chassis_No).Value.ToString.Substring(4)
                        End If
                        objCommand.CommandText = ""
                        objCommand.CommandText = "SELECT dbo.DispatchDetailTable.ArticleDefId, Stuff(DispatchDetailTable.Engine_No, 1, 4, '') As Engine_No, Stuff(DispatchDetailTable.Chassis_No, 1, 4, '') As Chassis_No " _
                                                        & " FROM dbo.ArticleDefTable INNER JOIN " _
                                                        & " dbo.DispatchDetailTable ON dbo.ArticleDefTable.ArticleId = dbo.DispatchDetailTable.ArticleDefId WHERE dbo.ArticleDefTable.ArticleId <> 0"
                        If Engine_No.Length > 0 Then
                            objCommand.CommandText += " AND Stuff(DispatchDetailTable.Engine_No, 1, 4, '') =N'" & Engine_No & "'"
                        End If
                        ...
                        If Chassis_No.Length > 0 Then
                            objCommand.CommandText += " AND Stuff(DispatchDetailTable.Chassis_No, 1, 4, '') =N'" & Chassis_No & "'"
                        End If
                            ...
                            If dtVehicleEngineNoIdentificationInfo.Rows.Count > 0 Then
                                ...
                                If dtVehicleEngineNoIdentificationInfo.Rows(0).Item("Engine_No").ToString.Length > 0 Or Engine_No.Length > 0 Then
                                    If Engine_No = dtVehicleEngineNoIdentificationInfo.Rows(0).Item("Engine_No").ToString Then
                                        Throw New Exception("Engine no [" & Me.grd.GetRows(i).Cells(grdEnm.Engine_No).Value.ToString & "] already exists")
                                    End If
                                End If
                                'End If
                            End If
                            ...
                            If dtVehicleChasisNoIdentificationInfo.Rows.Count > 0 Then
                                ...
                                If dtVehicleChasisNoIdentificationInfo.Rows(0).Item("Chassis_No").ToString.Length > 0 Or Chassis_No.Length > 0 Then
                                    If Chassis_No = dtVehicleChasisNoIdentificationInfo.Rows(0).Item("Chassis_No").ToString Then
                                        Throw New Exception("Chassis no [" & Me.grd.GetRows(i).Cells(grdEnm.Chassis_No).Value.ToString & "] already exists")
                                    End If
                                End If
                                'End If
                            End If
                    ...
                    Else
                        StockDetail.Rate = Val(dtGrd.Rows(i).Item(grdEnm.Rate).ToString)
                    End If
                    ...
                    StockDetail.OutAmount = ((Val(dtGrd.Rows(i).Item(grdEnm.TotalQty).ToString)) * StockDetail.Rate) ''TASK-408
                    ...
                    ''TASK TFS2572 done on 28-02-2018

                    StockDetail.BatchNo = dtGrd.Rows(i).Item(grdEnm.BatchNo).ToString
                    StockDetail.ExpiryDate = CType(dtGrd.Rows(i).Item("ExpiryDate").ToString, Date).ToString("yyyy-M-d h:mm:ss tt")
                    StockDetail.Origin = dtGrd.Rows(i).Item("Origin").ToString
                    ''End TFS2572
                    ''TASK TFS2732
                    StockDetail.Engine_No = dtGrd.Rows(i).Item("Engine_No").ToString
                    StockDetail.Chassis_No = dtGrd.Rows(i).Item("Chassis_No").ToString
                    ''END TASK TFS2732
                    ...
                    objCommand.CommandText = ""
                    objCommand.CommandText = "Insert into DispatchDetailTable (DispatchId, ArticleDefId,ArticleSize, Sz1, Qty,Price, Sz7,CurrentPrice,BatchNo, BatchID,LocationID, ArticleDefMasterId, Pack_Desc, CostPrice, PlanUnit, PlanQty, Lot_No,Rack_No,Comments, SubDepartmentID, AllocationDetailId, ParentId, EstimationId, TotalEstimatedQty, PackPrice, Engine_No, Chassis_No, ExpiryDate, Origin) values( " _
                                                        & " ident_current('DispatchMasterTable'), " & Val(dtGrd.Rows(i).Item(grdEnm.ItemId).ToString) & ",N'" & (dtGrd.Rows(i).Item(grdEnm.Unit).ToString) & "'," & Val(dtGrd.Rows(i).Item(grdEnm.Qty).ToString) & ", " _
                                                        & " " & Val(dtGrd.Rows(i).Item(grdEnm.TotalQty).ToString) & ", " & Val(dtGrd.Rows(i).Item(grdEnm.Rate).ToString) & ", " & Val(dtGrd.Rows(i).Item(grdEnm.PackQty).ToString) & "  , " & Val(dtGrd.Rows(i).Item(grdEnm.CurrentPrice).ToString) & ",N'" & dtGrd.Rows(i).Item(grdEnm.BatchNo).ToString & "'," & dtGrd.Rows(i).Item(grdEnm.BatchId).ToString & "," & dtGrd.Rows(i).Item(grdEnm.LocationId).ToString & ", " & Val(dtGrd.Rows(i).Item(grdEnm.ArticleDefMasterId).ToString) & ", N'" & dtGrd.Rows(i).Item(grdEnm.Pack_Desc).ToString.Replace("'", "''") & "', " & Val(StockDetail.Rate) & ", N'" & dtGrd.Rows(i).Item("PlanUnit").ToString.Replace("'", "''") & "', " & Val(dtGrd.Rows(i).Item("PlanQty").ToString) & ",N'" & dtGrd.Rows(i).Item("LotNo").ToString.Replace("'", "''") & "',N'" & dtGrd.Rows(i).Item("Rack_No").ToString.Replace("'", "''") & "',N'" & dtGrd.Rows(i).Item("Comments").ToString.Replace("'", "''") & "', " & Val(dtGrd.Rows(i).Item(grdEnm.SubDepartmentID).ToString) & " , " & Val(dtGrd.Rows(i).Item("AllocationDetailId").ToString) & ", " & Val(dtGrd.Rows(i).Item("ParentId").ToString) & ", " & Val(dtGrd.Rows(i).Item("EstimationId").ToString) & ", " & Val(dtGrd.Rows(i).Item("EstimatedQty").ToString) & ", " & Val(dtGrd.Rows(i).Item("PackPrice").ToString) & ", N'" & dtGrd.Rows(i).Item("Engine_No").ToString.Replace("'", "''") & "', N'" & dtGrd.Rows(i).Item("Chassis_No").ToString.Replace("'", "''") & "',N'" & IIf(dtGrd.Rows(i).Item("ExpiryDate").ToString = "", "NULL", dtGrd.Rows(i).Item("ExpiryDate").ToString) & "',N'" & dtGrd.Rows(i).Item("Origin").ToString & "')"
                    ...
                    '' TASK: TFS1083 maintain and track record of estimation
                    If Me.cmbTicket.SelectedIndex > 0 Then
                        ...
                        objCommand.CommandText = ""
                        objCommand.CommandText = " If Not Exists(Select TrackEstimationId From tblTrackEstimation Where ArticleId =" & Val(dtGrd.Rows(i).Item(grdEnm.ItemId).ToString) & " And EstimationId = " & Val(dtGrd.Rows(i).Item("EstimationId").ToString) & " And DepartmentId = " & Val(dtGrd.Rows(i).Item(grdEnm.SubDepartmentID).ToString) & ") Insert Into tblTrackEstimation(ArticleId, DispatchedQty, EstimatedQty, EstimationId, DispatchId, DispatchDetailId, DepartmentId) " _
    ...
    Sub EditRecord()
        Try
            ...
            cmbContractTicket.SelectedValue = Me.grdSaved.CurrentRow.Cells("ContractTicketId").Value
            ...
            If Val(Me.grdSaved.GetRow.Cells("PlanTicketId").Value.ToString) > 0 Then
    ...
    Private Sub DisplayPODetail(ByVal ReceivingID As Integer, Optional ByVal Condition As String = "")
        ...
        If Condition = String.Empty Then
            ...
            '           & " dbo.ArticleGroupDefTable Article_Group ON Article.ArticleGroupId = Article_Group.ArticleGroupId LEFT OUTER JOIN ArticleDefTableMaster ON ArticleDefTableMaster.ArticleId = Article.MasterId " _
            '           & " Where Recv_D.PurchaseOrderID =" & ReceivingID & ""
            str = "SELECT Recv_D.LocationId, Article.ArticleCode, Article.ArticleDescription AS item , ArticleColorDefTable.ArticleColorName as Color, 'xxxx' as BatchNo, getDate() as ExpiryDate, '' as Origin, Recv_D.ArticleSize AS unit,  Cast(IsNull(Recv_D.Sz1, 0) As float) AS Qty, Recv_D.Price as Rate, " _
                       & " Convert(float, (Recv_D.Qty * Recv_D.Price)) AS Total, " _
                       & " Article.ArticleGroupId as CategoryId, Recv_D.ArticleDefId as ItemId,Sz7 as PackQty,Recv_D.Price as CurrentPrice, IsNull(Recv_D.PackPrice, 0) As PackPrice, 0 as BatchID, 0 as ArticleDefMasterId, ArticleDefTableMaster.ArticleDescription as ArticleDescriptionMaster, Recv_D.Pack_Desc, Isnull(Article_Group.SubSubId,0) as PurchaseAccountId,Article_Group.CGSAccountId, 0 as CostPrice,  '' as PlanUnit, 0 as PlanQty, '' as LotNo, '' as Rack_No,  '' AS Engine_No, '' AS Chassis_No, Recv_D.Comments as Comments, IsNull(Stock.CurrStock,0) as Stock,  Cast(IsNull(Recv_D.Qty, 0) As float) As TotalQty, 0 As SubDepartmentID, '' As SubDepartment, 0 As AllocationDetailId, 0 As ParentId, 0 As EstimationId, 0 As EstimatedQty, 0 As DispatchId, 0 As DispatchDetailId   FROM dbo.PurchaseOrderDetailTable Recv_D INNER JOIN " _
                       & " dbo.ArticleDefTable Article ON Recv_D.ArticleDefId = Article.ArticleId  LEFT OUTER JOIN ArticleColorDefTable On ArticleColorDefTable.ArticleColorId = Article.ArticleColorId LEFT OUTER JOIN " _
        ...
        ElseIf Condition = "Plan" Then
            ...
            str = "SELECT Recv_D.LocationId, Article.ArticleCode, Article.ArticleDescription AS item , ArticleColorDefTable.ArticleColorName as Color, 'xxxx' as BatchNo, Recv_D.ArticleSize AS unit, ((Cast(Isnull(Recv_D.PlanQty,0) As float)*Cast(IsNull(Article.PackQty, 0) As float))-Cast(isnull(IssuedQty,0) As float)) AS Qty, Recv_D.Price as Rate, " _
                                             & " CASE WHEN recv_d.articlesize = 'Loose' THEN Convert(float, ((Isnull(Recv_D.PlanQty,0)-isnull(IssuedQty,0)) * Recv_D.Price)) ELSE Convert(float,(((Isnull(Recv_D.PlanQty,0)-isnull(IssuedQty,0))  * Article.PackQty) * Recv_D.Price)) END AS Total, " _
                                             & " Article.ArticleGroupId as CategoryId, Recv_D.ArticleDefId as ItemId,Recv_D.PackQty, Recv_D.Price as CurrentPrice, 0 As PackPrice, 0 as BatchID, Recv_D.ArticleMasterId as ArticleDefMasterId, ArticleDefTableMaster.ArticleDescription as ArticleDescriptionMaster, Recv_D.ArticleSize as Pack_Desc, Isnull(Article_Group.SubSubId,0) as PurchaseAccountId, Article_Group.CGSAccountId, 0 as CostPrice, '' as PlanUnit, 0 as PlanQty, '' as LotNo, '' as Rack_No,  '' AS Engine_No, '' AS Chassis_No, '' as Comments, IsNull(Stock.CurrStock,0) as Stock, Cast(IsNull(Recv_D.PlanQty, 0) As float)-Cast(IsNull(Recv_D.IssuedQty, 0) As float) As TotalQty, 0 As SubDepartmentID, '' As SubDepartment, 0 As AllocationDetailId, 0 As ParentId, 0 As EstimationId, 0 As EstimatedQty , 0 As DispatchId, 0 As DispatchDetailId  FROM dbo.PlanCostSheetDetailTable Recv_D INNER JOIN " _
                                             & " dbo.ArticleDefTable Article ON Recv_D.ArticleDefId = Article.ArticleId  LEFT OUTER JOIN ArticleColorDefTable On ArticleColorDefTable.ArticleColorId = Article.ArticleColorId LEFT OUTER JOIN " _
                                             & " dbo.ArticleGroupDefTable Article_Group ON Article.ArticleGroupId = Article_Group.ArticleGroupId LEFT OUTER JOIN ArticleDefTableMaster ON ArticleDefTableMaster.ArticleId = Recv_D.ArticleMasterId LEFT OUTER JOIN(Select ArticleDefID, SUM(IsNull(InQty,0)-ISNull(OutQty,0)) as CurrStock From StockDetailTable Group By ArticleDefId Having SUM(IsNull(InQty,0)-ISNull(OutQty,0)) <> 0) Stock On Stock.ArticleDefId = Recv_D.ArticleDefId " _
                                             & " Where Recv_D.PlanId =" & Me.cmbPlan.SelectedValue & " AND (Isnull(Recv_D.PlanQty,0)-isnull(IssuedQty,0)) <> 0  AND Recv_D.PlanId in (Select PlanId From PlanDetailTable) "
        ElseIf Condition = "Planning" Then
            ''TASK TFS1496 addition column of PackPrice
            str = "SELECT Recv_D.LocationId, Article.ArticleCode, Article.ArticleDescription AS item , ArticleColorDefTable.ArticleColorName as Color, 'xxxx' as BatchNo, getDate() as ExpiryDate, '' as Origin, Recv_D.ArticleSize AS unit, ((Cast(Isnull(Recv_D.PlanQty,0) As float)*Cast(IsNull(Article.PackQty, 0) As float)) -Cast(isnull(IssuedQty,0) As float)) AS Qty, Recv_D.Price as Rate, " _
                                            & " CASE WHEN recv_d.articlesize = 'Loose' THEN Convert(float, ((Isnull(Recv_D.PlanQty,0)-isnull(IssuedQty,0)) * Recv_D.Price)) ELSE Convert(float,(((Isnull(Recv_D.PlanQty,0)-isnull(IssuedQty,0))  * Article.PackQty) * Recv_D.Price)) END AS Total, " _
                                            & " Article.ArticleGroupId as CategoryId, Recv_D.ArticleDefId as ItemId,Recv_D.PackQty, Recv_D.Price as CurrentPrice, 0 As PackPrice, 0 as BatchID, Recv_D.ArticleMasterId as ArticleDefMasterId, ArticleDefTableMaster.ArticleDescription as ArticleDescriptionMaster, Recv_D.ArticleSize as Pack_Desc, Isnull(Article_Group.SubSubId,0) as PurchaseAccountId, Article_Group.CGSAccountId, 0 as CostPrice, '' as PlanUnit, 0 as PlanQty, '' as LotNo, '' as Rack_No,  '' AS Engine_No, '' AS Chassis_No, '' as Comments, IsNull(Stock.CurrStock,0) as Stock, Cast(IsNull(Recv_D.Qty, 0) As float) As TotalQty, 0 As SubDepartmentID, '' As SubDepartment, 0 As AllocationDetailId, 0 As ParentId, 0 As EstimationId, 0 As EstimatedQty, 0 As DispatchId, 0 As DispatchDetailId FROM dbo.PlanDetailTable Recv_D INNER JOIN " _
                                            & " dbo.ArticleDefTable Article ON Recv_D.ArticleDefId = Article.ArticleId  LEFT OUTER JOIN ArticleColorDefTable On ArticleColorDefTable.ArticleColorId = Article.ArticleColorId LEFT OUTER JOIN " _
                                            & " dbo.ArticleGroupDefTable Article_Group ON Article.ArticleGroupId = Article_Group.ArticleGroupId LEFT OUTER JOIN ArticleDefTableMaster ON ArticleDefTableMaster.ArticleId = Recv_D.ArticleMasterId LEFT OUTER JOIN(Select ArticleDefID, SUM(IsNull(InQty,0)-ISNull(OutQty,0)) as CurrStock From StockDetailTable Group By ArticleDefId Having SUM(IsNull(InQty,0)-ISNull(OutQty,0)) <> 0) Stock On Stock.ArticleDefId = Recv_D.ArticleDefId " _
                                            & " Where Recv_D.PlanId =" & Me.cmbPlan.SelectedValue & " AND (Isnull(Recv_D.PlanQty,0)-isnull(IssuedQty,0)) <> 0  AND Recv_D.PlanId in (Select PlanId From PlanDetailTable) "
    ...
    Private Sub DisplayDetail(ByVal ReceivingID As Integer)
        ...
        '& " Where Recv_D.DispatchID =" & ReceivingID & ""
        ''TASK TFS1496 addition column of PackPrice
        str = "SELECT Recv_D.LocationID, Article.ArticleCode, Article.ArticleDescription AS item, ArticleColorDefTable.ArticleColorName as Color, Recv_D.BatchNo , Recv_D.ExpiryDate, isnull(Recv_D.Origin,'') as Origin, Recv_D.ArticleSize AS unit,  Cast(IsNull(Recv_D.Sz1, 0) As float) AS Qty, Recv_D.Price as Rate, " _
        & " Convert(float, (Recv_D.Qty * Recv_D.Price)) AS Total, " _
        & " Article.ArticleGroupId as CategoryId, Recv_D.ArticleDefId as ItemId,Recv_D.Sz7 as PackQty, Recv_D.CurrentPrice, IsNull(Recv_D.PackPrice, 0) As PackPrice, Recv_D.BatchID, Isnull(Recv_D.ArticleDefMasterId,0) as ArticleDefMasterId, MasterItem.ArticleDescription as [ArticleDescriptionMaster], Recv_D.Pack_Desc, Isnull(Article_Group.SubSubId,0) as PurchaseAccountId, Article_Group.CGSAccountId,Isnull(Recv_D.CostPrice,0) as CostPrice, Recv_D.PlanUnit, Isnull(Recv_D.PlanQty,0) as PlanQty,Recv_D.Lot_No as LotNo, Recv_D.Rack_No, Recv_D.Engine_No, Recv_D.Chassis_No, Recv_D.Comments, IsNull(Stock.CurrStock,0) as Stock, Cast(IsNull(Recv_D.Qty, 0) AS float) As TotalQty,  IsNull(SubDepartmentID, 0) As SubDepartmentID, tblProSteps.prod_step As SubDepartment, IsNull(Recv_D.AllocationDetailId, 0) As AllocationDetailId, IsNull(Recv_D.ParentId, 0) As ParentId, IsNull(Recv_D.EstimationId, 0) As EstimationId, IsNull(Recv_D.TotalEstimatedQty, 0) As EstimatedQty, IsNull(Recv_D.DispatchId, 0) As DispatchId, IsNull(Recv_D.DispatchDetailId, 0) As DispatchDetailId, Cast(IsNull(Recv_D.Qty, 0) AS float) As CheckQty, IsNull(tblTrackEstimation.DispatchedQty, 0) As IssuedQty FROM dbo.DispatchDetailTable Recv_D LEFT OUTER JOIN " _
        & " dbo.ArticleDefTable Article ON Recv_D.ArticleDefId = Article.ArticleId  LEFT OUTER JOIN ArticleColorDefTable On ArticleColorDefTable.ArticleColorId = Article.ArticleColorId LEFT OUTER JOIN " _
        ...
        & " INNER JOIN tblDefLocation ON Recv_D.LocationID = tblDefLocation.Location_ID LEFT JOIN tblProSteps ON Recv_D.SubDepartmentID = tblProSteps.ProdStep_Id LEFT OUTER JOIN tblTrackEstimation ON Recv_D.EstimationId = tblTrackEstimation.EstimationId And Recv_D.ArticleDefId = tblTrackEstimation.ArticleId And Recv_D.SubDepartmentID = tblTrackEstimation.DepartmentId" _
        & " Where Recv_D.DispatchID =" & ReceivingID & ""
    ...
    Private Sub DisplayAllocationDetail(ByVal ReceivingID As Integer)
        ...
        Try
            ...
            If flgAvrRate = False Then
                str = "SELECT 1 As LocationID, Article.ArticleCode, Article.ArticleDescription AS item, ArticleColorDefTable.ArticleColorName as Color, '' As BatchNo , getDate() as ExpiryDate, '' as Origin, '' AS unit, Cast((IsNull(Recv_D.Quantity, 0)-IsNull(Recv_D.IssuedQuantity, 0)) As float) AS Qty, Article.PurchasePrice as Rate, " _
             & " (IsNull(Recv_D.Quantity, 0)-IsNull(Recv_D.IssuedQuantity, 0)) *  Article.PurchasePrice  AS Total, " _
             & " Article.ArticleGroupId as CategoryId, Recv_D.ProductID as ItemId, 0 As PackQty, Article.PurchasePrice As CurrentPrice, 0 As BatchID, Isnull(Recv_D.ProductMasterID, 0) as ArticleDefMasterId, ArticleDefTableMaster.ArticleDescription as [ArticleDescriptionMaster], '' As Pack_Desc, Isnull(Article_Group.SubSubId,0) as PurchaseAccountId, Article_Group.CGSAccountId, Article.Cost_Price as CostPrice, '' As PlanUnit, 0 As PlanQty, '' as LotNo, '' As Rack_No,  '' AS Engine_No, '' AS Chassis_No, Recv_D.Remarks As Comments, IsNull(Stock.CurrStock,0) as Stock, Cast((IsNull(Recv_D.Quantity, 0)-IsNull(Recv_D.IssuedQuantity, 0)) As float) As TotalQty, Recv_D.DepartmentID As SubDepartmentID, tblProSteps.prod_step As SubDepartment, Recv_D.DetailAllocation_ID As AllocationDetailId, IsNull(Recv_D.ParentId, 0) As ParentId, 0 As EstimationId, 0 As EstimatedQty, 0 As DispatchId, 0 As DispatchDetailId FROM AllocationDetail Recv_D INNER JOIN AllocationMaster ON Recv_D.Master_Allocation_ID = AllocationMaster.Master_Allocation_ID LEFT OUTER JOIN " _
             & " dbo.ArticleDefTable Article ON Recv_D.ProductID = Article.ArticleId  LEFT OUTER JOIN ArticleColorDefTable On ArticleColorDefTable.ArticleColorId = Article.ArticleColorId LEFT OUTER JOIN " _
             & " dbo.ArticleGroupDefTable Article_Group ON Article.ArticleGroupId = Article_Group.ArticleGroupId LEFT OUTER JOIN ArticleDefTableMaster ON ArticleDefTableMaster.ArticleId = Recv_D.ProductMasterID LEFT OUTER JOIN(Select ArticleDefID, SUM(IsNull(InQty,0)-ISNull(OutQty,0)) as CurrStock From StockDetailTable Group By ArticleDefId Having SUM(IsNull(InQty,0)-ISNull(OutQty,0)) <> 0) Stock On Stock.ArticleDefId = Recv_D.ProductID " _
            & " LEFT JOIN tblProSteps ON Recv_D.DepartmentID = tblProSteps.ProdStep_Id Where AllocationMaster.TicketID =" & ReceivingID & " And IsNull(Recv_D.Quantity, 0) > IsNull(Recv_D.IssuedQuantity, 0)"
            Else
                str = "SELECT 1 As LocationID, Article.ArticleCode, Article.ArticleDescription AS item, ArticleColorDefTable.ArticleColorName as Color, '' As BatchNo , getDate() as ExpiryDate, '' as Origin, '' AS unit, Cast((IsNull(Recv_D.Quantity, 0)-IsNull(Recv_D.IssuedQuantity, 0)) As float) AS Qty, Article.Cost_Price as Rate, " _
             & " (IsNull(Recv_D.Quantity, 0)-IsNull(Recv_D.IssuedQuantity, 0)) *  Article.PurchasePrice  AS Total, " _
             & " Article.ArticleGroupId as CategoryId, Recv_D.ProductID as ItemId, 0 As PackQty, Article.PurchasePrice As CurrentPrice, 0 As BatchID, Isnull(Recv_D.ProductMasterID, 0) as ArticleDefMasterId, ArticleDefTableMaster.ArticleDescription as [ArticleDescriptionMaster], '' As Pack_Desc, Isnull(Article_Group.SubSubId,0) as PurchaseAccountId, Article_Group.CGSAccountId, Article.Cost_Price as CostPrice, '' As PlanUnit, 0 As PlanQty, '' as LotNo, '' As Rack_No,  '' AS Engine_No, '' AS Chassis_No, Recv_D.Remarks As Comments, IsNull(Stock.CurrStock,0) as Stock, Cast((IsNull(Recv_D.Quantity, 0)-IsNull(Recv_D.IssuedQuantity, 0)) As float) As TotalQty, Recv_D.DepartmentID As SubDepartmentID, tblProSteps.prod_step As SubDepartment, Recv_D.DetailAllocation_ID As AllocationDetailId, IsNull(Recv_D.ParentId, 0) As ParentId, 0 As EstimationId, 0 As EstimatedQty, 0 As DispatchId, 0 As DispatchDetailId FROM AllocationDetail Recv_D INNER JOIN AllocationMaster ON Recv_D.Master_Allocation_ID = AllocationMaster.Master_Allocation_ID LEFT OUTER JOIN " _
             & " dbo.ArticleDefTable Article ON Recv_D.ProductID = Article.ArticleId  LEFT OUTER JOIN ArticleColorDefTable On ArticleColorDefTable.ArticleColorId = Article.ArticleColorId LEFT OUTER JOIN " _
             & " dbo.ArticleGroupDefTable Article_Group ON Article.ArticleGroupId = Article_Group.ArticleGroupId LEFT OUTER JOIN ArticleDefTableMaster ON ArticleDefTableMaster.ArticleId = Recv_D.ProductMasterID LEFT OUTER JOIN(Select ArticleDefID, SUM(IsNull(InQty,0)-ISNull(OutQty,0)) as CurrStock From StockDetailTable Group By ArticleDefId Having SUM(IsNull(InQty,0)-ISNull(OutQty,0)) <> 0) Stock On Stock.ArticleDefId = Recv_D.ProductID " _
        ...
        End Try
    End Sub
    Private Sub DisplayEstimationDetail(ByVal TicketId As Integer)
        Dim str As String = String.Empty
        ...
        Try
            ...
            If flgAvrRate = False Then
                ...
                'If flgAvrRate = False Then
                '    str = "SELECT " & Me.cmbCategory.SelectedValue & " As LocationID, Article.ArticleCode, Article.ArticleDescription AS item, ArticleColorDefTable.ArticleColorName as Color, '' As BatchNo , '' AS unit, Case When Sum(IsNull(DispatchDetailTable.TotalEstimatedQty, 0))> 0 Then Sum(Cast((IsNull(DispatchDetailTable.TotalEstimatedQty, 0)-IsNull(DispatchDetailTable.Qty, 0)) As float)) Else Sum(IsNull(Recv_D.Quantity, 0)) End  AS Qty, Article.PurchasePrice as Rate, " _
                ...
                'Else
                '    str = "SELECT " & Me.cmbCategory.SelectedValue & " As LocationID, Article.ArticleCode, Article.ArticleDescription AS item, ArticleColorDefTable.ArticleColorName as Color, '' As BatchNo , '' AS unit, Case When Sum(DispatchDetailTable.TotalEstimatedQty)> 0 Then Sum(Cast((IsNull(DispatchDetailTable.TotalEstimatedQty, 0)-IsNull(DispatchDetailTable.Qty, 0)) As float)) Else Sum(IsNull(Recv_D.Quantity, 0)) End  AS Qty, Article.Cost_Price as Rate, " _
                ' & " Case When Sum(IsNull(DispatchDetailTable.TotalEstimatedQty, 0))> 0 Then Sum(Cast((IsNull(DispatchDetailTable.TotalEstimatedQty, 0)-IsNull(DispatchDetailTable.Qty, 0)) As float)) Else Sum(IsNull(Recv_D.Quantity, 0)) End *  Article.PurchasePrice  AS Total, " _
                ...
                'Commented on 23-07-2017 to load items with out heirarch from old cost sheet
                '        str = "SELECT " & Me.cmbCategory.SelectedValue & " As LocationID, Article.ArticleCode, Article.ArticleDescription AS item, ArticleColorDefTable.ArticleColorName as Color, '' As BatchNo , '' AS unit, Sum(IsNull(Recv_D.Quantity, 0))-Cast(IsNull(tblTrackEstimation.DispatchedQty, 0) As float) AS Qty, Article.PurchasePrice as Rate, " _
                ' & " Sum(Cast(IsNull(Recv_D.Quantity, 0) As float))-Cast(IsNull(tblTrackEstimation.DispatchedQty, 0) As float)  *  Article.PurchasePrice  AS Total, " _
                ...
                ' & " dbo.ArticleDefTable Article ON Convert(Int, Recv_D.ProductID) = Article.ArticleId  LEFT OUTER JOIN ArticleColorDefTable On ArticleColorDefTable.ArticleColorId = Article.ArticleColorId LEFT OUTER JOIN " _
                ' & " dbo.ArticleGroupDefTable Article_Group ON Article.ArticleGroupId = Article_Group.ArticleGroupId LEFT OUTER JOIN ArticleDefTable As MasterItem ON MasterItem.ArticleId = Recv_D.ParentId LEFT OUTER JOIN(Select ArticleDefID, SUM(IsNull(InQty,0)-ISNull(OutQty,0)) as CurrStock From StockDetailTable Group By ArticleDefId Having SUM(IsNull(InQty,0)-ISNull(OutQty,0)) <> 0) Stock On Stock.ArticleDefId = Convert(Int, Recv_D.ProductID) " _
                '& " LEFT JOIN tblProSteps ON Recv_D.SubDepartmentID = tblProSteps.ProdStep_Id LEFT OUTER JOIN tblTrackEstimation ON Recv_D.MaterialEstMasterID = tblTrackEstimation.EstimationId And Recv_D.ProductId = tblTrackEstimation.ArticleId  Where MaterialEstimation.PlanTicketId =" & ReceivingID & " GROUP BY Article.ArticleCode, Article.ArticleDescription, ArticleColorDefTable.ArticleColorName, Article.PurchasePrice, Article.ArticleGroupId, Recv_D.ProductId, Recv_D.PlanItemId, MasterItem.ArticleDescription, Article_Group.SubSubId, Article_Group.CGSAccountId, Article.Cost_Price , Recv_D.SubDepartmentID , tblProSteps.prod_step, IsNull(Recv_D.ParentId, 0), IsNull(Recv_D.MaterialEstMasterID, 0), IsNull(tblTrackEstimation.DispatchedQty, 0), IsNull(tblTrackEstimation.DispatchId, 0), IsNull(tblTrackEstimation.DispatchDetailId, 0)  Having IsNull(tblTrackEstimation.DispatchedQty, 0) < Sum(IsNull(Recv_D.Quantity, 0)) "
                '    Else
                '        str = "SELECT " & Me.cmbCategory.SelectedValue & " As LocationID, Article.ArticleCode, Article.ArticleDescription AS item, ArticleColorDefTable.ArticleColorName as Color, '' As BatchNo , '' AS unit, Sum(IsNull(Recv_D.Quantity, 0))-IsNull(tblTrackEstimation.DispatchedQty, 0) AS Qty, Article.Cost_Price as Rate, " _
                '     & " Sum(Cast(IsNull(Recv_D.Quantity, 0) As float))-Cast(IsNull(tblTrackEstimation.DispatchedQty, 0) As Float)  *  Article.PurchasePrice  AS Total, " _
                '     & " Article.ArticleGroupId as CategoryId, Convert(Int, Recv_D.ProductID) as ItemId, 0 As PackQty, Article.PurchasePrice As CurrentPrice, 0 As BatchID, Isnull(Recv_D.ProductMasterID, 0) as ArticleDefMasterId, MasterItem.ArticleDescription as [ArticleDescriptionMaster], '' As Pack_Desc, Isnull(Article_Group.SubSubId,0) as PurchaseAccountId, Article_Group.CGSAccountId, Article.Cost_Price as CostPrice, '' As PlanUnit, 0 As PlanQty, '' as LotNo, '' As Rack_No, '' As Comments, IsNull(Stock.CurrStock,0) as Stock, Sum(IsNull(Recv_D.Quantity, 0)-IsNull(tblTrackEstimation.DispatchedQty, 0)) End  AS TotalQty, IsNull(Recv_D.SubDepartmentID, 0) As SubDepartmentID, tblProSteps.prod_step As SubDepartment, 0 As AllocationDetailId, IsNull(Recv_D.ParentId, 0) As ParentId, IsNull(Recv_D.MaterialEstMasterID, 0) As EstimationId, Sum(IsNull(Recv_D.Quantity, 0)) As EstimatedQty, IsNull(tblTrackEstimation.DispatchId, 0) As DispatchId, IsNull(tblTrackEstimation.DispatchDetailId, 0) As DispatchDetailId FROM MaterialEstimationDetailTable Recv_D INNER JOIN MaterialEstimation ON Recv_D.MaterialEstMasterID = MaterialEstimation.Id LEFT OUTER JOIN " _
                '     & " dbo.ArticleDefTable Article ON Convert(Int, Recv_D.ProductID)= Article.ArticleId  LEFT OUTER JOIN ArticleColorDefTable On ArticleColorDefTable.ArticleColorId = Article.ArticleColorId LEFT OUTER JOIN " _
                '     & " dbo.ArticleGroupDefTable Article_Group ON Article.ArticleGroupId = Article_Group.ArticleGroupId LEFT OUTER JOIN ArticleDefTable As MasterItem ON MasterItem.ArticleId = Recv_D.ParentId LEFT OUTER JOIN(Select ArticleDefID, SUM(IsNull(InQty,0)-ISNull(OutQty,0)) as CurrStock From StockDetailTable Group By ArticleDefId Having SUM(IsNull(InQty,0)-ISNull(OutQty,0)) <> 0) Stock On Stock.ArticleDefId = Convert(Int, Recv_D.ProductID) " _
                '    & " LEFT JOIN tblProSteps ON Recv_D.SubDepartmentID = tblProSteps.ProdStep_Id LEFT OUTER JOIN tblTrackEstimation ON Recv_D.MaterialEstMasterID = tblTrackEstimation.EstimationId And Recv_D.ProductId = tblTrackEstimation.ArticleId Where MaterialEstimation.PlanTicketId =" & ReceivingID & " GROUP BY Article.ArticleCode, Article.ArticleDescription, ArticleColorDefTable.ArticleColorName, Article.PurchasePrice, Article.ArticleGroupId, Recv_D.ProductId, Recv_D.PlanItemId, MasterItem.ArticleDescription, Article_Group.SubSubId, Article_Group.CGSAccountId, Article.Cost_Price, Recv_D.SubDepartmentID, tblProSteps.prod_step, IsNull(Recv_D.ParentId, 0), IsNull(Recv_D.MaterialEstMasterID, 0), IsNull(tblTrackEstimation.DispatchedQty, 0), IsNull(tblTrackEstimation.DispatchId, 0), IsNull(tblTrackEstimation.DispatchDetailId, 0) Having IsNull(tblTrackEstimation.DispatchedQty, 0) < Sum(IsNull(Recv_D.Quantity, 0)) "
                '    End If
                ' End Commented on 23-07-2017 to load items with out heirarch from old cost sheet
                ''TASK: TFS1083 loading and maintaining record of estimation
                ''TASK TFS1496 addition column of PackPrice
                str = "SELECT " & cmbCategory.SelectedValue & " As LocationID, Article.ArticleCode, Article.ArticleDescription AS item, ArticleColorDefTable.ArticleColorName as Color, '' As BatchNo , getDate() as ExpiryDate, '' as Origin, '' AS unit, (Convert(Float, IsNull(Recv_D.Quantity, 0))-Convert(Float, IsNull(tblTrackEstimation.DispatchedQty, 0))) AS Qty, Article.PurchasePrice as Rate, " _
       & " (IsNull(Recv_D.Quantity, 0)-IsNull(tblTrackEstimation.DispatchedQty, 0))  *  IsNull(Article.PurchasePrice, 0)  AS Total, " _
      & " Article.ArticleGroupId as CategoryId, Recv_D.ProductId as ItemId, 0 As PackQty, Article.PurchasePrice As CurrentPrice, 0 As PackPrice, 0 As BatchID, Isnull(Recv_D.PlanItemId, 0) as ArticleDefMasterId, ArticleDefTableMaster.ArticleDescription as [ArticleDescriptionMaster], '' As Pack_Desc, Isnull(Article_Group.SubSubId,0) as PurchaseAccountId, Article_Group.CGSAccountId, Article.Cost_Price as CostPrice, '' As PlanUnit, 0 As PlanQty, '' as LotNo, '' As Rack_No,  '' AS Engine_No, '' AS Chassis_No, '' As Comments, IsNull(Stock.CurrStock,0) as Stock, (Convert(Float, IsNull(Recv_D.Quantity, 0))-Convert(Float, IsNull(tblTrackEstimation.DispatchedQty, 0))) As TotalQty, IsNull(Recv_D.SubDepartmentID, 0) As SubDepartmentID, tblProSteps.prod_step As SubDepartment, 0 As AllocationDetailId, 0 As ParentId, IsNull(Recv_D.EstimationId, 0) As EstimationId, IsNull(Recv_D.Quantity, 0) As EstimatedQty, IsNull(tblTrackEstimation.DispatchId, 0) As DispatchId, IsNull(tblTrackEstimation.DispatchDetailId, 0) As DispatchDetailId, (Convert(Float, IsNull(Recv_D.Quantity, 0))-Convert(Float, IsNull(tblTrackEstimation.DispatchedQty, 0))) As CheckQty, IsNull(tblTrackEstimation.DispatchedQty, 0) As IssuedQty FROM  " _
     & " dbo.ArticleDefTable Article Inner Join(Select IsNull(Recv_D.MaterialEstMasterID, 0) As EstimationId, IsNull(Recv_D.ProductId, 0) As ProductId, IsNull(MaterialEstimation.PlanTicketId, 0) As PlanTicketId, IsNull(Recv_D.SubDepartmentID, 0) As SubDepartmentID, IsNull(Recv_D.PlanItemId, 0) As PlanItemId, Sum(IsNull(Recv_D.Quantity, 0)) As Quantity FROM MaterialEstimationDetailTable Recv_D INNER JOIN MaterialEstimation ON Recv_D.MaterialEstMasterID = MaterialEstimation.Id Where  MaterialEstimation.PlanTicketId =" & TicketId & " Group By IsNull(Recv_D.MaterialEstMasterID, 0), IsNull(Recv_D.ProductId, 0), IsNull(MaterialEstimation.PlanTicketId, 0), IsNull(Recv_D.SubDepartmentID, 0), IsNull(Recv_D.PlanItemId, 0)) As Recv_D  ON Recv_D.ProductID = Article.ArticleId LEFT OUTER JOIN ArticleColorDefTable On ArticleColorDefTable.ArticleColorId = Article.ArticleColorId LEFT OUTER JOIN " _
      & " dbo.ArticleGroupDefTable Article_Group ON Article.ArticleGroupId = Article_Group.ArticleGroupId LEFT OUTER JOIN(Select ArticleDefID, SUM(IsNull(InQty,0)-ISNull(OutQty,0)) as CurrStock From StockDetailTable Group By ArticleDefId Having SUM(IsNull(InQty,0)-ISNull(OutQty,0)) <> 0) Stock On Stock.ArticleDefId = Convert(Int, Recv_D.ProductId) " _
       & " LEFT JOIN tblProSteps ON Recv_D.SubDepartmentID = tblProSteps.ProdStep_Id LEFT OUTER JOIN tblTrackEstimation ON Recv_D.EstimationId = tblTrackEstimation.EstimationId And Recv_D.ProductId = tblTrackEstimation.ArticleId And Recv_D.SubDepartmentID = tblTrackEstimation.DepartmentId Left Outer Join ArticleDefTableMaster ON Isnull(Recv_D.PlanItemId, 0) = ArticleDefTableMaster.ArticleId Where Recv_D.PlanTicketId =" & TicketId & " And Abs(Convert(decimal(18, 4), IsNull(Recv_D.Quantity, 0))-Convert(decimal(18, 4), IsNull(tblTrackEstimation.DispatchedQty, 0))) > 0 "
            Else
                ''TASK TFS1496 addition column of PackPrice
    ...
    Private Sub DisplayEstimationDepartmentWise(ByVal DepartmentID As Integer, ByVal TicketID As Integer)
      ...
      & " (IsNull(Recv_D.Quantity, 0)-IsNull(tblTrackEstimation.DispatchedQty, 0))  *  IsNull(Article.PurchasePrice, 0)  AS Total, " _
     & " Article.ArticleGroupId as CategoryId, Recv_D.ProductId as ItemId, 0 As PackQty, Article.PurchasePrice As CurrentPrice, 0 As PackPrice, 0 As BatchID, Isnull(Recv_D.PlanItemId, 0) as ArticleDefMasterId, ArticleDefTableMaster.ArticleDescription as [ArticleDescriptionMaster], '' As Pack_Desc, Isnull(Article_Group.SubSubId,0) as PurchaseAccountId, Article_Group.CGSAccountId, Article.Cost_Price as CostPrice, '' As PlanUnit, 0 As PlanQty, '' as LotNo, '' As Rack_No,  '' AS Engine_No, '' AS Chassis_No, '' As Comments, IsNull(Stock.CurrStock,0) as Stock, (Convert(Float, IsNull(Recv_D.Quantity, 0))-Convert(Float, IsNull(tblTrackEstimation.DispatchedQty, 0))) As TotalQty, IsNull(Recv_D.SubDepartmentID, 0) As SubDepartmentID, tblProSteps.prod_step As SubDepartment, 0 As AllocationDetailId, 0 As ParentId, IsNull(Recv_D.EstimationId, 0) As EstimationId, IsNull(Recv_D.Quantity, 0) As EstimatedQty, IsNull(tblTrackEstimation.DispatchId, 0) As DispatchId, IsNull(tblTrackEstimation.DispatchDetailId, 0) As DispatchDetailId, (Convert(Float, IsNull(Recv_D.Quantity, 0))-Convert(Float, IsNull(tblTrackEstimation.DispatchedQty, 0))) As CheckQty, IsNull(tblTrackEstimation.DispatchedQty, 0) As IssuedQty FROM  " _
    & " dbo.ArticleDefTable Article Inner Join(Select IsNull(Recv_D.MaterialEstMasterID, 0) As EstimationId, IsNull(Recv_D.ProductId, 0) As ProductId, IsNull(MaterialEstimation.PlanTicketId, 0) As PlanTicketId, IsNull(Recv_D.SubDepartmentID, 0) As SubDepartmentID, IsNull(Recv_D.PlanItemId, 0) As PlanItemId, Sum(IsNull(Recv_D.Quantity, 0)) As Quantity FROM MaterialEstimationDetailTable Recv_D INNER JOIN MaterialEstimation ON Recv_D.MaterialEstMasterID = MaterialEstimation.Id Where  MaterialEstimation.PlanTicketId =" & TicketID & " And IsNull(Recv_D.SubDepartmentID, 0) = " & DepartmentID & " Group By IsNull(Recv_D.MaterialEstMasterID, 0), IsNull(Recv_D.ProductId, 0), IsNull(MaterialEstimation.PlanTicketId, 0), IsNull(Recv_D.SubDepartmentID, 0), IsNull(Recv_D.PlanItemId, 0)) As Recv_D  ON Recv_D.ProductID = Article.ArticleId LEFT OUTER JOIN ArticleColorDefTable On ArticleColorDefTable.ArticleColorId = Article.ArticleColorId LEFT OUTER JOIN " _
     & " dbo.ArticleGroupDefTable Article_Group ON Article.ArticleGroupId = Article_Group.ArticleGroupId LEFT OUTER JOIN(Select ArticleDefID, SUM(IsNull(InQty,0)-ISNull(OutQty,0)) as CurrStock From StockDetailTable Group By ArticleDefId Having SUM(IsNull(InQty,0)-ISNull(OutQty,0)) <> 0) Stock On Stock.ArticleDefId = Convert(Int, Recv_D.ProductId) " _
      & " LEFT JOIN tblProSteps ON Recv_D.SubDepartmentID = tblProSteps.ProdStep_Id LEFT OUTER JOIN tblTrackEstimation ON Recv_D.EstimationId = tblTrackEstimation.EstimationId And Recv_D.ProductId = tblTrackEstimation.ArticleId And Recv_D.SubDepartmentID = tblTrackEstimation.DepartmentId Left Outer Join ArticleDefTableMaster ON Isnull(Recv_D.PlanItemId, 0) = ArticleDefTableMaster.ArticleId Where Recv_D.PlanTicketId =" & TicketID & " And Recv_D.SubDepartmentID =" & DepartmentID & " And Abs(Convert(decimal(18, 4), IsNull(Recv_D.Quantity, 0))-Convert(decimal(18, 4), IsNull(tblTrackEstimation.DispatchedQty, 0))) > 0 "
            Else
                str = "SELECT " & cmbCategory.SelectedValue & " As LocationID, Article.ArticleCode, Article.ArticleDescription AS item, ArticleColorDefTable.ArticleColorName as Color, '' As BatchNo , getDate() as ExpiryDate, '' as Origin, '' AS unit, (Convert(Float, IsNull(Recv_D.Quantity, 0))-Convert(Float, IsNull(tblTrackEstimation.DispatchedQty, 0))) AS Qty, Article.Cost_Price as Rate, " _
      & " (IsNull(Recv_D.Quantity, 0)-IsNull(tblTrackEstimation.DispatchedQty, 0))  *  IsNull(Article.PurchasePrice, 0)  AS Total, " _
      & " Article.ArticleGroupId as CategoryId, Recv_D.ProductId as ItemId, 0 As PackQty, Article.PurchasePrice As CurrentPrice, 0 As PackPrice, 0 As BatchID, Isnull(Recv_D.PlanItemId, 0) as ArticleDefMasterId, ArticleDefTableMaster.ArticleDescription as [ArticleDescriptionMaster], '' As Pack_Desc, Isnull(Article_Group.SubSubId,0) as PurchaseAccountId, Article_Group.CGSAccountId, Article.Cost_Price as CostPrice, '' As PlanUnit, 0 As PlanQty, '' as LotNo, '' As Rack_No, '' AS Engine_No, '' AS Chassis_No, '' As Comments, IsNull(Stock.CurrStock,0) as Stock, (Convert(Float, IsNull(Recv_D.Quantity, 0))-Convert(Float, IsNull(tblTrackEstimation.DispatchedQty, 0))) As TotalQty, IsNull(Recv_D.SubDepartmentID, 0) As SubDepartmentID, tblProSteps.prod_step As SubDepartment, 0 As AllocationDetailId, 0 As ParentId, IsNull(Recv_D.EstimationId, 0) As EstimationId, IsNull(Recv_D.Quantity, 0) As EstimatedQty, IsNull(tblTrackEstimation.DispatchId, 0) As DispatchId, IsNull(tblTrackEstimation.DispatchDetailId, 0) As DispatchDetailId, (Convert(Float, IsNull(Recv_D.Quantity, 0))-Convert(Float, IsNull(tblTrackEstimation.DispatchedQty, 0))) As CheckQty, IsNull(tblTrackEstimation.DispatchedQty, 0) As IssuedQty FROM  " _
     ...
     & " LEFT JOIN tblProSteps ON Recv_D.SubDepartmentID = tblProSteps.ProdStep_Id LEFT OUTER JOIN tblTrackEstimation ON Recv_D.EstimationId = tblTrackEstimation.EstimationId And Recv_D.ProductId = tblTrackEstimation.ArticleId  And Recv_D.SubDepartmentID = tblTrackEstimation.DepartmentId Left Outer Join ArticleDefTableMaster ON Isnull(Recv_D.PlanItemId, 0) = ArticleDefTableMaster.ArticleId Where Recv_D.PlanTicketId =" & TicketID & " And Recv_D.SubDepartmentID =" & DepartmentID & " And Abs(Convert(decimal(18, 4), IsNull(Recv_D.Quantity, 0))-Convert(decimal(18, 4), IsNull(tblTrackEstimation.DispatchedQty, 0))) > 0"
            ...
            ''Commented on 23-07-2017 to load items with out heirarch from old cost sheet
            '        str = "SELECT 1 As LocationID, Article.ArticleCode, Article.ArticleDescription AS item, ArticleColorDefTable.ArticleColorName as Color, '' As BatchNo , '' AS unit, Sum(IsNull(Recv_D.Quantity, 0))-Cast(IsNull(tblTrackEstimation.DispatchedQty, 0) As float) AS Qty, Article.PurchasePrice as Rate, " _
            ' & " Sum(Cast(IsNull(Recv_D.Quantity, 0) As float))-Cast(IsNull(tblTrackEstimation.DispatchedQty, 0) As float)  *  Article.PurchasePrice  AS Total, " _
            ' & " Article.ArticleGroupId as CategoryId, Convert(Int, Recv_D.ProductID) as ItemId, 0 As PackQty, Article.PurchasePrice As CurrentPrice, 0 As BatchID, Isnull(Recv_D.PlanItemId, 0) as ArticleDefMasterId, MasterItem.ArticleDescription as [ArticleDescriptionMaster], '' As Pack_Desc, Isnull(Article_Group.SubSubId,0) as PurchaseAccountId, Article_Group.CGSAccountId, Article.Cost_Price as CostPrice, '' As PlanUnit, 0 As PlanQty, '' as LotNo, '' As Rack_No, '' As Comments, Sum(IsNull(Stock.CurrStock,0)) as Stock, Sum(IsNull(Recv_D.Quantity, 0))-IsNull(tblTrackEstimation.DispatchedQty, 0) As TotalQty, IsNull(Recv_D.SubDepartmentID, 0) As SubDepartmentID, tblProSteps.prod_step As SubDepartment, 0 As AllocationDetailId, IsNull(Recv_D.ParentId, 0) As ParentId, IsNull(Recv_D.MaterialEstMasterID, 0) As EstimationId, Sum(IsNull(Recv_D.Quantity, 0)) As EstimatedQty, IsNull(tblTrackEstimation.DispatchId, 0) As DispatchId, IsNull(tblTrackEstimation.DispatchDetailId, 0) As DispatchDetailId FROM MaterialEstimationDetailTable Recv_D INNER JOIN MaterialEstimation ON Recv_D.MaterialEstMasterID = MaterialEstimation.Id LEFT OUTER JOIN " _
            ' & " dbo.ArticleDefTable Article ON Convert(Int, Recv_D.ProductID) = Article.ArticleId  LEFT OUTER JOIN ArticleColorDefTable On ArticleColorDefTable.ArticleColorId = Article.ArticleColorId LEFT OUTER JOIN " _
            ' & " dbo.ArticleGroupDefTable Article_Group ON Article.ArticleGroupId = Article_Group.ArticleGroupId LEFT OUTER JOIN ArticleDefTable As MasterItem ON MasterItem.ArticleId = Recv_D.ParentId LEFT OUTER JOIN(Select ArticleDefID, SUM(IsNull(InQty,0)-ISNull(OutQty,0)) as CurrStock From StockDetailTable Group By ArticleDefId Having SUM(IsNull(InQty,0)-ISNull(OutQty,0)) <> 0) Stock On Stock.ArticleDefId = Convert(Int, Recv_D.ProductID) " _
            '& " LEFT JOIN tblProSteps ON Recv_D.SubDepartmentID = tblProSteps.ProdStep_Id LEFT OUTER JOIN tblTrackEstimation ON Recv_D.MaterialEstMasterID = tblTrackEstimation.EstimationId And Recv_D.ProductId = tblTrackEstimation.ArticleId  Where MaterialEstimation.PlanTicketId =" & ReceivingID & " GROUP BY Article.ArticleCode, Article.ArticleDescription, ArticleColorDefTable.ArticleColorName, Article.PurchasePrice, Article.ArticleGroupId, Recv_D.ProductId, Recv_D.PlanItemId, MasterItem.ArticleDescription, Article_Group.SubSubId, Article_Group.CGSAccountId, Article.Cost_Price , Recv_D.SubDepartmentID , tblProSteps.prod_step, IsNull(Recv_D.ParentId, 0), IsNull(Recv_D.MaterialEstMasterID, 0), IsNull(tblTrackEstimation.DispatchedQty, 0), IsNull(tblTrackEstimation.DispatchId, 0), IsNull(tblTrackEstimation.DispatchDetailId, 0)  Having IsNull(tblTrackEstimation.DispatchedQty, 0) < Sum(IsNull(Recv_D.Quantity, 0)) "
            '    Else
            '        str = "SELECT 1 As LocationID, Article.ArticleCode, Article.ArticleDescription AS item, ArticleColorDefTable.ArticleColorName as Color, '' As BatchNo , '' AS unit, Sum(IsNull(Recv_D.Quantity, 0))-IsNull(tblTrackEstimation.DispatchedQty, 0) AS Qty, Article.Cost_Price as Rate, " _
            '     & " Sum(Cast(IsNull(Recv_D.Quantity, 0) As float))-Cast(IsNull(tblTrackEstimation.DispatchedQty, 0) As Float)  *  Article.PurchasePrice  AS Total, " _
            '     & " Article.ArticleGroupId as CategoryId, Convert(Int, Recv_D.ProductID) as ItemId, 0 As PackQty, Article.PurchasePrice As CurrentPrice, 0 As BatchID, Isnull(Recv_D.ProductMasterID, 0) as ArticleDefMasterId, MasterItem.ArticleDescription as [ArticleDescriptionMaster], '' As Pack_Desc, Isnull(Article_Group.SubSubId,0) as PurchaseAccountId, Article_Group.CGSAccountId, Article.Cost_Price as CostPrice, '' As PlanUnit, 0 As PlanQty, '' as LotNo, '' As Rack_No, '' As Comments, IsNull(Stock.CurrStock,0) as Stock, Sum(IsNull(Recv_D.Quantity, 0)-IsNull(tblTrackEstimation.DispatchedQty, 0)) End  AS TotalQty, IsNull(Recv_D.SubDepartmentID, 0) As SubDepartmentID, tblProSteps.prod_step As SubDepartment, 0 As AllocationDetailId, IsNull(Recv_D.ParentId, 0) As ParentId, IsNull(Recv_D.MaterialEstMasterID, 0) As EstimationId, Sum(IsNull(Recv_D.Quantity, 0)) As EstimatedQty, IsNull(tblTrackEstimation.DispatchId, 0) As DispatchId, IsNull(tblTrackEstimation.DispatchDetailId, 0) As DispatchDetailId FROM MaterialEstimationDetailTable Recv_D INNER JOIN MaterialEstimation ON Recv_D.MaterialEstMasterID = MaterialEstimation.Id LEFT OUTER JOIN " _
            '     & " dbo.ArticleDefTable Article ON Convert(Int, Recv_D.ProductID)= Article.ArticleId  LEFT OUTER JOIN ArticleColorDefTable On ArticleColorDefTable.ArticleColorId = Article.ArticleColorId LEFT OUTER JOIN " _
            '     & " dbo.ArticleGroupDefTable Article_Group ON Article.ArticleGroupId = Article_Group.ArticleGroupId LEFT OUTER JOIN ArticleDefTable As MasterItem ON MasterItem.ArticleId = Recv_D.ParentId LEFT OUTER JOIN(Select ArticleDefID, SUM(IsNull(InQty,0)-ISNull(OutQty,0)) as CurrStock From StockDetailTable Group By ArticleDefId Having SUM(IsNull(InQty,0)-ISNull(OutQty,0)) <> 0) Stock On Stock.ArticleDefId = Convert(Int, Recv_D.ProductID) " _
            '    & " LEFT JOIN tblProSteps ON Recv_D.SubDepartmentID = tblProSteps.ProdStep_Id LEFT OUTER JOIN tblTrackEstimation ON Recv_D.MaterialEstMasterID = tblTrackEstimation.EstimationId And Recv_D.ProductId = tblTrackEstimation.ArticleId Where AllocationMaster.TicketID =" & ReceivingID & " GROUP BY Article.ArticleCode, Article.ArticleDescription, ArticleColorDefTable.ArticleColorName, Article.PurchasePrice, Article.ArticleGroupId, Recv_D.ProductId, Recv_D.PlanItemId, MasterItem.ArticleDescription, Article_Group.SubSubId, Article_Group.CGSAccountId, Article.Cost_Price, Recv_D.SubDepartmentID, tblProSteps.prod_step, IsNull(Recv_D.ParentId, 0), IsNull(Recv_D.MaterialEstMasterID, 0), IsNull(tblTrackEstimation.DispatchedQty, 0), IsNull(tblTrackEstimation.DispatchId, 0), IsNull(tblTrackEstimation.DispatchDetailId, 0) Having IsNull(tblTrackEstimation.DispatchedQty, 0) < Sum(IsNull(Recv_D.Quantity, 0)) "
            '    End If
            '' End Commented on 23-07-2017 to load items with out heirarch from old cost sheet
    ...
    Private Function Update_Record() As Boolean
        ...
        Try
            ...
            objCommand.CommandText = "Update DispatchMasterTable set DispatchNo =N'" & txtPONo.Text & "',DispatchDate=N'" & dtpPODate.Value.ToString("yyyy-M-d h:mm:ss tt") & "',VendorId=" & cmbVendor.ActiveRow.Cells(0).Value & ", PurchaseOrderId=" & IIf(Me.cmbPo.SelectedIndex > 0, Me.cmbPo.SelectedValue, "NULL") & ", " _
                                 & " DispatchQty=" & Val(Me.grd.GetTotal(Me.grd.RootTable.Columns("TotalQty"), Janus.Windows.GridEX.AggregateFunction.Sum)) & ",DispatchAmount=" & Val(Me.grd.GetTotal(Me.grd.RootTable.Columns("Total"), Janus.Windows.GridEX.AggregateFunction.Sum)) & ", CashPaid=" & Val(txtPaid.Text) & ", Remarks=N'" & txtRemarks.Text.Replace("'", "''") & "', PONo=N'" & Me.txtPO_No.Text.Replace("'", "''") & "', PlanId=" & IIf(Me.cmbPlan.SelectedIndex = -1, 0, Me.cmbPlan.SelectedValue) & ", EmpId=" & Me.ComboBox1.SelectedValue & ", FixedAssetAccountId=" & Me.cmbFixedAssets.Value & ",  CylinderSecurityAccountId=" & Me.cmbSecurity.Value & ", Issued=" & IIf(Me.chkIssued.Checked = True, 1, 0) & ", DepartmentId=" & IIf(Me.cmbDepartment.SelectedIndex = -1, "NULL", Me.cmbDepartment.SelectedValue) & ",StoreIssuanceAccountId=" & AccountId2 & ",UpdateUser = '" & LoginUserName & "', PlanTicketId= " & IIf(Me.cmbTicket.SelectedIndex = -1, "NULL", Me.cmbTicket.SelectedValue) & " , ContractTicketId= " & IIf(Me.cmbContractTicket.SelectedIndex = -1, "NULL", Me.cmbContractTicket.SelectedValue) & " Where DispatchID= " & txtReceivingID.Text & " " ''TASK-408 added TotalQty instead of Qty on 13-06-2016
            'Altered by Ali Ansari Task#201508020 regarding update user column
            objCommand.ExecuteNonQuery()
            ...
            If Me.chkIssued.Checked = True Then 'Task:M21 If Issued Checked Then Update Voucher
                If (flgStoreIssuenceVoucher = True Or flgCylinderVoucher = True) Then 'Code By Imran 3-7-2013 Ref by Request ID: 725
                    If lngVoucherMasterId > 0 Then
                        ...
                        'Task:M101 Added Field Remarks
                        'TASK TFS1427 added Posted_UserName 
            ...
            If Me.cmbTicket.SelectedIndex > 0 Then
                ...
                objCommand.CommandText = ""
                objCommand.CommandText = "Select ArticleDefId, Qty, IsNull(EstimationId, 0) As EstimationId, IsNull(SubDepartmentId, 0) As SubDepartmentId From DispatchDetailTable Where DispatchId=" & Val(Me.txtReceivingID.Text) & ""
                ...
                If Not eDt Is Nothing Then
                    If eDt.Rows.Count > 0 Then
                        For Each dr As DataRow In eDt.Rows
                            objCommand.CommandText = ""
                            objCommand.CommandText = " Update tblTrackEstimation Set DispatchedQty=IsNull(DispatchedQty,0)- " & dr.Item("Qty") & " Where  EstimationId=" & dr.Item("EstimationId") & " And ArticleId=" & dr.Item("ArticleDefId") & " And DepartmentId=" & dr.Item("SubDepartmentId") & " "
            ...
            For i = 0 To dtGrd.Rows.Count - 1
                If dtGrd.Rows(i).Item(grdEnm.TotalQty) > 0 Then
                    ...
                    StockDetail.InAmount = 0
                    StockDetail.OutAmount = ((Val(dtGrd.Rows(i).Item(grdEnm.TotalQty).ToString)) * StockDetail.Rate) ''TASK-408 added TotalQty instead of Qty on 13-06-2016
                    StockDetail.Remarks = dtGrd.Rows(i).Item(grdEnm.Comments).ToString
                    ''Start TASK-470 on 01-07-2016
                    StockDetail.PackQty = dtGrd.Rows(i).Item(grdEnm.PackQty).ToString
                    StockDetail.Out_PackQty = dtGrd.Rows(i).Item(grdEnm.Qty).ToString
                    ...
                    objCommand.CommandText = "Insert into DispatchDetailTable (DispatchId, ArticleDefId,ArticleSize, Sz1, Qty,Price, Sz7,CurrentPrice,BatchNo, BatchID,LocationID, ArticleDefMasterId, Pack_Desc,CostPrice, PlanUnit, PlanQty, Lot_No, Rack_No,Comments, SubDepartmentID, AllocationDetailId, ParentId, EstimationId, TotalEstimatedQty, PackPrice, Engine_No, Chassis_No, ExpiryDate, Origin) values( " _
                                           & " " & txtReceivingID.Text & " ," & Val(dtGrd.Rows(i).Item(grdEnm.ItemId).ToString) & ",N'" & (dtGrd.Rows(i).Item(grdEnm.Unit).ToString) & "'," & Val(dtGrd.Rows(i).Item(grdEnm.Qty).ToString) & ", " _
                                           & " " & Val(dtGrd.Rows(i).Item(grdEnm.TotalQty).ToString) & ", " & Val(dtGrd.Rows(i).Item(grdEnm.Rate).ToString) & ", " & Val(dtGrd.Rows(i).Item(grdEnm.PackQty).ToString) & "  , " & Val(dtGrd.Rows(i).Item(grdEnm.CurrentPrice).ToString) & ",N'" & dtGrd.Rows(i).Item(grdEnm.BatchNo).ToString & "'," & dtGrd.Rows(i).Item(grdEnm.BatchId).ToString & "," & dtGrd.Rows(i).Item(grdEnm.LocationId).ToString & ", " & Val(dtGrd.Rows(i).Item(grdEnm.ArticleDefMasterId).ToString) & ", N'" & dtGrd.Rows(i).Item(grdEnm.Pack_Desc).ToString.Replace("'", "''") & "', " & Val(StockDetail.Rate) & ", N'" & dtGrd.Rows(i).Item("PlanUnit").ToString.Replace("'", "''") & "', " & Val(dtGrd.Rows(i).Item("PlanQty").ToString) & ", N'" & dtGrd.Rows(i).Item("LotNo").ToString.Replace("'", "''") & "',N'" & dtGrd.Rows(i).Item("Rack_No").ToString.Replace("'", "''") & "', N'" & dtGrd.Rows(i).Item("Comments").ToString.Replace("'", "''") & "', " & Val(dtGrd.Rows(i).Item(grdEnm.SubDepartmentID).ToString) & ", " & Val(dtGrd.Rows(i).Item("AllocationDetailId").ToString) & ", " & Val(dtGrd.Rows(i).Item("ParentId").ToString) & ", " & Val(dtGrd.Rows(i).Item("EstimationId").ToString) & ", " & Val(dtGrd.Rows(i).Item("EstimatedQty").ToString) & ", " & Val(dtGrd.Rows(i).Item("PackPrice").ToString) & ", N'" & dtGrd.Rows(i).Item("Engine_No").ToString.Replace("'", "''") & "', N'" & dtGrd.Rows(i).Item("Chassis_No").ToString.Replace("'", "''") & "',N'" & IIf(dtGrd.Rows(i).Item("ExpiryDate").ToString = "", "NULL", dtGrd.Rows(i).Item("ExpiryDate").ToString) & "',N'" & dtGrd.Rows(i).Item("Origin").ToString & "')" ''TASK-408 added TotalQty instead of Qty on 13-06-2016
                    '& " " & IIf(dtGrd.Rows(i).Item(grdEnm.Unit).ToString = "Loose", Val(dtGrd.Rows(i).Item(grdEnm.Qty).ToString), (Val(dtGrd.Rows(i).Item(grdEnm.Qty).ToString) * Val(dtGrd.Rows(i).Item(grdEnm.PackQty).ToString))) & ", " & Val(dtGrd.Rows(i).Item(grdEnm.Rate).ToString) & ", " & Val(dtGrd.Rows(i).Item(grdEnm.PackQty).ToString) & "  , " & Val(dtGrd.Rows(i).Item(grdEnm.CurrentPrice).ToString) & ",N'" & dtGrd.Rows(i).Item(grdEnm.BatchNo).ToString & "'," & dtGrd.Rows(i).Item(grdEnm.BatchId).ToString & "," & dtGrd.Rows(i).Item(grdEnm.LocationId).ToString & ", " & Val(dtGrd.Rows(i).Item(grdEnm.ArticleDefMasterId).ToString) & ", N'" & dtGrd.Rows(i).Item(grdEnm.Pack_Desc).ToString.Replace("'", "''") & "', " & Val(StockDetail.Rate) & ", N'" & dtGrd.Rows(i).Item("PlanUnit").ToString.Replace("'", "''") & "', " & Val(dtGrd.Rows(i).Item("PlanQty").ToString) & ", N'" & dtGrd.Rows(i).Item("LotNo").ToString.Replace("'", "''") & "',N'" & dtGrd.Rows(i).Item("Rack_No").ToString.Replace("'", "''") & "', N'" & dtGrd.Rows(i).Item("Comments").ToString.Replace("'", "''") & "')"
                    objCommand.ExecuteNonQuery()
                    If Me.cmbTicket.SelectedIndex > 0 Then
                        '                        TrackEstimationId	int	Unchecked
    ...
    Private Sub cmbItem_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbItem.Leave
        Try
            ...
            ''Below lines are commented on 28-02-2018
    ...
    Private Sub DeleteToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        Try
            ...
            If Me.cmbPlan.SelectedIndex > 0 Then
                ...
                End If
            End If
            ''TASK: TFS1083 reduced the quantity from estimation in case deletion.
            cm.CommandText = ""
            cm.CommandText = "Select ArticleDefId, Qty, IsNull(EstimationId, 0) As EstimationId, IsNull(SubDepartmentId, 0) As SubDepartmentId From DispatchDetailTable Where DispatchId=" & Val(Me.grdSaved.CurrentRow.Cells("DispatchId").Value.ToString) & ""
            ...
            If Not eDt Is Nothing Then
                If eDt.Rows.Count > 0 Then
                    For Each dr As DataRow In eDt.Rows
                        cm.CommandText = ""
                        cm.CommandText = " Update tblTrackEstimation Set DispatchedQty=IsNull(DispatchedQty,0)- " & dr.Item("Qty") & " Where  EstimationId=" & dr.Item("EstimationId") & " And ArticleId=" & dr.Item("ArticleDefId") & " And DepartmentId=" & dr.Item("SubDepartmentId") & " "
                        'cm.CommandText = " Update tblTrackEstimation Set DispatchedQty=IsNull(DispatchedQty,0)-IsNull(DispatchDetail.AllocationQuantity,0) From tblTrackEstimation,(Select EstimationId, ArticleDefId, SUM(IsNull(Qty,0)) as AllocationQuantity From DispatchDetailTable WHERE EstimationId=" & Val(eDt.Rows(0).Item(0).ToString) & " And DispatchId=" & Val(Me.txtReceivingID.Text) & " AND IsNull(EstimationId,0) <> 0 Group By EstimationId, ArticleDefId) as DispatchDetail WHERE DispatchDetail.EstimationId = tblTrackEstimation.EstimationId And DispatchDetail.ArticleDefId = tblTrackEstimation.ArticleId "
                        cm.ExecuteNonQuery()
                    Next
                End If
    ...
    Private Sub BtnRefresh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnRefresh.Click
        Try
            ...
            id = Me.cmbContractTicket.SelectedValue
            FillCombo("ContractTicket")
            Me.cmbContractTicket.SelectedValue = id
    ...
    Public Sub ApplyGridSettings(Optional ByVal Condition As String = "") Implements IGeneral.ApplyGridSettings
        Try
            For Each col As Janus.Windows.GridEX.GridEXColumn In Me.grd.RootTable.Columns
                If col.Index <> grdEnm.LocationId AndAlso col.Index <> grdEnm.Qty AndAlso col.Index <> grdEnm.TotalQty AndAlso col.Index <> grdEnm.Rate AndAlso col.Index <> grdEnm.LotNo AndAlso col.Index <> grdEnm.Rack_No AndAlso col.Index <> grdEnm.Engine_No AndAlso col.Index <> grdEnm.Chassis_No AndAlso col.Index <> grdEnm.Comments Then
    ...
    Private Sub CtrlGrdBar1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CtrlGrdBar1.Load
        Try
            If IO.File.Exists(Application.ExecutablePath & "\..\Layouts\" & Me.Name & "_" & Me.grd.Name) Then
                ...
                fs.Dispose()
            End If
            Me.CtrlGrdBar1.txtGridTitle.Text = CompanyTitle & Chr(10) & "Store Issuance"
        Catch ex As Exception
            ShowErrorMessage(ex.Message)
        End Try
    End Sub
    ...
    Private Sub cmbDepartment_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbDepartment.SelectedIndexChanged
        Try
            If Not cmbDepartment.SelectedValue <= 0 AndAlso Me.cmbTicket.SelectedValue > 0 Then
                'DisplayAllocationDepartmentWise(Me.cmbDepartment.SelectedValue, Me.cmbTicket.SelectedValue)
        ...
        Try
            If Me.grd.GetRow.RowType = Janus.Windows.GridEX.RowType.Record Then
                ...
                If IsEstimation = True Then
                    ...
                    End If
                Else
                    If e.Column.Key = "Qty" AndAlso Me.grd.GetRow.Cells("EstimationId").Value > 0 Then
                        If Me.grd.GetRow.Cells("Qty").Value > Me.grd.GetRow.Cells("CheckQty").Value Then
                            Dim Difference As Double = (Me.grd.GetRow.Cells("Qty").Value - Me.grd.GetRow.Cells("CheckQty").Value)
                            If (Difference + Me.grd.GetRow.Cells("IssuedQty").Value) > Me.grd.GetRow.Cells("EstimatedQty").Value Then
                                msg_Error("Quantity exceeds estimated quantity")
                                Me.grd.GetRow.Cells("Qty").Value = CheckQty
                                Me.grd.GetRow.Cells("TotalQty").Value = CheckQty
                            End If
    ...
    Private Sub cmbBatchNo_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbBatchNo.SelectedIndexChanged
        Try
            If Me.cmbBatchNo.SelectedIndex > 0 Then
    ...
    Private Sub cmbCostSheetItems_InitializeLayout(sender As Object, e As Win.UltraWinGrid.InitializeLayoutEventArgs) Handles cmbCostSheetItems.InitializeLayout
    ...
    Private Sub CtrlGrdBar2_Load(sender As Object, e As EventArgs) Handles CtrlGrdBar2.Load
        Try
            If IO.File.Exists(Application.ExecutablePath & "\..\Layouts\" & Me.Name & "_" & Me.grdSaved.Name) Then
                ...
                fs.Dispose()
            End If
            Me.CtrlGrdBar2.txtGridTitle.Text = CompanyTitle & Chr(10) & "Store Issuance"
        Catch ex As Exception
            ShowErrorMessage(ex.Message)
        End Try
    End Sub
    ...
    Private Sub txtPackQty_TextChanged(sender As Object, e As EventArgs) Handles txtPackQty.TextChanged
        ...
        End Try
    End Sub
    'rafay created this function for stock click
    ...
    Private Sub cmbPo_SelectedValueChanged(sender As Object, e As EventArgs) Handles cmbPo.SelectedValueChanged
        Try
            Dim str As String
            str = "SELECT ContractId from ContractMasterTable where ContractNo = '" & cmbPo.Text & "'"
            Dim dt As DataTable = GetDataTable(str)
            If dt.Rows.Count > 0 Then
                Dim str1 As String = "select TicketId, TicketNo from TicketMasterTable where ContractId = " & dt.Rows(0).Item("ContractId") & " AND Status = 'Close'"
                FillDropDown(cmbContractTicket, str1)
            End If
        Catch ex As Exception
            ShowErrorMessage(ex.Message)
        End Try
