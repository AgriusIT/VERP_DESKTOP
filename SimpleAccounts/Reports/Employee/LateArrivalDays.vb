'Ali Faisal : TFS1482 : Add report for Late Arrival Days
Imports System.Windows.Forms
Imports System.Data.OleDb
Imports SBModel
Imports SBDal
Imports SBUtility
Partial Class LateArrivalDays
    Implements IGeneral
    ...
    End Enum
    ''' <summary>
    ''' Ali Faisal : TFS1482 : Period selection to show records in that period
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks>Ali Faisal : TFS1482</remarks>
    ...
    End Sub
    ''' <summary>
    ''' Ali Faisal : TFS1482 : Form loading and fill dropdowns
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    ...
    End Sub
    ''' <summary>
    ''' Ali Faisal : TFS1482 : Apply security rights
    ''' </summary>
    ''' <remarks></remarks>
    ...
    End Sub
    ''' <summary>
    ''' Ali Faisal : TFS1482 : Get all records on the basis of provided criteria
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub GetAllRecords()
        Try
            Dim str As String = ""
            Dim dt As DataTable
            str = "SELECT     Emp.Employee_ID, Emp.Employee_Name AS [Emp Name], Emp.Employee_Code AS [Emp Code], Emp.CostCentre AS CostCenterId, tblDefCostCenter.Name AS CostCenter, EmployeeDesignationDefTable.EmployeeDesignationName AS Designation, EmployeeDeptDefTable.EmployeeDeptName AS Department, EmpIn.Date, EmpIn.Time " _
                ...
                & "(SELECT     tblAttendanceDetail_1.EmpId, Convert(varchar(12),tblAttendanceDetail_1.AttendanceTime,108) AS Time, tblAttendanceDetail_1.AttendanceDate AS Date " _
                ...
                & "ShiftTable ON tblAttendanceDetail_1.ShiftId = ShiftTable.ShiftId WHERE (CONVERT(DateTime, tblAttendanceDetail_1.AttendanceDate, 102) BETWEEN CONVERT(DateTime, '" & Me.dtpFromDate.Value.ToString("yyyy-MM-dd 00:00:00") & "', 102) AND CONVERT(DateTime, '" & Me.dtpToDate.Value.ToString("yyyy-MM-dd 23:59:59") & "', 102)) AND (tblAttendanceDetail_1.AttendanceStatus NOT IN ('Short Leave', 'Half Leave', 'Short Absent')) AND CONVERT(varchar(8), tblAttendanceDetail_1.AttendanceTime, 108) >= '" & Me.dtpAttendanceTime.Value.ToString("HH:mm:ss") & "' GROUP BY tblAttendanceDetail_1.EmpId, tblAttendanceDetail_1.AttendanceTime, tblAttendanceDetail_1.AttendanceDate " _
                & " ) AS EmpIn ON Emp.Employee_ID = EmpIn.EmpId " _
                & "WHERE (Emp.Employee_ID <> 0) AND (EmpIn.Time <> '') "
    ...
    End Sub
    ''' <summary>
    ''' Ali Faisal : TFS1482 : Apply grid settings to show/hide columns and formating
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub ApplyGridSettings()
        Try
            Me.grd.RootTable.Columns(enmgrd.EmpId).Visible = False
            Me.grd.RootTable.Columns(enmgrd.CostCenterId).Visible = False
            Me.grd.RootTable.Columns(enmgrd.AttendanceDate).AggregateFunction = Janus.Windows.GridEX.AggregateFunction.Count
            Me.grd.RootTable.Columns(enmgrd.AttendanceDate).FormatString = "" & str_DisplayDateFormat
            Me.grd.RootTable.Columns(enmgrd.Time).FormatString = DateTimePickerFormat.Time
            Me.grd.GroupByBoxVisible = True
            Me.grd.AutomaticSort = False
            Dim gridgrp As New Janus.Windows.GridEX.GridEXGroup(Me.grd.RootTable.Columns(enmgrd.EmpName))
            gridgrp.GroupPrefix = String.Empty
            Me.grd.RootTable.Groups.Add(gridgrp)
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ''' <summary>
    ''' Ali Faisal : TFS1482 : Load control
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub CtrlGrdBar1_Load(sender As Object, e As EventArgs) Handles CtrlGrdBar1.Load
        Try
            If IO.File.Exists(Application.ExecutablePath & "\..\Layouts\" & Me.Name & "_" & Me.grd.Name) Then
                ...
                fs.Dispose()
            End If
            Me.CtrlGrdBar1.txtGridTitle.Text = CompanyTitle & Chr(10) & "Late arrival days report" & Chr(10) & "From Date: " & dtpFromDate.Value.ToString("dd-MM-yyyy").ToString & Chr(10) & "To Date: " & dtpToDate.Value.ToString("dd-MM-yyyy").ToString & ""
        Catch ex As Exception
            ShowErrorMessage(ex.Message)
        End Try
    End Sub
