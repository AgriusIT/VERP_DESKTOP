...
Imports SBDal
Imports SBModel
Public Class frmEmployeeWiseLedger
    ...
    Private Sub ApplySecurityRights()
        Try
            If LoginGroup = "Administrator" Then
                Me.Visible = True
                'Me.optDetail.Enabled = True
                'Me.optSummary.Enabled = True
                'Me.rbtPrint.Enabled = True
                'Me.rbtView.Enabled = True
                'Me.rbtDetail.Enabled = True
                'Me.rbtSummary.Enabled = True
            ...
            End If
            Me.Visible = False
            'Me.optDetail.Enabled = False
            'Me.optSummary.Enabled = False
            'Me.rbtPrint.Enabled = False
            'Me.rbtView.Enabled = False
            'Me.rbtDetail.Enabled = False
            ' Me.rbtSummary.Enabled = False
            ...
            For i As Integer = 0 To Rights.Count - 1
                ...
                ElseIf Rights.Item(i).FormControlName = "Field Chooser" Then
                    Me.CtrlGrdBar3.mGridChooseFielder.Enabled = True
                    'ElseIf Rights.Item(i).FormControlName = "Detail" Then
                    'Me.rbtDetail.Enabled = True
                    'Me.optDetail.Enabled = True
                    'ElseIf Rights.Item(i).FormControlName = "Summary" Then
                    'Me.rbtSummary.Enabled = True
                    'Me.optSummary.Enabled = True
                End If
    ...
    Private Sub rptLedger_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            Dim Str As String = "SELECT TOP 100 PERCENT coa_detail_id, detail_title, detail_code, account_type, sub_sub_title, sub_title, main_title, main_type,cityname as City, TerritoryName as Territory, isnull(Active,0) as Active,dbo.vwCOADetail.Contact_Email as Email FROM dbo.vwCOADetail WHERE     (coa_detail_id > 0) AND main_type NOT IN ('Income') AND account_type NOT IN ('Vendor','Customer','Bank','Cash','Inventory') " & IIf(flgCompanyRights = True, " AND vwCOADetail.CompanyId=" & MyCompanyId, "") & "  " & IIf(LoginGroup = "Administrator", "", " and Isnull(AccessLevel,'Everyone')='Everyone'") & " order by detail_title"
        ...
        Try
            If Me.pnlCost.Visible = True Then
                ...
                If Me.cmbCostCenterHead.SelectedIndex > 0 Then
                    ' Str = "select * from tbldefCostCenter where CostCenterGroup='" & Me.cmbCostCenterHead.Text & "'" ''Commented Aganist TFS3320
                    Str = "If exists(select CostCentre_Id FROM tblUserCostCentreRights where UserID = " & LoginUserId & " ) SELECT * FROM tblDefCostCenter where CostCenterID in (select CostCentre_Id FROM tblUserCostCentreRights where UserID = " & LoginUserId & " ) And CostCenterGroup='" & Me.cmbCostCenterHead.Text & "' Else SELECT  CostCenterID,Name FROM tblDefCostCenter WHERE CostCenterGroup='" & Me.cmbCostCenterHead.Text & "'"
                ...
                Else
                    '' Str = "select * from tbldefCostCenter " ''Commented Aganist TFS3320
                    Str = "If exists(select CostCentre_Id FROM tblUserCostCentreRights where UserID = " & LoginUserId & " ) SELECT  * FROM tblDefCostCenter  where CostCenterID in (select CostCentre_Id FROM tblUserCostCentreRights where UserID = " & LoginUserId & " ) Else SELECT  * FROM tblDefCostCenter where 1=1 "
                    If Me.chkIncludeCostCenter.Checked = True Then
    ...
    Private Sub FillCombo(Optional ByVal Condition As String = "")
        Try
            ...
            If Condition = "CostCenter" Then
                ...
                '' str = "select * from tbldefCostCenter " TFS3320
                str = "If exists(select CostCentre_Id FROM tblUserCostCentreRights where UserID = " & LoginUserId & " ) SELECT  * FROM tblDefCostCenter  where CostCenterID in (select CostCentre_Id FROM tblUserCostCentreRights where UserID = " & LoginUserId & " ) Else SELECT  * FROM tblDefCostCenter where 1=1 "
                If Me.chkIncludeCostCenter.Checked = True Then
                    str += " And Active IN(1,0,NULL)"
                Else
            ...
            ElseIf Condition = "CurrCostCenter" Then
                str = "select * from tbldefCostCenter order by sortorder , name " ''TFS3320
                FillDropDown(Me.cmbCurrCostCentre, str)
            ElseIf Condition = "AdvanceRequest" Then
                str = "SELECT AdvanceRequestTable.RequestID, AdvanceRequestTable.RequestNo FROM tblDefAdvancesType INNER JOIN AdvanceRequestTable ON tblDefAdvancesType.Id = AdvanceRequestTable.Advance_TypeID WHERE (tblDefAdvancesType.SalaryDeduct = 1) and AdvanceRequestTable.RequestStatus = 'Approved'"
    ...
    Public Sub GetLedger()
        ...
        Try
            ...
            RequestId = cmbAdvanceRequest.SelectedValue
        ...
        Try
            ...
            If chkMultiCurrency.Checked = False Then
                str = "  SELECT  ISNULL(Opening.Opening, 0) AS Opening, COA.coa_detail_id, COA.detail_title, " & IIf(CurrencySymbol = "PKR", " ISNULL(Trans.Debit_Amount, 0) As debit_amount", " Case When IsNull(Trans.Currency_Debit_Amount, 0) > 0 Then Trans.Currency_Debit_Amount Else ISNULL(Trans.Debit_Amount, 0) End AS debit_amount ") & ", " _
                          ...
                          & " WHERE VD.coa_detail_id=" & _AccountId & " AND (CONVERT(varchar, V.Voucher_Date, 102) < CONVERT(Datetime, '" & dptFromDate.ToString("yyyy-M-d 00:00:00") & "', 102)) " & IIf(_Post = False, " AND V.Post=1 ", "") & " " & IIf(_CostCenter > 0, " AND VD.CostCenterID=" & _CostCenter & "", "") & " " & IIf(Me.lstEmployee.SelectedIDs.Length > 0, " AND VD.EmpID IN (" & Me.lstEmployee.SelectedIDs & ") ", "") & " " & IIf(RequestId > 0, " AND VD.RequestId = (" & RequestId & ") ", "") & " " _
                          ...
                          & " WHERE V_D.coa_detail_id=" & _AccountId & " AND (CONVERT(Varchar, V.Voucher_Date, 102) BETWEEN CONVERT(Datetime, '" & dptFromDate.ToString("yyyy-M-d 00:00:00") & "', 102) AND CONVERT(Datetime, '" & dptToDate.ToString("yyyy-M-d 23:23:59") & "', 102)) " & IIf(_Post = False, " AND V.Post=1 ", "") & " " & IIf(_CostCenter > 0, " AND V_D.CostCenterId=" & _CostCenter & "", "") & " " & IIf(Me.lstEmployee.SelectedIDs.Length > 0, " AND V_D.EmpID IN (" & Me.lstEmployee.SelectedIDs & ") ", "") & " " & IIf(RequestId > 0, " AND V_D.RequestId = (" & RequestId & ") ", "") & " )  " _
                          & " Trans ON Trans.coa_detail_id = COA.coa_detail_id WHERE Coa.coa_detail_id=" & _AccountId & " AND (Opening.Opening > 0 OR Trans.debit_amount > 0 OR Trans.credit_amount > 0) ORDER By Convert(Datetime, Voucher_Date, 102) Asc "
            Else
    ...
    Private Sub grdLedger_LinkClicked(ByVal sender As System.Object, ByVal e As Janus.Windows.GridEX.ColumnActionEventArgs) Handles grdLedger.LinkClicked
        ...
        Try
            ...
            If e.Column.Key = "Voucher_Code" Then
                frmModProperty.Tags = Me.grdLedger.GetRow.Cells("Voucher_Code").Text
    ...
    Private Sub CtrlGrdBar3_Load(sender As Object, e As EventArgs) Handles CtrlGrdBar3.Load
        Try
            If IO.File.Exists(Application.ExecutablePath & "\..\Layouts\" & Me.Name & "_" & Me.grdLedger.Name) Then
                ...
                fs.Dispose()
            End If
            Me.CtrlGrdBar3.txtGridTitle.Text = CompanyTitle & Chr(10) & "Employee Ledger"
        Catch ex As Exception
            ShowErrorMessage(ex.Message)
        End Try
    End Sub

    Private Sub rbActive_CheckedChanged(sender As Object, e As EventArgs) Handles rbActive.CheckedChanged, rbInactive.CheckedChanged
        '' TASK TFS4904
        If isFormLoaded = False Then Exit Sub
        If Me.rbActive.Checked = True Then
            FillListBox(Me.lstEmployee.ListItem, "SELECT Employee_Id, Employee_Code + ' ~ ' + Employee_Name Employee_Name FROM tblDefEmployee WHERE ISNULL(Active, 0) = 1")
            _SearchDt = CType(Me.lstEmployee.ListItem.DataSource, DataTable)
            _SearchDt.AcceptChanges()
            Me.lstEmployee.DeSelect()
        Else
            FillListBox(Me.lstEmployee.ListItem, "SELECT Employee_Id, Employee_Code + ' ~ ' + Employee_Name Employee_Name FROM tblDefEmployee WHERE IsNull(Active, 0) = 0")
            _SearchDt = CType(Me.lstEmployee.ListItem.DataSource, DataTable)
            _SearchDt.AcceptChanges()
            Me.lstEmployee.DeSelect()
        End If
