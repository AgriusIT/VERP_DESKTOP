...
''TAKS TFS4695 Muhammad Amin done on 08-10-2018. Split Agieng Receivable report into parts.
...
Public Class frmGrdRptAgingReceiveables
    ...
    Private Sub FillGrid()
        Try
            ''Below line is commented against Task TFS3350
            'Dim str As String = "SP_Rpt_Receivable '" & Date.Now.ToString("yyyy-M-d 23:59:59") & "', " & strAging & ", " & str1stAging & ", '" & str1stAgingName & "'," & str2ndAging & ", '" & str2ndAgingName & "'," & str3rdAging & ", '" & str3rdAgingName & "'," & IIf(Me.chkIncludeUnPosted.Checked = True, 1, 0) & ", '" & 0 & "', '" & IIf(Me.chkShowWithOutCostCenter.Checked = True, "0", "" & 0 & "") & "', '" & 0 & "','" & 0 & "','" & 0 & "'"
            Dim str As String = "SP_Rpt_Receivable '" & Date.Now.ToString("yyyy-M-d 23:59:59") & "', " & strAging & ", " & str1stAging & ", '" & str1stAgingName & "'," & str2ndAging & ", '" & str2ndAgingName & "'," & str3rdAging & ", '" & str3rdAgingName & "'," & IIf(Me.chkIncludeUnPosted.Checked = True, 1, 0) & ", '" & lstSubSubAccount.SelectedIDs & "', '" & IIf(Me.chkShowWithOutCostCenter.Checked = True, "0", "" & Me.lstCostCenter.SelectedIDs & "") & "', '" & lstRegion.SelectedIDs & "','" & lstZone.SelectedIDs & "','" & lstBelt.SelectedIDs & "', " & IIf(cmbPropertyType.SelectedValue > 0, cmbPropertyType.SelectedValue, 0) & ""
            ...
            Me.grd.RootTable.Columns("coa_detail_id").Visible = False ''TFS4897
            Me.grd.RootTable.Columns("main_sub_sub_id").Visible = False ''TFS4897
            Me.grd.RootTable.Columns("Balance").FormatString = "N" & DecimalPointInValue ''TFS4897
            Me.grd.RootTable.Columns("Current_Amount").FormatString = "N" & DecimalPointInValue ''TFS4897
            Me.grd.RootTable.Columns("30_60").FormatString = "N" & DecimalPointInValue ''TFS4897
            Me.grd.RootTable.Columns("60_90").FormatString = "N" & DecimalPointInValue ''TFS4897
            Me.grd.RootTable.Columns("90+").FormatString = "N" & DecimalPointInValue ''TFS4897
            Me.grd.RootTable.Columns("Balance").TotalFormatString = "N" & DecimalPointInValue ''TFS4897
            Me.grd.RootTable.Columns("Current_Amount").TotalFormatString = "N" & DecimalPointInValue ''TFS4897
            Me.grd.RootTable.Columns("30_60").TotalFormatString = "N" & DecimalPointInValue ''TFS4897
            Me.grd.RootTable.Columns("60_90").TotalFormatString = "N" & DecimalPointInValue ''TFS4897
            Me.grd.RootTable.Columns("90+").TotalFormatString = "N" & DecimalPointInValue ''TFS4897
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        Try
            ...
            'AddRptParam("@ToDate", Now.Date.ToString("yyyy-M-d 23:59:59"))
            ...
            ''Below line is commented against TASK TFS4695
            'ShowReport("AgeingReceivable", "" & IIf(Me.lstSubSubAccount.SelectedIDs <> "", " {SP_Rpt_Receivable.main_sub_sub_id} IN [" & Me.lstSubSubAccount.SelectedIDs & "]", "Nothing") & "", "Nothing", Now.Date.ToString("yyyy-M-d 23:59:59"), False) ''{SP_Rpt_Receivable;1.main_sub_sub_id} =
            If grd.RowCount > 0 Then
                Dim dtReport As DataTable = CType(Me.grd.DataSource, DataTable)
                dtReport.AcceptChanges()
                ShowReport("AgeingReceivable", , , , , , , dtReport)
            Else
    ...
    Private Sub FillCombo()
        ...
        Try
            ...
            FillDropDown(Me.cmbPropertyType, "SELECT PropertyTypeId, PropertyType FROM PropertyType", True)
...
#End Region
    ...
    Private Sub lstHeadCostCenter_SelectedIndexChaned(sender As Object, e As IndexEventArgs) Handles lstHeadCostCenter.SelectedIndexChaned
        Try
            ''TASK TFS3350
            If ArePropertyFilters = False Then
            ...
            Else
                If Me.lstHeadCostCenter.SelectedIDs.Length > 0 Then
                    If cmbPropertyType.SelectedIndex > 0 Then
                        FillListBox(Me.lstCostCenter.ListItem, "SELECT CostCenterID, Name FROM tblDefCostCenter WHERE Active = 1 And CostCenterGroup IN (" & Me.lstHeadCostCenter.SelectedItems & ") And CostCenterID In ( " _
                                     & " Select CostCenterID from PropertyProfile   where InvId in ( " _
                                     & " Select PropertyItemId from PropertyItem Inner Join PropertyType on PropertyItem.PropertyTypeId = PropertyType.PropertyTypeId where PropertyType.PropertyTypeId = " & cmbPropertyType.SelectedValue & ")) ")
                        lstCostCenter.SelectAll()
                    Else
                        FillListBox(Me.lstCostCenter.ListItem, "SELECT CostCenterID, Name FROM tblDefCostCenter WHERE Active = 1 And CostCenterGroup IN (" & Me.lstHeadCostCenter.SelectedItems & ") And CostCenterID In ( " _
                                     & " Select CostCenterID from PropertyProfile  where InvId in ( " _
                                     & " Select PropertyItemId from PropertyItem Inner Join PropertyType on PropertyItem.PropertyTypeId = PropertyType.PropertyTypeId )) ")
                        lstCostCenter.SelectAll()
                    End If
                Else
                    If cmbPropertyType.SelectedIndex > 0 Then
                        FillListBox(Me.lstCostCenter.ListItem, "SELECT CostCenterID, Name FROM tblDefCostCenter WHERE Active = 1 And CostCenterID In ( " _
                                     & " Select CostCenterID from PropertyProfile   where InvId in ( " _
                                     & " Select PropertyItemId from PropertyItem Inner Join PropertyType on PropertyItem.PropertyTypeId = PropertyType.PropertyTypeId where PropertyType.PropertyTypeId = " & cmbPropertyType.SelectedValue & ")) ")
                        lstCostCenter.SelectAll()
                    Else
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub btnLoadOld_Click(sender As Object, e As EventArgs) Handles btnLoadOld.Click
        Try
            frmMain.LoadControl("frmGrdRptAgingReceiveablesOld")
            UpdateConfigValue("True", "IsDisplayedOldReceivable")
        Catch ex As Exception
            ShowErrorMessage(ex.Message)
        End Try
    End Sub
    ...
    Private Sub cmbPropertyType_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbPropertyType.SelectedIndexChanged
        Try
            If ArePropertyFilters = True Then
                If cmbPropertyType.SelectedIndex > 0 Then
                    FillListBox(Me.lstHeadCostCenter.ListItem, "SELECT DISTINCT CostCenterGroup, CostCenterGroup FROM tblDefCostCenter WHERE (CostCenterGroup <> '') AND Active = 1 And CostCenterGroup In ( " _
                                 & " Select Branch.Name from PropertyProfile Left Outer Join Branch on PropertyProfile.BranchId =Branch.BranchId  where InvId in ( " _
                                 & " Select PropertyItemId from PropertyItem Inner Join PropertyType on PropertyItem.PropertyTypeId = PropertyType.PropertyTypeId where PropertyType.PropertyTypeId = " & cmbPropertyType.SelectedValue & "))")
                    FillListBox(Me.lstCostCenter.ListItem, "SELECT CostCenterID, Name FROM tblDefCostCenter WHERE Active = 1 And CostCenterID In ( " _
                                 & " Select CostCenterID from PropertyProfile   where InvId in ( " _
                                 & " Select PropertyItemId from PropertyItem Inner Join PropertyType on PropertyItem.PropertyTypeId = PropertyType.PropertyTypeId where PropertyType.PropertyTypeId = " & cmbPropertyType.SelectedValue & ")) ")
                ...
                Else
                    ...
                    FillListBox(Me.lstCostCenter.ListItem, "SELECT CostCenterID, Name FROM tblDefCostCenter WHERE Active = 1 And CostCenterID In ( " _
                                 ...
                                 & " Select PropertyItemId from PropertyItem Inner Join PropertyType on PropertyItem.PropertyTypeId = PropertyType.PropertyTypeId )) ")
                    lstHeadCostCenter.DeSelect()
                    lstCostCenter.DeSelect() ''TFS4897
                End If
            Else
                Exit Sub
            End If
