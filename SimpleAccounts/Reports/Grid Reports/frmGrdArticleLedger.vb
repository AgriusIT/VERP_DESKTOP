...
Imports SBModel
Imports System.Data
Public Class frmGrdArticleLedger
    ...
    Private Sub frmGrdArticleLedger_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            UltraDropDownSearching(Me.cmbItems, frmModProperty.blnListSeachStartWith, frmModProperty.blnListSeachContains)
            IsOpenedForm = True
    ...
    Private Sub GetSecurityRights()
        ...
        End Try
    End Sub
    Private Sub FillCombo(ByVal strCondition As String) ''TASK1191015
        ...
        If strCondition = "Item" Then
            ...
            FillUltraDropDown(Me.cmbItems, str)
            Me.cmbItems.Rows(0).Activate()
            Me.cmbItems.DisplayLayout.Bands(0).Columns("Id").Hidden = True
            Me.cmbItems.DisplayLayout.Bands(0).Columns("Size ID").Hidden = True
            Me.cmbItems.DisplayLayout.Bands(0).Columns("Stock").Hidden = True
            Me.cmbItems.DisplayLayout.Bands(0).Columns("PurchasePrice").Hidden = True
            Me.cmbItems.DisplayLayout.Bands(0).Columns("AccountId").Hidden = True '' Add New Column for Sales Account Id
            Me.cmbItems.DisplayLayout.Bands(0).Columns("ServiceItem").Hidden = True '' Task:2388 Servicie Item Column Hidden
            Me.cmbItems.DisplayLayout.Bands(0).Columns("SortOrder").Hidden = True
            Me.cmbItems.DisplayLayout.Bands(0).Columns("SalesAccountId").Hidden = True '' Task:2388 Servicie Item Column Hidden
            Me.cmbItems.DisplayLayout.Bands(0).Columns("CGSAccountId").Hidden = True

            If rbtnByName.Checked = True Then
                Me.cmbItems.DisplayMember = Me.cmbItems.Rows(0).Cells(2).Column.Key.ToString
            Else
                Me.cmbItems.DisplayMember = Me.cmbItems.Rows(0).Cells(1).Column.Key.ToString
            End If
        End If
    ...
    Private Sub btnGenerate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGenerate.Click
        Try
            ...
            ''22-6-2015 TASKM226151 Imran Ali Runnting Total Problem Fixed
            strQuery = "Select * From (SELECT 'Opening' AS DocNo, Convert(DateTime, '" & Me.dtpFrom.Value.AddDays(-1).ToString("dd-MMM-yyyy 00:00:00") & "',102) AS DocDate, 'OP' AS StockDocType, 'Opening Stock' AS Remarks, '' AS ProjectName, '' as [Account Code], '' as [Account Title], '' as City, '' as Area, " _
            ...
            & " dbo.ArticleDefView.ArticleSizeName, dbo.ArticleDefView.ArticleUnitName, dbo.ArticleDefView.PackQty, '' AS Comments, '' AS Engine_No, '' AS Chassis_No, 0 AS Rate,  " _
            ...
            & " Case When SUM(ISNULL(dbo.StockDetailTable.InQty, 0) - ISNULL(dbo.StockDetailTable.OutQty, 0)) <0 then 0 else SUM(ISNULL(dbo.StockDetailTable.Rate, 0) * ISNULL(dbo.StockDetailTable.InQty, 0) - ISNULL(dbo.StockDetailTable.OutQty, 0)) End AS In_Amount,  " _
            ...
            & " Abs(Case When SUM(ISNULL(dbo.StockDetailTable.InQty, 0) - ISNULL(dbo.StockDetailTable.OutQty, 0)) > 0 then 0 else SUM(ISNULL(dbo.StockDetailTable.Rate, 0) * ISNULL(dbo.StockDetailTable.InQty, 0) - ISNULL(dbo.StockDetailTable.OutQty, 0)) End) AS Out_Amount,Convert(decimal(18,4), 0) AS Qty_Balance, Convert(decimal(18,4), 0) AS Amount_Balance, 0 as StockTransDetailId " _
            ...
            & " CASE WHEN tblDefLocation_1.location_name <> '' THEN tblDefLocation_1.location_name ELSE dbo.vwCOADetail.detail_title END AS detail_title, vwCOADetail.CityName as City, vwCOADetail.TerritoryName as Area,  " _
            ...
            & " dbo.StockDetailTable.Engine_No, dbo.StockDetailTable.Chassis_No, dbo.StockDetailTable.Rate, dbo.StockDetailTable.InQty AS In_Qty,  " _
            & " dbo.StockDetailTable.InQty * dbo.StockDetailTable.Rate AS In_Amount, dbo.StockDetailTable.OutQty AS Out_Qty,  " _
            & " dbo.StockDetailTable.OutQty * dbo.StockDetailTable.Rate AS Out_Amount, 0 AS Qty_Balance, 0 AS Amount_Balance, dbo.StockDetailTable.StockTransDetailId " _
            & " FROM dbo.StockDetailTable INNER JOIN " _
            & " dbo.StockMasterTable ON dbo.StockDetailTable.StockTransId = dbo.StockMasterTable.StockTransId INNER JOIN " _
            & " dbo.tblDefLocation ON dbo.StockDetailTable.LocationId = dbo.tblDefLocation.location_id INNER JOIN " _
            & " dbo.ArticleDefView ON dbo.StockDetailTable.ArticleDefId = dbo.ArticleDefView.ArticleId LEFT OUTER JOIN " _
    ...
    Private Sub rbtByCode_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbtnByCode.CheckedChanged
        Try
            ...
            If Me.rbtnByCode.Checked = True Then
                Me.cmbItems.DisplayMember = Me.cmbItems.DisplayLayout.Bands(0).Columns("Code").Key.ToString

            Else
                Me.cmbItems.DisplayMember = Me.cmbItems.DisplayLayout.Bands(0).Columns("Item").Key.ToString
            End If
        Catch ex As Exception
            ShowErrorMessage(ex.Message)
        End Try
