...
Partial Class GroupWiseSalesReport
    ...
    Sub CallShowReport(Optional ByVal Print As Boolean = False)
        Try
            ...
            AddRptParam("@GroupId", Me.cmbDepartment.SelectedValue)
            ShowReport("rptGroupWiseSalesReport")
        Catch ex As Exception
            Throw ex
        Finally
    ...
    Private Sub SummaryofSalesTaxInvoices_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            ...
            FillDropDown(Me.cmbDepartment, "select * from ArticleGroupDefTable order by sortorder , ArticleGroupName", True)
            GetSecurityRights()
        Catch ex As Exception
            ShowErrorMessage(ex.Message)
        End Try
